...
export default async function AccountPage() {
  ...
  return <main className="section accountPage"><div className="shell">
    ...
    <div className="accountWelcome card"><div><span className="badge">{roleLabel}</span><h2>{profile?.full_name || user.email}</h2><p className="muted">{profile?.company_name ? `${profile.company_name}${profile.tax_number ? ` · ${profile.tax_number}` : ''}` : user.email}</p></div><Link className="btn btnPrimary" href="/webaruhaz">Új rendelés</Link></div>
    ...
    <section className="tableCard"><table className="adminTable"><thead><tr><th>Rendelés</th><th>Állapot</th><th>Összeg</th><th>Szállítás / fizetés</th><th>Dátum</th></tr></thead><tbody>{(orders ?? []).map(order=><tr key={order.id}><td><strong>{order.order_number}</strong></td><td><span className="badge">{order.status}</span></td><td>{formatHuf(order.total_gross_huf)}</td><td>{order.shipping_method ?? '—'} / {order.payment_method ?? '—'}</td><td>{new Intl.DateTimeFormat('hu-HU',{dateStyle:'short'}).format(new Date(order.created_at))}</td></tr>)}</tbody></table>{!orders?.length&&<p className="muted" style={{padding:20}}>Még nincs rendelésed.</p>}</section>
  </div></main>;
}
