...
type IdentityChangeRow={id:string;requested_name:string;requested_tax_number:string;created_at:string};
...
export default async function B2BAccountPage({searchParams}:{searchParams:Promise<{invite?:string}>}){
  ...
  const[{data:members,error:memberError},{data:invites,error:inviteError},{data:orders,error:orderError},{data:identityChange,error:identityError}]=await Promise.all([
    ...
    admin.from('b2b_account_identity_change_requests').select('id,requested_name,requested_tax_number,created_at').eq('instance_id',instance.id).eq('account_id',context.accountId).eq('status','pending').maybeSingle(),
  ]);
  ...
  const loadError=Boolean(memberError||inviteError||orderError||identityError||profileResult.error);
  const pendingIdentity=(identityChange as IdentityChangeRow|null);
  ...
  return <main className="section"><div className="shell">
    ...
    {!loadError&&<B2BIdentityPanel accountId={context.accountId} accountName={context.accountName} taxNumber={context.taxNumber} status={context.status} currentRole={context.memberRole} pending={pendingIdentity?{id:pendingIdentity.id,requestedName:pendingIdentity.requested_name,requestedTaxNumber:pendingIdentity.requested_tax_number,createdAt:pendingIdentity.created_at}:null}/>}
