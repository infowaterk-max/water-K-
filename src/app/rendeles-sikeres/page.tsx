...
export default async function OrderSuccess({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  const params=await searchParams; const parsed=z.string().uuid().safeParse(params.token);
  ...
  const admin=createAdminClient(); const {data:order,error}=await admin.from('orders').select('order_number,total_gross_huf,payment_method,status').eq('confirmation_token',parsed.data).maybeSingle();
  ...
  const cardPayment=order.payment_method==='kh_card'; const products=await getProducts();
  return <main className="section"><div className="shell confirmationShell"><span className="successMark" aria-hidden="true">✓</span><span className="eyebrow">Hitelesített rendelés</span><h1 className="sectionTitle">Köszönjük. A rendelésed megvan.</h1><p className="lead">Rendelési azonosító: <strong>{order.order_number}</strong></p><div className="confirmationTotal" aria-label={`Fizetendő ${formatHuf(order.total_gross_huf)}`}>{formatHuf(order.total_gross_huf)}</div><div className="card confirmationNextStep"><span className="badge">Következő lépés</span><h2>{cardPayment?'Bankkártyás fizetés – K&H':'Banki átutalás'}</h2><p className="muted">{cardPayment?'A rendelés rögzítve van. Fizetett állapotba kizárólag a hitelesített banki visszajelzés után kerül.':'A rendelési visszaigazolásban találod az átutaláshoz szükséges adatokat. A rendelés állapota a beérkezett fizetés után frissül.'}</p></div><div className="orderTimeline" aria-label="Rendelés folyamata"><div><strong>1</strong><span>Rendelés rögzítve</span></div><div><strong>2</strong><span>Fizetés ellenőrzése</span></div><div><strong>3</strong><span>Csomagolás</span></div><div><strong>4</strong><span>Szállítás</span></div></div><p className="helperText">Aktuális állapot: {String(order.status).replaceAll('_',' ')}</p><div className="actions"><Link className="btn btnPrimary" href="/fiokom">Rendeléseim</Link><Link className="btn btnGhost" href="/webaruhaz">Webáruház</Link></div><ProductRecommendations products={products} context="confirmation"/></div></main>;
}
