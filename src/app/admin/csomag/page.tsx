...
import { commercialAddonLabel } from '@/lib/plans/commercial-policy';
...
import {CommercialPackageSummary} from '@/components/admin/commercial-package-summary';
...
export default async function PackagePage({searchParams}:Props){
  ...
  return <section className="adminMain">
    ...
    <section className="card"><span className="eyebrow">Külön aktiválható kiegészítők</span><h2>Add-onok Alap és Pro mellé</h2><p className="muted">Az Add-on külön entitlement: nem válik automatikusan Pro funkcióvá, és kikapcsoláskor a konfigurációját nem töröljük. A kereskedelmi árpolicy önmagában nem kapcsol entitlementet.</p><div className="cards">{Object.values(ADDONS).map(addon=>{const compatible=addon.compatiblePlans.includes(current);const active=enabledAddons.includes(addon.code);return <article className="card" key={addon.code}><span className="badge">{active?'Aktív':compatible?'Külön Add-on':`${PLANS[addon.compatiblePlans[0]].name} szükséges`}</span><h3>{addon.name}</h3><p>{addon.description}</p><p className="muted">{commercialAddonLabel(addon.code)}</p><p className="muted">Kompatibilis: {addon.compatiblePlans.map(plan=>PLANS[plan].name).join(' + ')}</p>{!compatible&&<p className="muted">Csomagváltáskor a korábbi Add-on konfiguráció megmarad, de új Add-on művelet nem futtatható.</p>}</article>})}</div></section>
  </section>;
}
