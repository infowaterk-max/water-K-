...
export default async function PackagePage({ searchParams }: PackagePageProps) {
  ...
  return <section className="adminContent">
    <div className="adminHeader"><div><p className="eyebrow">Shoperation</p><h1>Csomagkezelés</h1><p className="muted">A Shoperation Alap teljes értékű webshop. A Shoperation Pro a napi működés fölé épülő digitális iroda és üzleti intelligencia.</p></div><div className="card"><span className="muted">Aktív csomag</span><h2>Shoperation {currentPlan.name}</h2><p>{currentPlan.description}</p></div></div>
    {params.reason === 'pro-required' && <div className="card"><span className="badge">Pro funkció</span><h2>Ez a funkció a Shoperation Pro része</h2><p>{requestedLabel ?? 'A megnyitott üzleti modul'} a teljes értékű Shoperation Alap fölé épülő Pro képesség.</p></div>}
    ...
    <section className="card"><p className="eyebrow">Fejlesztési ütemterv</p><h2>Következő Pro képesség</h2><p>A külső API-hozzáférést csak a külön token-, scope-, rotáció-, rate limit- és auditmodell elkészülte után aktiváljuk. Addig nem jelenik meg aktív Pro jogosultságként.</p><ul>{PLANNED_PRO_FEATURES.map((feature) => <li key={feature}>{FEATURE_LABELS[feature]} · előkészítés alatt</li>)}</ul></section>
    ...
    {current === 'alap' ? <div className="card"><h2>Shoperation Pro bővítés</h2><p>A meglévő webshop és adatai megmaradnak; a digitális iroda és a fejlett üzleti modulok nyílnak meg.</p></div> : <div className="card"><h2>Az elkészült Pro funkciók aktívak</h2><p>A fejlesztési ütemtervben jelölt képességek és a külön extrák ettől függetlenül kezelhetők.</p></div>}
  </section>;
}
