...
import { EmailBuilderWordLikeShellV2 } from '@/components/admin/email-builder-wordlike-shell-v2';
...
export default async function EmailTemplateEditorPage({params}:{params:Promise<{id:string}>}){
  ...
  return <div className={styles.workspacePage}><EmailBuilderWordLikeShellV2 previewContext={previewContext}><EmailBuilderMobileShell><EmailBuilderEditor template={{id:template.id,name:template.name,status:template.status,activeVersionId:template.active_version_id,updatedAt:template.updated_at}} initialDocument={parsed.data} previewContext={previewContext}/></EmailBuilderMobileShell></EmailBuilderWordLikeShellV2></div>;
}
