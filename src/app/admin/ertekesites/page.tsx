...
export default async function SalesAdmin(){
 ...
 const sums=(summaryResult.data??[])as Summary[],opps=opportunityResult.data??[],tasks=taskResult.data??[],offers=offerResult.data??[],variants=variantResult.data??[],loadError=Boolean(summaryResult.error||opportunityResult.error||taskResult.error||offerResult.error||variantResult.error),canAct=!loadError,total=sums.reduce((x,r)=>x+Number(r.pipeline_net_huf||0),0),openCount=sums.reduce((x,r)=>x+Number(r.open_count||0),0),overdue=sums.reduce((x,r)=>x+Number(r.overdue_pipeline_net_huf||0),0);
 return <section className="adminMain">
  ...
  <section className="auditGuide"><div><span className="eyebrow">Mit jelentenek a számok?</span><h2>Valós, lezárható értékesítési munka</h2></div><p>A „Nyitott lehetőségek értéke” az összes jelenleg aktív lehetőség nettó értéke. A darabszám azt mutatja, hány nyitott lehetőség vár feldolgozásra. A lejárt érték olyan lehetőség, amelynek határideje már elmúlt.</p></section>
  {canAct?<CommercialRefresh/>:<p className="muted">Frissítés az adatok teljes betöltése után indítható.</p>}
  <div className="cards adminMetricCards"><div className="card"><span className="badge">Nyitott lehetőségek értéke</span><div className="price">{summaryResult.error?'—':formatHuf(total)}</div></div><div className="card"><span className="badge">Nyitott lehetőségek</span><div className="price">{summaryResult.error?'—':openCount}</div></div><div className="card"><span className="badge">Lejárt lehetőség</span><div className="price">{summaryResult.error?'—':formatHuf(overdue)}</div></div><div className="card"><span className="badge">Nyitott feladat</span><div className="price">{taskResult.error?'—':tasks.length}</div></div></div>
  <section className="card salesOpportunityCard"><h2>Legfontosabb lehetőségek</h2><div className="adminTableScroll adminMobileCardTableWrap"><table className="adminTable adminMobileCardTable salesOpportunityTable"><thead><tr><th>Prioritás</th><th>Csatorna</th><th>Típus</th><th>Várható nettó érték</th><th>Javasolt következő lépés</th><th>Művelet</th></tr></thead><tbody>{opps.map(x=>{const rfq=rfqContext(x.source);return <tr key={x.id}><td data-mobile-label="Prioritás"><strong>{x.priority_score}</strong></td><td data-mobile-label="Csatorna">{x.channel.toUpperCase()}</td><td data-mobile-label="Típus">{rfq?'Ajánlatkérés':readable(x.kind,kindLabels)}{rfq?<><br/><span className="muted">{rfq.summary||'B2B partnerkérés'}{rfq.note?` · ${rfq.note}`:''}</span></>:null}</td><td data-mobile-label="Várható nettó érték">{formatHuf(Number(x.expected_value_net_huf))}</td><td data-mobile-label="Következő lépés">{x.recommended_action??x.reason??'Nincs javaslat'}</td><td data-mobile-label="Művelet">{canAct?<><OpportunityActions id={x.id}/><OfferCreate opportunityId={x.id} variants={variants} defaultVariantId={rfq?.firstVariantId} defaultQuantity={rfq?.firstQuantity}/></>:<span className="muted">Adatbetöltés szükséges</span>}</td></tr>})}</tbody></table></div>{!loadError&&!opps.length&&<p className="muted">Nincs nyitott értékesítési lehetőség.</p>}</section>
