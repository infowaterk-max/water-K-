...
export default async function SalesAdmin(){
 ...
 const[summaryResult,opportunityResult,taskResult,offerResult,variantResult]=await Promise.all([
  ...
  a.from('commercial_opportunities').select('id,channel,kind,status,priority_score,expected_value_net_huf,probability_percent,due_at,reason,recommended_action').eq('instance_id',scope.instanceId).in('status',['open','in_progress']).order('priority_score',{ascending:false}).limit(100),
 ...
 const sums=(summaryResult.data??[])as Summary[],opps=opportunityResult.data??[],tasks=taskResult.data??[],offers=offerResult.data??[],variants=variantResult.data??[],loadError=Boolean(summaryResult.error||opportunityResult.error||taskResult.error||offerResult.error||variantResult.error),total=sums.reduce((x,r)=>x+Number(r.pipeline_net_huf||0),0),weighted=sums.reduce((x,r)=>x+Number(r.weighted_pipeline_net_huf||0),0),overdue=sums.reduce((x,r)=>x+Number(r.overdue_pipeline_net_huf||0),0);
 return <section className="adminMain">
  ...
  {loadError&&<div className="errorNotice"><strong>Az értékesítési adatok egy része most nem tölthető be.</strong></div>}
  ...
  <CommercialRefresh/>
  <div className="cards adminMetricCards"><div className="card"><span className="badge">Nyitott lehetőség</span><div className="price">{formatHuf(total)}</div></div><div className="card"><span className="badge">Várható érték</span><div className="price">{formatHuf(weighted)}</div></div><div className="card"><span className="badge">Lejárt lehetőség</span><div className="price">{formatHuf(overdue)}</div></div><div className="card"><span className="badge">Nyitott feladat</span><div className="price">{tasks.length}</div></div></div>
  <section className="card"><h2>Legfontosabb lehetőségek</h2><div className="adminTableScroll"><table className="adminTable"><thead><tr><th>Prioritás</th><th>Csatorna</th><th>Típus</th><th>Várható nettó érték</th><th>Becsült esély</th><th>Javasolt következő lépés</th><th>Művelet</th></tr></thead><tbody>{opps.map(x=><tr key={x.id}><td><strong>{x.priority_score}</strong></td><td>{x.channel.toUpperCase()}</td><td>{readable(x.kind,kindLabels)}</td><td>{formatHuf(Number(x.expected_value_net_huf))}</td><td>{Number(x.probability_percent).toFixed(0)}%</td><td>{x.recommended_action??x.reason??'Nincs javaslat'}</td><td><OpportunityActions id={x.id}/><OfferCreate opportunityId={x.id} variants={variants}/></td></tr>)}</tbody></table></div>{!loadError&&!opps.length&&<p className="muted">Nincs nyitott értékesítési lehetőség.</p>}</section>
  <div className="cards"><section className="card"><h2>Értékesítési feladatok</h2>{tasks.map(x=><div key={x.id}><p><strong>{x.priority} · {x.title}</strong><br/><span className="muted">{x.description??'Nincs leírás'}</span></p><TaskActions id={x.id}/></div>)}{!tasks.length&&<p className="muted">Nincs nyitott értékesítési feladat.</p>}</section><section className="card"><h2>Aktív ajánlatok</h2>{offers.map(x=><div key={x.id}><p><strong>{readable(x.status,statusLabels)} · {x.discount_percent}% kedvezmény</strong><br/><span className="muted">{x.quantity} db · elvárt minimum árrés {x.minimum_margin_percent}% · {x.total_net_huf==null?'még nincs jóváhagyott érték':formatHuf(Number(x.total_net_huf))}</span></p><OfferActions id={x.id} status={x.status}/></div>)}{!offers.length&&<p className="muted">Nincs aktív ajánlat.</p>}</section></div>
 </section>;
}
