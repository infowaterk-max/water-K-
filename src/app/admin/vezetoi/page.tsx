...
export default async function Page(){
  ...
  return <section className="adminMain">
    ...
    <div className="cards adminMetricCards">
      <div className="card"><span className="badge">Összes bevétel</span><div className="price">{ce?'—':formatHuf(revenue)}</div><p className="muted">fizetett rendelések</p></div>
      <div className="card"><span className="badge">Visszatérő vásárlók aránya</span><div className="price">{ce?'—':`${repeatRate.toFixed(1)}%`}</div><p className="muted">{ce?'Az adat most nem elérhető.':`${repeat} visszatérő / ${customers} fizető ügyfél`}</p></div>
    ...
    <div className="splitFeature">
      <section className="featurePanel">
        <span className="eyebrow">B2C</span><h2>{ce?'—':formatHuf(Number(retail?.revenue_gross_huf??0))}</h2>
        <div className="integrationList">
          <div><span>Fizető ügyfelek</span><strong>{ce?'—':retail?.paying_customers??0}</strong></div>
          <div><span>Visszatérő vásárlók aránya</span><strong>{ce?'—':`${Number(retail?.repeat_rate_percent??0).toFixed(1)}%`}</strong></div>
          <div><span>Átlagos rendelési érték</span><strong>{ce?'—':formatHuf(Number(retail?.aov_gross_huf??0))}</strong></div>
          <div><span>Átlagos történeti ügyfélérték</span><strong>{ce?'—':formatHuf(Number(retail?.ltv_gross_huf??0))}</strong></div>
          <div><span>90+ napja inaktív</span><strong>{ce?'—':retail?.inactive_90d_customers??0}</strong></div>
        </div>
      </section>
      <section className="featurePanel darkPanel">
        <span className="eyebrow">B2B / viszonteladó</span><h2>{ce?'—':formatHuf(Number(reseller?.revenue_gross_huf??0))}</h2>
        <div className="integrationList">
          <div><span>Fizető partnerek</span><strong>{ce?'—':reseller?.paying_customers??0}</strong></div>
          <div><span>Visszatérő partnerek aránya</span><strong>{ce?'—':`${Number(reseller?.repeat_rate_percent??0).toFixed(1)}%`}</strong></div>
          <div><span>Átlagos rendelési érték</span><strong>{ce?'—':formatHuf(Number(reseller?.aov_gross_huf??0))}</strong></div>
          <div><span>Átlagos történeti partnerérték</span><strong>{ce?'—':formatHuf(Number(reseller?.ltv_gross_huf??0))}</strong></div>
          <div><span>90+ napja inaktív</span><strong>{ce?'—':reseller?.inactive_90d_customers??0}</strong></div>
        </div>
      </section>
    </div>

    <section className="card">
      <span className="eyebrow">Megtartás első vásárlási hónap szerint</span>
      <h2>Visszatérnek-e az új ügyfelek?</h2>
    ...
    <section className="featurePanel">
      <span className="eyebrow">Döntési fókusz</span><h2>Hol kell most beavatkozni?</h2>
      <div className="integrationList">
        <div><span>Nyitott mentett kosarak</span><strong>{ge?'—':growth?.open_checkout_recoveries??0}</strong></div>
        <div><span>Esedékes ügyfélút-lépések</span><strong>{ge?'—':growth?.due_journey_steps??0}</strong></div>
        <div><span>Lejárt B2B újrarendelések</span><strong>{ge?'—':growth?.overdue_resellers??0}</strong></div>
        <div><span>Hamarosan esedékes B2B újrarendelések</span><strong>{ge?'—':growth?.due_soon_resellers??0}</strong></div>
      </div>
    </section>
  </section>;
}
