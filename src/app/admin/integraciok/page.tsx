...
export default async function IntegrationsAdmin({searchParams}:Props){
 ...
 let jobs:Job[]=[];let loadError=false;const counts:Record<string,number>={};
 ...
 try{
  ...
  Object.keys(statusLabels).forEach((key,index)=>{const result=countResults[index];if(result.error)loadError=true;counts[key]=result.count??0});
 ...
 return <section className="adminMain">
  ...
  {problemCount>0&&<div className="warningNotice"><strong>{problemCount} hibás vagy blokkolt feladat vár beavatkozásra.</strong> Előbb nyisd meg a kapcsolódó rendelést és ellenőrizd, hogy a külső szolgáltatónál valóban nem teljesült-e a művelet.</div>}
