...
const shippingLabels:Record<string,string>={
  ...
  external_gls_home:'GLS Házhozszállítás',external_mpl_home:'MPL Házhozszállítás',external_mpl_automata:'Posta / Csomagautomata',
  external_mpl_postapont:'Postapontok (COOP/MOL)',external_foxpost:'FOXPOST',external_gls_parcel:'GLS CsomagPont / Automata',
...
export default async function AdminOrderPage({params}:{params:Promise<{id:string}>}){
  ...
  const[
    ...
    supabase.from('orders').select('*').eq('id',id).eq('instance_id',scope.instanceId).maybeSingle(),
    supabase.from('order_items').select('id,product_name,variant_label,sku,quantity,unit_gross_huf,line_total_gross_huf').eq('order_id',id).eq('instance_id',scope.instanceId),
    supabase.from('order_events').select('id,event_type,from_status,to_status,metadata,created_at').eq('order_id',id).eq('instance_id',scope.instanceId).order('created_at',{ascending:false}),
    supabase.from('integration_jobs').select('id,kind,provider,status,attempt_count,last_error,created_at,updated_at').eq('order_id',id).eq('instance_id',scope.instanceId).order('created_at',{ascending:false}),
    admin.from('payment_attempts').select('id,provider_code,provider_reference,status,amount_huf,failure_code,failure_message,created_at,completed_at').eq('order_id',id).eq('instance_id',scope.instanceId).order('created_at',{ascending:false}),
  ]);
