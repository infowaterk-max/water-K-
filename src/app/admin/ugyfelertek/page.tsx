...
export default async function CustomerValueAdmin(){
 ...
 return <section className="adminMain">
  ...
  <div className="cards adminMetricCards"><div className="card"><span className="badge">Értékelt ügyfelek</span><div className="price">{rows.length}</div></div><div className="card"><span className="badge">Visszatérő ügyfél</span><div className="price">{repeat}</div></div><div className="card"><span className="badge">Arany + Platina</span><div className="price">{high}</div></div><div className="card"><span className="badge">Felhasználható pontok</span><div className="price">{liability.toLocaleString('hu-HU')} pont</div></div></div>
  <section className="card"><h2>Értékszintek</h2><div className="cards">{tiers.map(t=><div key={t.tier}><strong>{tierLabels[t.tier]}</strong><div className="price">{t.count}</div></div>)}</div></section>
