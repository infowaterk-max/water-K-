...
const readTimeLabel=(value:string|null)=>value?`${new Intl.DateTimeFormat('hu-HU',{hour:'2-digit',minute:'2-digit',timeZone:'Europe/Budapest'}).format(new Date(value))} ✓`:'Még nem olvasta';
...
export default async function TeamChatPage({searchParams}:{searchParams:Promise<SearchParams>}){
  ...
  const selectedLinks=selectedThread?links.filter(link=>link.thread_id===selectedThread.id):[];
  const selectedAttachments=selectedThread?attachments.filter(item=>item.thread_id===selectedThread.id):[];
  const linkedCount=(type:ObjectLink['object_type'])=>new Set(selectedLinks.filter(link=>link.object_type===type).map(link=>link.object_id)).size;
    ...
    <aside className="teamChatInfoPane" id="team-chat-info" aria-label="Beszélgetés adatai">
      {selectedThread?<>
        ...
        <section className="teamChatInfoSection"><div className="teamChatSectionLabel"><span>Résztvevők</span><b>{selectedParticipants.length}</b></div><div className="teamChatParticipantList">{selectedParticipants.map(participant=><div key={participant.user_id}><span className="teamChatAvatar">{initials(labelFor(participant.user_id))}</span><span><strong>{participant.user_id===actor.id?'Te':labelFor(participant.user_id)}</strong><TeamChatPresenceIndicator userId={participant.user_id}/><small>{participant.participant_role==='owner'?'Beszélgetésgazda':'Résztvevő'}</small></span>{isSelectedOwner&&participant.user_id!==actor.id&&<form action={managePrivateParticipantAction}><input type="hidden" name="threadId" value={selectedThread.id}/><input type="hidden" name="targetUserId" value={participant.user_id}/><input type="hidden" name="operation" value="remove"/><button aria-label={`${labelFor(participant.user_id)} eltávolítása`}>×</button></form>}</div>)}</div>
          <div className="teamChatContextKeyValue"><span>Beszélgetésgazda</span><strong>{selectedOwner?(selectedOwner.user_id===actor.id?'Te':labelFor(selectedOwner.user_id)):'—'}</strong></div>
        ...
        {isSelectedOwner&&selectedParticipants.length>1&&<section className="teamChatInfoSection"><h2>Gazda átadása</h2><form action={transferPrivateThreadOwnerAction} className="teamChatOwnerTransfer"><input type="hidden" name="threadId" value={selectedThread.id}/><select name="targetUserId" required defaultValue=""><option value="" disabled>Új beszélgetésgazda…</option>{selectedParticipants.filter(p=>p.user_id!==actor.id).map(p=><option key={p.user_id} value={p.user_id}>{labelFor(p.user_id)}</option>)}</select><button className="btn btnGhost">Átadás</button></form></section>}
        {canBusinessObjects&&<section className="teamChatInfoSection"><h2>Kapcsolt üzleti elemek</h2><div className="teamChatContextStats"><div><span>Árajánlatok</span><strong>{linkedCount('commercial_offer')}</strong></div><div><span>Support ticketek</span><strong>{linkedCount('support_ticket')}</strong></div><div><span>Feladatok</span><strong>{linkedCount('task')}</strong></div><div><span>Rendelések</span><strong>{linkedCount('order')}</strong></div><div><span>Visszáruk</span><strong>{linkedCount('return_case')}</strong></div></div></section>}
        <section className="teamChatInfoSection"><h2>Olvasottság</h2><div className="teamChatContextReads">{selectedParticipants.map(participant=><div key={`read-${participant.user_id}`}><span>{participant.user_id===actor.id?'Te':labelFor(participant.user_id)}</span><strong>{readTimeLabel(participant.last_read_at)}</strong></div>)}</div></section>
        {secureAttachments&&<section className="teamChatInfoSection"><div className="teamChatSectionLabel"><span>Megosztott fájlok</span><b>{selectedAttachments.length}</b></div><div className="teamChatContextFiles">{selectedAttachments.slice(0,4).map(item=><a key={item.id} href={`/api/admin/office/attachments/${item.id}`}><span>📎</span><strong>{item.original_name}</strong><small>{fileSize(item.byte_size)}</small></a>)}{!selectedAttachments.length&&<p>Nincs megosztott fájl.</p>}</div></section>}
        <section className="teamChatInfoSection"><h2>Adatvédelem és megőrzés</h2><p className="teamChatContextPrivacy">A beszélgetést csak az aktív résztvevők olvashatják. Owner/Admin szerepkör önmagában nem ad betekintést a privát chat tartalmába. 90 nap inaktivitás után archiválható, a tartalomra 12 hónapos megőrzési szabály alkalmazható.</p></section>
        <section className="teamChatInfoSection"><h2>Secure Attachments</h2><p className="teamChatContextPrivacy">{secureAttachments?'Aktív ennél a csomagnál. Csak ellenőrzött, privát fájlok jelennek meg és tölthetők le.':'Nincs bekapcsolva ennél a csomagnál. A chat továbbra is használható szöveges üzenetekkel és üzleti objektum-kapcsolásokkal.'}</p></section>
