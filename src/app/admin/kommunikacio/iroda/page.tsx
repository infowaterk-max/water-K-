...
export default async function CustomerEmailWorkspace({searchParams}:{searchParams:Promise<{q?:string;filter?:string}>}){
  ...
  const threads=(threadResult.data??[])as Thread[],threadIds=threads.map(thread=>thread.id);
  ...
  const messages=(messageResult.data??[])as Message[],messageIds=messages.map(message=>message.id);
  ...
  const attachments=(attachmentResult.data??[])as Attachment[],objectLinks=(objectLinkResult.data??[])as ObjectLink[];
  ...
  const visible=threads.filter(thread=>(filter==='all'||filter==='unread'&&unread(thread)||filter==='urgent'&&thread.priority==='urgent'||filter==='closed'&&thread.status==='closed'||filter==='open'&&thread.status==='open')&&(!needle||thread.subject.toLowerCase().includes(needle)||thread.customer_email?.toLowerCase().includes(needle)||orders.find(order=>order.id===thread.order_id)?.order_number.toLowerCase().includes(needle)));

  return <section className="adminMain">
    <div className="sectionIntro"><div><span className="eyebrow">Ügyféllevelezés · Alap + Pro</span><h1 className="sectionTitle">Webshop ↔ ügyfél e-mail munkatér</h1><p className="lead">Ez a felület kizárólag a webshop és az ügyfelek közötti e-mailes kommunikációhoz tartozik. A munkatársi beszélgetések külön Team Chatben zajlanak.</p></div><div className="adminToolbar">{canChat&&<Link className="btn btnGhost" href="/admin/kommunikacio/chat">Team Chat</Link>}{advancedEmail&&<Link className="btn btnGhost" href="/admin/kommunikacio/iroda/hub">Communication Hub</Link>}{advancedEmail&&<Link className="btn btnGhost" href="/admin/kommunikacio/felugyelet">Küldési felügyelet</Link>}<Link className="btn btnPrimary" href="/admin/kommunikacio/iroda/uj">Új e-mail</Link></div></div>
    <div className="adminAuditNotice"><strong>Külön kommunikációs csatorna</strong><p>Az ügyfél nem lehet Team Chat résztvevő, a belső Team Chat üzenetek pedig nem jelennek meg ezen az oldalon. Küldés csak külön jóváhagyott Digitális Iroda postafiókból aktiválható; a működő webshop jelenlegi e-mail címeit nem használjuk.</p></div>
    {!advancedEmail&&<div className="adminAuditNotice"><strong>Alap ügyféllevelezés</strong><p>Olvasás, válasz, új 1:1 e-mail, olvasatlan állapot, saját piszkozat és biztonságosan ellenőrzött csatolmány elérhető. A csapatkiosztás, objektumkapcsolatok, CC/BCC, több postafiók és delegáció Pro funkció.</p></div>}
    ...
    <div className="cards adminMetricCards"><article className="card"><span className="badge">Nyitott ügyféllevelek</span><div className="price">{threadResult.error?'—':threads.filter(thread=>thread.status==='open').length}</div></article><article className="card"><span className="badge">Olvasatlan bejövő</span><div className="price">{loadError?'—':threads.filter(unread).length}</div></article><article className="card"><span className="badge">Sürgős</span><div className="price">{threadResult.error?'—':threads.filter(thread=>thread.status==='open'&&thread.priority==='urgent').length}</div></article>{advancedEmail&&<article className="card"><span className="badge">Lejárt feladat</span><div className="price">{taskResult.error?'—':overdue.length}</div></article>}</div>

    <form className="adminToolbar"><input name="q" defaultValue={q} placeholder="Keresés téma, ügyfél e-mail vagy rendelés alapján"/><select name="filter" defaultValue={filter}><option value="open">Nyitott</option><option value="unread">Olvasatlan</option><option value="urgent">Sürgős</option><option value="closed">Lezárt</option><option value="all">Összes</option></select><button className="btn btnPrimary">Szűrés</button></form>
    ...
    <section><span className="eyebrow">{threadResult.error?'—':visible.length} találat</span><h2>Ügyféllevelezések</h2><div className="cards">
      {visible.map(thread=>{const threadMessages=messages.filter(message=>message.thread_id===thread.id).slice(0,20).reverse();const order=orders.find(item=>item.id===thread.order_id);const isUnread=unread(thread);const sendingConfigured=Boolean(thread.mailbox_key&&activeMailboxKeys.has(thread.mailbox_key)&&routedThreadIds.has(thread.id));const replyDraft=draftByThread.get(thread.id);return <article className="card" key={thread.id}>
        <div className="adminToolbar"><span className="badge">{priorityLabel[thread.priority]??thread.priority}</span>{isUnread&&<span className="badge">Olvasatlan</span>}{replyDraft&&<span className="badge">Saját piszkozat</span>}{order&&<Link className="textLink" href={`/admin/rendelesek/${order.id}`}>{order.order_number}</Link>}</div>
        <h3>{thread.subject}</h3><p className="muted">{thread.customer_email??'Nincs ügyfél e-mail'}{advancedEmail&&<> · {thread.assigned_to?labelFor(thread.assigned_to):'Nincs felelős'}</>}</p>
        {!loadError&&<form action={updateThreadAction} className="adminToolbar"><input type="hidden" name="threadId" value={thread.id}/><select name="priority" defaultValue={thread.priority}><option value="low">Alacsony</option><option value="normal">Normál</option><option value="high">Magas</option><option value="urgent">Sürgős</option></select><select name="status" defaultValue={thread.status}><option value="open">Nyitott</option><option value="closed">Lezárt</option></select>{advancedEmail&&<select name="assigneeUserId" defaultValue={thread.assigned_to??''}><option value="">Nincs felelős</option>{assignees.map(member=><option key={member.userId} value={member.userId}>{member.label}</option>)}</select>}<button className="btn btnGhost">Frissítés</button></form>}
        ...
        <div className="integrationList">{threadMessages.map(message=>{const job=message.communication_job_id?jobMap.get(message.communication_job_id):null;const messageAttachments=attachments.filter(item=>item.message_id===message.id);const links=objectLinks.filter(item=>item.message_id===message.id);return <div key={message.id}><div>
          <strong>{message.kind==='email_in'?'Ügyfél → webshop':'Webshop → ügyfél'}</strong>
          {message.kind==='email_out'&&message.author_id&&<><br/><span className="muted">Tényleges küldő: {labelFor(message.author_id)}{message.acting_for_user_id?` · ${labelFor(message.acting_for_user_id)} nevében`:''}</span></>}
          {message.subject&&<><br/>{message.subject}</>}{!!message.cc_emails?.length&&<><br/><span className="muted">CC: {message.cc_emails.join(', ')}</span></>}{!!message.bcc_emails?.length&&<><br/><span className="muted">BCC: {message.bcc_emails.join(', ')}</span></>}<br/><span className="muted" style={{whiteSpace:'pre-wrap'}}>{message.body}</span>
          {messageAttachments.length>0&&<div className="adminToolbar">{messageAttachments.map(item=><Link key={item.id} className="textLink" href={`/api/admin/office/attachments/${item.id}`}>📎 {item.original_name} · {fileSize(item.byte_size)}</Link>)}</div>}
          {advancedEmail&&links.length>0&&<div className="adminToolbar">{links.map(link=><span className="badge" key={link.id}>{objectLabel[link.object_type]} · {link.object_type==='order'?(orders.find(o=>o.id===link.object_id)?.order_number??shortId(link.object_id)):shortId(link.object_id)}</span>)}</div>}
        </div><span className="muted">{job?jobLabel[job.status]??job.status:new Intl.DateTimeFormat('hu-HU',{dateStyle:'short',timeStyle:'short',timeZone:'Europe/Budapest'}).format(new Date(message.created_at))}</span></div>})}</div>
    ...
    </div>{!threadResult.error&&!visible.length&&<div className="card"><p className="muted">Nincs a szűrésnek megfelelő ügyféllevelezés.</p></div>}</section>
  </section>;
}
