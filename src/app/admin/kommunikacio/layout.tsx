...
import './digital-office-support-alert.css';
...
async function DigitalOfficeAuthorizedNavigation(){
  ...
  const supportAccess=Boolean(access?.supportEnabled&&access?.canSupport);
  return <>
    <DigitalOfficeNavigation
      officeEmail={access?.officeEmail??false}
      supportEnabled={access?.supportEnabled??false}
    ...
    />
    <DigitalOfficeSupportAlert enabled={supportAccess}/>
  </>;
}
