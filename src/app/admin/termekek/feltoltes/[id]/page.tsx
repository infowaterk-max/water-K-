...
export default async function EditProductIntakePage({params}:{params:Promise<{id:string}>}){
  ...
  const[productResult,variantsResult,mediaResult,attributesResult,assignmentResult,productChannelsResult,globalChannelsResult]=await Promise.all([
  ...
  const product=productResult.data;if(productResult.error||!product||product.active||variantsResult.error||mediaResult.error||attributesResult.error||assignmentResult.error||productChannelsResult.error||globalChannelsResult.error)notFound();
  ...
  const media=(mediaResult.data??[]).map(item=>({id:item.id,name:item.original_name,url:admin.storage.from('product-media').getPublicUrl(item.storage_path).data.publicUrl}));
