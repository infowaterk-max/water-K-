...
export default async function Cashflow() {
  ...
  return <section className="adminMain">
    <span className="eyebrow">Pro · Pénzáram-tervezés</span><h1 className="sectionTitle">90 napos működési pénzáram-előrejelzés</h1><p className="lead">A bevételi oldal az utolsó 90 nap fizetett rendeléseinek napi átlagából becsül, a kiadási oldal pedig a rögzített beszerzési fizetési határidőket használja.</p>
    ...
    <ReportingLegend kinds={['fact', 'calculation']} />
    <section className="auditGuide"><div><span className="eyebrow">Fontos értelmezés</span><h2>Működési előrejelzés, nem bankszámla-egyenleg</h2></div><p>A becslés nem tartalmaz minden adót, bért, rezsit és egyéb pénzmozgást. A rendelési forgalom bruttó, a rögzített beszerzési kötelezettség a beszerzési adatokból jön, ezért ez korai figyelmeztető üzleti mutató, nem könyvelési cash-flow kimutatás.</p></section>
    ...
    <ReportingEvidence kind="fact">
      <section className="card"><h2>Kifizetési időablakok</h2><div className="integrationList"><div><span>Lejárt</span><strong>{pe ? '—' : formatHuf(Math.round(overdue))}</strong></div><div><span>0–30 nap</span><strong>{pe ? '—' : formatHuf(Math.round(due30))}</strong></div><div><span>31–60 nap</span><strong>{pe ? '—' : formatHuf(Math.round(due60))}</strong></div><div><span>61–90 nap</span><strong>{pe ? '—' : formatHuf(Math.round(due90))}</strong></div></div></section>
      <section className="card"><h2>Utolsó 90 nap fizetett forgalma</h2><div className="adminTableScroll"><table className="adminTable"><thead><tr><th>Hónap</th><th>Rendelés</th><th>Bruttó forgalom</th></tr></thead><tbody>{history.map(([m, v]) => <tr key={m}><td><strong>{m}</strong></td><td>{v.orders}</td><td><strong>{formatHuf(Math.round(v.revenue))}</strong></td></tr>)}</tbody></table></div>{!oe && history.length === 0 && <p className="muted">Még nincs fizetett rendelési előzmény az elmúlt 90 napból.</p>}</section>
    </ReportingEvidence>
  </section>;
}
