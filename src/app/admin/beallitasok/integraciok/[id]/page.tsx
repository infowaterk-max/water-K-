...
export default async function IntegrationJobDetail({params}:{params:Promise<{id:string}>}){
  ...
  let order:null|{order_number:string;status:string;customer_email:string;total_gross_huf:number}=null;let orderContextError=false;
  if(job.order_id){const {data,error:orderError}=await admin.from('orders').select('order_number,status,customer_email,total_gross_huf').eq('id',job.order_id).eq('instance_id',scope.instanceId).maybeSingle();order=data;if(orderError||!data)orderContextError=true;}
  ...
  return <section className="adminMain">
    ...
    {orderContextError&&<div className="errorNotice" role="alert"><strong>A kapcsolódó rendelés most nem ellenőrizhető.</strong> Kézi újrafuttatást addig nem engedünk.</div>}{job.status==='processing'&&processingAgeMinutes>=30&&<div className="errorNotice" role="alert"><strong>A feladat több mint {processingAgeMinutes} perce feldolgozás alatt van.</strong> Ellenőrizd a szolgáltatói oldalt és a naplót, mielőtt kézi ismétlést indítasz.</div>}
    ...
    <section className="card"><div className="adminToolbar"><div><h2>Művelet</h2><p className="muted">Sikeres vagy éppen futó feladat nem indítható újra. A kézi újrafuttatás bekerül az admin auditnaplóba.</p></div><IntegrationJobControl id={job.id} disabled={orderContextError||job.status==='processing'||job.status==='succeeded'}/></div></section>
  </section>;
}
