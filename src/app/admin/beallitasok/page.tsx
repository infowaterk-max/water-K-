...
export default async function AdminSettingsPage(){
  ...
  return <section className="adminMain">
    ...
    <div className="tableCard"><h2>Integrációs outbox</h2><table className="adminTable"><thead><tr><th>Provider</th><th>Művelet</th><th>Állapot</th><th>Próba</th><th>Hiba</th><th>Időpont</th></tr></thead><tbody>{jobs.map(job=><tr key={job.id}><td>{job.provider}</td><td>{job.kind}</td><td><span className="badge">{job.status}</span></td><td>{job.attempt_count}</td><td>{job.last_error??'—'}</td><td>{new Intl.DateTimeFormat('hu-HU',{dateStyle:'short',timeStyle:'short'}).format(new Date(job.created_at))}</td></tr>)}</tbody></table>{jobs.length===0&&<p className="muted" style={{padding:20}}>Még nincs integrációs feladat.</p>}</div>
