...
export default async function AdminSettingsPage(){
  ...
  let dbHealthy=false;let dataLoadError=false;let jobLoadError=false;let webhookLoadError=false;let pingLoadError=false;
  try{
    ...
    jobLoadError=Boolean(jobResult.error);webhookLoadError=Boolean(webhookResult.error);pingLoadError=Boolean(pingResult.error);dataLoadError=jobLoadError||webhookLoadError||pingLoadError;
    if(!jobLoadError&&jobResult.data)jobs=jobResult.data;
    if(!webhookLoadError&&webhookResult.data)webhooks=webhookResult.data;
    dbHealthy=!pingLoadError;
  ...
  const operationalRisk=dataLoadError?null:problems+staleProcessing+invalidWebhooks+(cronConfigured?0:1)+(serverKeyConfigured?0:1);
  return <section className="adminMain settingsPage">
    ...
    <div className="cards"><section className="card"><span className="badge">Adatbázis</span><h2>{pingLoadError?'Nem ellenőrizhető':dbHealthy?'Elérhető':'Hiba'}</h2><p className="muted">Szerveroldali Supabase lekérdezési próba.</p></section><section className="card"><span className="badge">Admin kulcs</span><h2>{serverKeyConfigured?'Konfigurálva':'Hiányzik'}</h2><p className="muted">A szerveroldali admin kulcs értékét nem jelenítjük meg.</p></section><section className="card"><span className="badge">Worker</span><h2>{cronConfigured?'Védett':'Nincs kulcs'}</h2><p className="muted">CRON_SECRET csak jelenlét szerint ellenőrizve.</p></section><section className="card"><span className="badge">E-mail</span><h2>{emailConfigured?'Konfigurálva':'Nincs kulcs'}</h2><p className="muted">Tranzakciós e-mail provider kulcsának jelenléte.</p></section></div>
    <div className="cards"><section className="card"><span className="badge">Sorban</span><h2>{jobLoadError?'—':queued}</h2><p className="muted">Függő vagy éppen futó integrációs művelet.</p></section><section className="card"><span className="badge">Figyelmeztetés</span><h2>{jobLoadError?'—':problems}</h2><p className="muted">Blokkolt vagy sikertelen integrációs feladat.</p></section><section className="card"><span className="badge">Külső visszajelzés</span><h2>{webhookLoadError?'—':webhooks.length}</h2><p className="muted">Nem hiteles esemény: {webhookLoadError?'—':invalidWebhooks}.</p></section></div>
    <div className="tableCard"><div className="adminToolbar"><div><h2>Integrációs feladatok</h2><p className="muted">A belső feladatlista webshoponként elkülönítve mutatja a külső szolgáltatások felé indított műveleteket.</p></div><Link className="btn" href="/admin/integraciok">Teljes műveleti központ</Link></div><div className="adminTableScroll"><table className="adminTable"><caption className="srOnly">Integrációs outbox</caption><thead><tr><th scope="col">Szolgáltató</th><th scope="col">Művelet</th><th scope="col">Állapot</th><th scope="col">Próba</th><th scope="col">Hiba</th><th scope="col">Időpont</th><th scope="col">Művelet</th></tr></thead><tbody>{jobs.map(job=><tr key={job.id}><td><Link className="textLink" href={`/admin/beallitasok/integraciok/${job.id}`}><strong>{job.provider}</strong></Link></td><td>{kindLabel[job.kind]??job.kind}</td><td><span className="badge">{statusLabel[job.status]??job.status}</span></td><td>{job.attempt_count}</td><td>{job.last_error??'—'}</td><td>{new Intl.DateTimeFormat('hu-HU',{dateStyle:'short',timeStyle:'short'}).format(new Date(job.created_at))}</td><td><IntegrationJobControl id={job.id} disabled={dataLoadError||job.status==='processing'||job.status==='succeeded'}/></td></tr>)}</tbody></table></div>{!dataLoadError&&jobs.length===0&&<p className="muted" style={{padding:20}}>Még nincs integrációs feladat.</p>}</div>
