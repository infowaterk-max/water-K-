...
const commerceSettingsFailed=(operation:string,error?:{message?:string}|null)=>{console.error(`commerce settings ${operation} failed`,error?.message??'missing persisted row');throw new Error('A fizetési, szállítási vagy számlázási beállítást nem tekintjük elmentettnek.')};
...
export async function updateCommerceProviderAction(formData:FormData){
 ...
 const{data:saved,error:saveError}=await admin.from('webshop_instance_provider_connections').upsert(row,{onConflict:'instance_id,provider_code'}).select('provider_code').maybeSingle();
 if(saveError||!saved)commerceSettingsFailed('provider upsert',saveError);
...
export async function verifyCommerceProviderAction(formData:FormData){
 ...
 const{data:saved,error:saveError}=await admin.from('webshop_instance_provider_connections').update({connection_status:status,onboarding_step:step,credential_fields_present:present,last_tested_at:new Date().toISOString(),last_test_message:message,updated_at:new Date().toISOString()}).eq('instance_id',scope.instanceId).eq('provider_code',providerCode).select('provider_code').maybeSingle();
 if(saveError||!saved)commerceSettingsFailed('provider verification persistence',saveError);
 revalidatePath('/admin/beallitasok/fizetes-szallitas');revalidatePath('/penztar');
}
