...
const categoryLabels:Record<string,string>={operations:'Műveletek',inventory:'Készlet',service:'Ügyfélszolgálat',commercial:'Értékesítés',customer:'Ügyfélérték',system:'Rendszer'};
...
export default async function ControlTowerPage(){
  ...
  const[queueResult,kpiResult,healthResult,categoryResult]=await Promise.all([
  ...
  const rows=(queueResult.data??[]) as AlertRow[];
  const kpi=(kpiResult.data??{open_alerts:0,critical_alerts:0,high_alerts:0,over_24h_alerts:0,overdue_tasks:0,operations_alerts:0,inventory_alerts:0,commercial_alerts:0,service_alerts:0,commercial_value_at_risk_net_huf:0,avg_alert_age_hours:0,control_health_score:100}) as Kpi;
  const health=(healthResult.data??{open_system_alerts:0,critical_system_alerts:0,failed_or_blocked_integration_jobs:0,failed_webhooks_7d:0,last_control_cycle_at:null}) as Health;
  const cats=(categoryResult.data??[]) as Cat[];
  const loadError=Boolean(queueResult.error||kpiResult.error||healthResult.error||categoryResult.error);

  return <section className="adminMain">
    <span className="eyebrow">Pro · Üzleti irányítóközpont</span>
    <h1 className="sectionTitle">Irányítóközpont</h1>
    <p className="lead">Egyesített kockázat-, kivétel- és döntési sor a műveletek, készlet, ügyfélszolgálat, értékesítés és integrációk fölött. A rendszer javasol és priorizál; magas hatású üzleti állapotot nem módosít automatikusan.</p>
    {loadError&&<div className="errorNotice" role="alert"><strong>Az irányítóközpont adatai részben hiányosak.</strong><p>Hiányzó forrásadat mellett ne tekints egy területet automatikusan zöldnek.</p></div>}

    <div className="actions"><ControlCycleButton/></div>
    <div className="cards adminMetricCards">
      <div className="card"><span className="badge">Kontroll-egészség</span><div className="price">{kpi.control_health_score}/100</div></div>
      <div className="card"><span className="badge">Nyitott jelzés</span><div className="price">{kpi.open_alerts}</div></div>
      <div className="card"><span className="badge">Kritikus</span><div className="price">{kpi.critical_alerts}</div></div>
      <div className="card"><span className="badge">24 órán túl</span><div className="price">{kpi.over_24h_alerts}</div></div>
      <div className="card"><span className="badge">Lejárt feladat</span><div className="price">{kpi.overdue_tasks}</div></div>
      <div className="card"><span className="badge">Érték kockázatban</span><div className="price">{formatHuf(Number(kpi.commercial_value_at_risk_net_huf))}</div></div>
    </div>

    <section className="card">
      <h2>Rendszerállapot</h2>
      <p className="muted">Nyitott rendszerjelzés: {health.open_system_alerts} · kritikus: {health.critical_system_alerts} · hibás/blokkolt integrációs feladat: {health.failed_or_blocked_integration_jobs} · sikertelen webhook 7 nap: {health.failed_webhooks_7d} · utolsó kontrollciklus: {health.last_control_cycle_at?new Date(health.last_control_cycle_at).toLocaleString('hu-HU'):'még nem futott'}</p>
    </section>

    <section className="card">
      <h2>Terhelés kategóriánként</h2>
      <div className="adminTableScroll"><table className="adminTable">
        <thead><tr><th>Terület</th><th>Súlyosság</th><th>Darab</th><th>Max. prioritás</th><th>Átlagos kor</th></tr></thead>
        <tbody>{cats.map(x=><tr key={`${x.category}:${x.severity}`}>
          <td>{categoryLabels[x.category]??x.category}</td>
          <td><span className={`adminStatePill ${stateTone(x.severity)}`}>{severityLabel(x.severity)}</span></td>
          <td>{x.alert_count}</td><td>{x.max_priority}</td><td>{Number(x.avg_age_hours).toFixed(1)} óra</td>
        </tr>)}</tbody>
      </table></div>
    </section>
    ...
    <section className="card">
      ...
      <div className="adminTableScroll"><table className="adminTable">
        ...
        <tbody>{rows.map(r=><tr key={r.alert_id}>
          ...
          <td>{r.priority_score}</td><td>{Number(r.age_hours).toFixed(1)} óra</td>
          ...
          <td>{r.task_status?<><strong>{taskStatusLabel(r.task_status)}</strong>{r.task_due_at&&<div className="muted">Határidő: {new Date(r.task_due_at).toLocaleString('hu-HU')}{r.task_overdue?' · LEJÁRT':''}</div>}<TaskActions taskId={r.task_id} status={r.task_status}/></>:'—'}</td>
          <td><Link className="btn btnGhost" href={moduleRoutes[r.category]??'/admin'}>Forrásmodul</Link></td>
          <td><AlertActions alertId={r.alert_id} status={r.status}/></td>
        </tr>)}</tbody>
      </table></div>
      {!loadError&&rows.length===0&&<p className="muted">Nincs nyitott kontrolljelzés. Futtasd a kontrollciklust a forrásrendszerek aktuális állapotának kiértékeléséhez.</p>}
    </section>
  </section>;
}
