...
const MERCHANT_NAV:NavSection[]=[
  ...
  {label:'Rendszer',items:[
    {href:'/admin/pilot-acceptance',label:'Pilot acceptance',permission:'store.manage'},
...
export default async function AdminLayout({children}:{children:React.ReactNode}){
  ...
  const merchantSections=MERCHANT_NAV.map(section=>({
    label:section.label,
    items:section.items.filter(item=>(!item.feature||hasPlanFeature(effectivePlan,item.feature))&&can(item.permission)).map(({href,label})=>({href,label})),
