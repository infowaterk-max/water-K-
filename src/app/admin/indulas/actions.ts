...
export async function openWebshopAction(){
  ...
  const [products,commerce,content]=await Promise.all([getProducts(),getCommerceSettings(),getAdminContent()]);
  ...
  const {error}=await admin.from('webshop_instances').update({status:'active',updated_at:new Date().toISOString()}).eq('id',instance.id).eq('status','pilot');
