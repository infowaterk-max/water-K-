...
const validityFormatter=new Intl.DateTimeFormat('hu-HU',{dateStyle:'medium',timeStyle:'short',timeZone:'Europe/Budapest'});
const validityLabel=(value:string|null)=>value?`Lejár: ${validityFormatter.format(new Date(value))}`:'Nincs lejárat';
...
export default async function TeamPage(){
  ...
  const[{data:{user}},actorRoles]=await Promise.all([supabase.auth.getUser(),getActiveStoreRoles(scope.instanceId)]);
  const canAssignOwner=scope.isPlatform||actorRoles.includes('owner');
  ...
  const loadError=Boolean(bindingError||profileResult.error||!user);
  ...
  return <section className="adminMain">
    ...
    <p className="lead">Adj konkrét személyeknek csak annyi hozzáférést, amennyire a feladatukhoz szükségük van. A delegáció webshop-szinten izolált, lehet időkorlátos, és minden módosítás bekerül az audit naplóba.</p>
    {loadError&&<div className="errorNotice" role="alert"><strong>A csapatadatok egy része most nem tölthető be.</strong><p>Hiányos állapotban a rendszer nem jelenít meg módosító jogosultságkezelést.</p></div>}
    ...
    <section className="card teamInviteCard"><div><span className="eyebrow">Új delegáció</span><h2>Regisztrált felhasználó hozzáadása</h2><p className="muted">Biztonsági okból csak már létező Shoperation-fiók kaphat jogosultságot. A lejáró delegáció az időablak végén automatikusan inaktívvá válik.</p></div>{!loadError&&<AddTeamMemberForm canAssignOwner={canAssignOwner}/>}</section>
    ...
    <section className="teamMemberSection"><div className="adminToolbar"><div><span className="eyebrow">Aktív hozzáférések</span><h2>Csapattagok</h2></div><span className="badge">{bindings.length} fő</span></div>{!loadError&&bindings.length===0&&<div className="adminAuditNotice"><strong>Nincs külön csapattag.</strong><p>A webshophoz jelenleg nem tartozik további aktív szerepkör.</p></div>}<div className="teamMemberGrid">{!loadError&&bindings.map(binding=>{const profile=profileById.get(binding.user_id);const isSelf=binding.user_id===user?.id;const isOrganization=binding.instance_id!==scope.instanceId;const ownerProtected=binding.role_code==='owner'&&!canAssignOwner;const editable=!isSelf&&!isOrganization&&!ownerProtected;const readOnlyReason=isSelf?'A saját jogosultságodat ezen a felületen biztonsági okból nem módosíthatod.':isOrganization?'Ez szervezeti szintű jogosultság, ezért ezen a webshop-oldalon csak olvasható.':ownerProtected?'Tulajdonosi jogosultságot csak egy másik tulajdonos módosíthat.':undefined;return <TeamMemberControl key={binding.user_id} userId={binding.user_id} name={profile?.full_name||profile?.email||`${binding.user_id.slice(0,8)}…`} email={profile?.email??'E-mail nem elérhető'} role={binding.role_code as TeamRole} scopeLabel={binding.instance_id===scope.instanceId?'Webshop-szintű':'Szervezeti'} validUntil={binding.valid_until} validityLabel={validityLabel(binding.valid_until)} editable={editable} readOnlyReason={readOnlyReason} canAssignOwner={canAssignOwner}/>})}</div></section>
  </section>;
}
