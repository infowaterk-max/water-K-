...
export async function addWishlistAction(formData:FormData){
  ...
  const{error}=await supabase.from('wishlists').upsert({instance_id:resolved.instance.id,user_id:auth.user.id,variant_id:variantId},{onConflict:'user_id,variant_id',ignoreDuplicates:true});
...
export async function removeWishlistAction(formData:FormData){
  ...
  await supabase.from('wishlists').delete().eq('instance_id',resolved.instance.id).eq('user_id',auth.user.id).eq('variant_id',variantId);
  revalidatePath(productPath(slug));
}
export async function stockNotificationAction(formData:FormData){
  ...
  const{error}=await supabase.from('stock_notifications').insert({instance_id:resolved.instance.id,variant_id:variantId,user_id:auth.user?.id??null,email,status:'waiting'});if(error&&error.code!=='23505')redirect(`${productPath(slug)}?notify=error`);redirect(`${productPath(slug)}?notify=ok`);
}
export async function submitReviewAction(formData:FormData){
  ...
  const{data:purchases}=await supabase.from('order_items').select('id,orders!inner(customer_id,status,instance_id)').eq('instance_id',resolved.instance.id).eq('variant_id',variantId).eq('orders.instance_id',resolved.instance.id).eq('orders.customer_id',auth.user.id).in('orders.status',['paid','processing','shipped','completed']).limit(1);
  const{error}=await supabase.from('product_reviews').insert({instance_id:resolved.instance.id,product_id:resolved.variant.product_id,user_id:auth.user.id,rating,reviewer_name:reviewerName||'Vásárló',title:title||null,body,status:'pending',verified_purchase:Boolean(purchases?.length)});if(error)redirect(`${productPath(slug)}?review=error`);redirect(`${productPath(slug)}?review=ok`);
}
