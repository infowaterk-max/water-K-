...
export async function addWishlistAction(formData: FormData) {
  ...
  await supabase.from('wishlists').upsert(
    { user_id: auth.user.id, variant_id: variantId },
...
export async function submitReviewAction(formData: FormData) {
  ...
  const { data: purchases } = await supabase
    ...
    .eq('orders.customer_id', auth.user.id)
  ...
  const { error } = await supabase.from('product_reviews').insert({
    ...
    rating,
    reviewer_name: reviewerName || 'Vásárló',
    title: title || null,
    body,
    status: 'pending',
    verified_purchase: Boolean(purchases?.length),
  });
