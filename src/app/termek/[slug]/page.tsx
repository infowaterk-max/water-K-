...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="section productPage"><div className="shell">
    <div className="productHero">
      <section className="productStage"><div className="productPack largePack"><small>WATER-K</small><strong>{product.size}</strong><span>vízmegtartó technológia</span></div><div className="floatingFacts"><span>9% K</span><span>Akár 3 év</span><span>≤50 ciklus</span></div></section>
      <section className="productInfo"><span className="eyebrow">Water-K · {product.audience === 'professional' ? 'viszonteladói' : 'lakossági'} kiszerelés</span><h1 className="sectionTitle">{product.name}</h1><p className="lead">{product.short}</p><div className="tagRow productTags">{product.useCases.map((x)=><span key={x}>{x}</span>)}</div><div className="productPriceBlock"><div className="price">{formatHuf(product.grossPrice)}</div><p className="muted">Nettó {formatHuf(product.netPrice)} · {product.stock > 0 ? `raktáron: ${product.stock} db` : 'jelenleg nem elérhető'}</p></div><div className="actions productActions"><AddToCart id={product.id} slug={product.slug} name={product.name} price={product.grossPrice}/><Link className="btn btnGhost" href="/kosar">Kosár megnyitása</Link></div><div className="purchaseTrust"><span>✓ Adatbázisból validált ár</span><span>✓ Készletfoglalás rendeléskor</span><span>✓ Magyar checkout</span></div></section>
    </div>
