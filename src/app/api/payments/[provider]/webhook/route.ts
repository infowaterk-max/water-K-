...
export async function POST(request:Request,{params}:{params:Promise<{provider:string}>}){
  ...
  try{
    const adapter=getPaymentGatewayAdapter(catalog.adapter_key);
    const verified=await adapter.verifyCallback({rawPayload,headers:Object.fromEntries(request.headers.entries()),url:requestUrl}) as PaymentCallbackResult;
    ...
    await recordWebhookEvent({provider,externalEventId:`rejected-${hash}`,signatureValid:false,payloadHash:hash,status:'rejected',errorMessage:'A fizetési webhook hitelesítése sikertelen.'}).catch(()=>undefined);
