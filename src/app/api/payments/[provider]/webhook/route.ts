...
export async function POST(request:Request,{params}:{params:Promise<{provider:string}>}){
 ...
 try{const adapter=getPaymentGatewayAdapter(catalog.adapter_key);const verified=await adapter.verifyCallback({rawPayload,headers:Object.fromEntries(request.headers.entries())}) as {paid:boolean;providerReference:string;eventId?:string;status?:PaymentState;eventType?:string};
 const eventId=verified.eventId??createHash('sha256').update(rawPayload).digest('hex');const status=verified.status??(verified.paid?'paid':'unknown');const result=await applyVerifiedPaymentEvent({providerCode:provider,eventId,providerReference:verified.providerReference,eventType:verified.eventType??'payment_callback',status,signatureValid:true,rawPayload});return NextResponse.json({ok:true,duplicate:result.duplicate});
