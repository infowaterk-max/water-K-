...
export async function POST(request:Request,{params}:{params:Promise<{provider:string}>}){
 ...
 }catch(error){const hash=createHash('sha256').update(`${requestUrl}\n${rawPayload}`).digest('hex');await admin.from('payment_events').insert({provider_code:provider,provider_event_id:`rejected-${hash}`,event_type:'rejected_callback',payment_status:'unknown',signature_valid:false,payload_hash:hash}).catch(()=>undefined);return NextResponse.json({error:'A fizetési webhook hitelesítése sikertelen.'},{status:401})}
}
