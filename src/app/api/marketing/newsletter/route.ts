...
export async function POST(request:Request){
 ...
 const{data:latest,error:lookupError}=await admin.from('marketing_consents').select('status').eq('instance_id',instance.id).eq('email',email).eq('channel','email').order('occurred_at',{ascending:false}).limit(1).maybeSingle();
 if(lookupError)return NextResponse.json({error:'A feliratkozás állapota most nem ellenőrizhető.'},{status:500});
 if(latest?.status==='granted')return NextResponse.json({ok:true,duplicate:true,message:'Már feliratkoztál.'},{status:200});
 ...
 return NextResponse.json({ok:true,duplicate:false,message:'Sikeres feliratkozás.'},{status:201});
}
