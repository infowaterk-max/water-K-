...
export async function POST(request:Request){
 ...
 const admin=createAdminClient(); const {error}=await admin.from('marketing_consents').insert({user_id:user.id,email:user.email,channel:'email',status:body.consent?'granted':'withdrawn',source:'account_settings',policy_version:'2026-08-v1',metadata:{actor:'customer'}}); if(error)return NextResponse.json({error:'A beállítás most nem menthető.'},{status:500});
