...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const{data:ticket,error:te}=await a.from('support_tickets').select('id,status').eq('id',id).eq('instance_id',instance.id).eq('user_id',user.id).maybeSingle();
  if(te||!ticket)return NextResponse.json({error:'Az ügy nem található.'},{status:404});
  if(ticket.status==='closed')return NextResponse.json({error:'A lezárt ügyhöz nem küldhető új válasz.'},{status:409});
  const{error}=await a.from('support_ticket_messages').insert({instance_id:instance.id,ticket_id:id,author_user_id:user.id,author_role:'customer',message:parsed.data.message});
  if(error)return NextResponse.json({error:'A válasz rögzítése nem sikerült.'},{status:500});
  await a.from('support_tickets').update({status:'open',updated_at:new Date().toISOString()}).eq('id',id).eq('instance_id',instance.id).eq('user_id',user.id);
