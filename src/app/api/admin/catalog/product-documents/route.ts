...
const createSchema=z.object({
  productId:z.string().uuid(),variantId:z.string().uuid().nullable().optional(),kind:kindSchema,title:z.string().trim().min(1).max(200),description:z.string().trim().max(2000).optional(),sortOrder:z.number().int().min(0).max(10000).default(0),visibility:z.enum(['public','account']).default('account'),postPurchaseDelivery:z.boolean().default(true),fileName:z.string().trim().min(1).max(255),mediaType:z.enum(mediaTypes),sizeBytes:z.number().int().min(1).max(26214400),
});
...
export async function POST(request:Request){
  ...
  const{data,error}=await admin.rpc('admin_prepare_product_document_v2',{p_instance_id:ctx.scope.instanceId,p_actor:ctx.actor.id,p_document_id:documentId,p_product_id:parsed.data.productId,p_variant_id:parsed.data.variantId??null,p_kind:parsed.data.kind,p_title:parsed.data.title,p_description:parsed.data.description??'',p_sort_order:parsed.data.sortOrder,p_visibility:parsed.data.visibility,p_post_purchase_delivery:parsed.data.postPurchaseDelivery,p_storage_path:path,p_original_name:parsed.data.fileName,p_media_type:parsed.data.mediaType,p_size_bytes:parsed.data.sizeBytes,p_checksum_sha256:null});
