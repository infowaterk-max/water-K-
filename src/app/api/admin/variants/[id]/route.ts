...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: current, error: currentError } = await admin.from('product_variants').select('stock_quantity,gross_price_huf,net_price_huf,reseller_gross_price_huf,reseller_net_price_huf,active,sku').eq('id', id).maybeSingle();
  ...
  const { data: updated, error } = await admin.from('product_variants').update(update).eq('id', id).select('stock_quantity,gross_price_huf,net_price_huf,reseller_gross_price_huf,reseller_net_price_huf,active,sku').single();
