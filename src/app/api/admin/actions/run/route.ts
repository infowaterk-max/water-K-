...
export async function POST(){
 ...
 const{data,error}=await a.rpc('process_action_cycle_v2',{p_instance_id:store.instanceId,p_run_key:runKey});
 if(error)return NextResponse.json({error:error.message},{status:500});
 const run=(data??{})as{id?:string;instance_id?:string;run_key?:string;completed_at?:string|null};
 if(!run.id||run.instance_id!==store.instanceId||run.run_key!==runKey||!run.completed_at)return NextResponse.json({error:'Az intézkedési ciklus eredménye nem igazolható.'},{status:500});
