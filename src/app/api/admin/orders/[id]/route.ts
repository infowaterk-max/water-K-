...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if(parsed.data.status==='processing'&&current.status!=='processing'&&current.shipping_method&&current.shipping_method!=='pickup'){
    const {data:existing}=await admin.from('integration_jobs').select('id').eq('order_id',id).eq('kind','shipment_create').in('status',['pending','processing','succeeded']).limit(1);
    if(!existing?.length){
      await enqueueIntegrationJob({orderId:id,kind:'shipment_create',provider:current.shipping_method,payload:{orderNumber:current.order_number}}).catch(async integrationError=>{
        await admin.from('order_events').insert({order_id:id,event_type:'integration_enqueue_failed',from_status:parsed.data.status,to_status:parsed.data.status,actor_user_id:actor.id,metadata:{provider:current.shipping_method,error:integrationError instanceof Error?integrationError.message:'unknown'}});
      });
    }
  }
