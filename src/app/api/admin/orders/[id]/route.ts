...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(nextStatus==='paid'&&currentStatus!=='paid')await enqueueOnce({orderId:id,kind:'invoice_create',provider:process.env.INVOICE_PROVIDER||'invoicing',orderNumber:current.order_number,actorId:actor.id,status:nextStatus});
