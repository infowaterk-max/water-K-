...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const{id}=await params;if(!z.string().uuid().safeParse(id).success)return NextResponse.json({error:'Érvénytelen azonosító.'},{status:400});
  let raw:unknown;try{raw=await request.json()}catch{return NextResponse.json({error:'Érvénytelen kérés.'},{status:400})}
  const parsed=schema.safeParse(raw);if(!parsed.success)return NextResponse.json({error:'Érvénytelen módosítás.'},{status:400});
  const admin=createAdminClient(),{data:before}=await admin.from('content_pages').select('*').eq('id',id).maybeSingle();if(!before)return NextResponse.json({error:'A tartalom nem található.'},{status:404});
  ...
  const{data:after,error}=await admin.from('content_pages').update(update).eq('id',id).select('*').single();if(error)return NextResponse.json({error:'A módosítás nem sikerült.'},{status:409});
  await recordAdminAudit({actorUserId:actor.id,action:'content.updated',entityType:'content_page',entityId:id,summary:'Tartalom módosítva',beforeState:before,afterState:after});
...
export async function DELETE(_:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const{id}=await params;if(!z.string().uuid().safeParse(id).success)return NextResponse.json({error:'Érvénytelen azonosító.'},{status:400});
  const admin=createAdminClient(),{data:before}=await admin.from('content_pages').select('*').eq('id',id).maybeSingle();if(!before)return NextResponse.json({error:'A tartalom nem található.'},{status:404});
  const{error}=await admin.from('content_pages').delete().eq('id',id);if(error)return NextResponse.json({error:'A törlés nem sikerült.'},{status:500});
  await recordAdminAudit({actorUserId:actor.id,action:'content.deleted',entityType:'content_page',entityId:id,summary:'Tartalom törölve',beforeState:before});return NextResponse.json({ok:true});
}
