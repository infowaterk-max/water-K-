...
const schema=z.object({
  ...
  attachmentIds:z.array(z.string().uuid()).min(1).max(OFFICE_PRIVATE_ATTACHMENT_MAX_FILES),
...
async function completeScan(db:ReturnType<typeof createAdminClient>,input:{attachmentId:string;scanNonce:string;result:'clean'|'infected'|'rejected'|'scan_error';sha256:string|null;detectedContentType:string|null;provider:string|null;engineVersion:string|null;malwareSignature:string|null;reason:string|null}){
  ...
  if(result.attachmentId!==input.attachmentId||result.result!==input.result)throw new Error('scan_completion_evidence_invalid');
  return result;
}
...
export async function POST(request:Request){
  ...
  if(!parsed.success)return NextResponse.json({error:'Érvénytelen csatolmány-ellenőrzési kérés.'},{status:400});
  ...
  for(const attachmentId of parsed.data.attachmentIds){
    ...
    if(error){
      ...
      if(reason.includes('OFFICE_ATTACHMENT_STORAGE_OBJECT_MISSING'))return NextResponse.json({error:'A feltöltött fájl nem igazolható a karantén tárhelyen.'},{status:409});
      return NextResponse.json({error:'A fájl biztonsági ellenőrzése nem indítható el.'},{status:409});
    }
    ...
    if(downloaded.error||!downloaded.data){
      await completeScan(db,{attachmentId,scanNonce:item.scanNonce,result:'scan_error',sha256:null,detectedContentType:null,provider:null,engineVersion:null,malwareSignature:null,reason:'quarantine_download_failed'}).catch(()=>null);
      return NextResponse.json({error:'A karanténban lévő fájl nem olvasható biztonsági ellenőrzésre.'},{status:503});
    }

    const bytes=new Uint8Array(await downloaded.data.arrayBuffer());
    const expectedSize=Number(item.byteSize??0);
    if(!Number.isSafeInteger(expectedSize)||expectedSize<1||bytes.byteLength!==expectedSize){
      await completeScan(db,{attachmentId,scanNonce:item.scanNonce,result:'rejected',sha256:'0'.repeat(64),detectedContentType:'application/octet-stream',provider:'builtin-content-inspector',engineVersion:'1',malwareSignature:null,reason:'uploaded_size_mismatch'}).catch(()=>null);
      await db.storage.from(item.storageBucket).remove([item.storagePath]).catch(()=>null);
      return NextResponse.json({error:`${item.originalName}: a feltöltött fájl mérete nem egyezik az előjegyzett fájllal.`},{status:422});
    }
    ...
    if(!inspection.ok){
      ...
      if(removed.error){
        await completeScan(db,{attachmentId,scanNonce:item.scanNonce,result:'scan_error',sha256:inspection.sha256,detectedContentType:inspection.detectedContentType,provider:'builtin-content-inspector',engineVersion:'1',malwareSignature:null,reason:'quarantine_delete_failed'}).catch(()=>null);
        return NextResponse.json({error:`${item.originalName}: a veszélyes vagy álcázott fájl karanténban maradt, letöltése tiltott.`},{status:500});
      }
      await completeScan(db,{attachmentId,scanNonce:item.scanNonce,result:'rejected',sha256:inspection.sha256,detectedContentType:inspection.detectedContentType,provider:'builtin-content-inspector',engineVersion:'1',malwareSignature:null,reason:inspection.reason??'content_signature_rejected'});
    ...
    if(verdict.status==='infected'){
      ...
      if(removed.error){
        ...
        return NextResponse.json({error:`${item.originalName}: fertőzésgyanús fájl karanténban maradt; hozzáférése tiltott.`},{status:500});
      }
      await completeScan(db,{attachmentId,scanNonce:item.scanNonce,result:'infected',sha256:inspection.sha256,detectedContentType:inspection.detectedContentType,provider:verdict.provider,engineVersion:verdict.engineVersion,malwareSignature:verdict.signature,reason:'malware_detected'});
    ...
    const completed=await completeScan(db,{attachmentId,scanNonce:item.scanNonce,result:'clean',sha256:inspection.sha256,detectedContentType:inspection.detectedContentType,provider:verdict.provider,engineVersion:verdict.engineVersion,malwareSignature:null,reason:null});
    if(completed.readyForFinalize!==true)return NextResponse.json({error:'A tiszta fájl scan-bizonyítéka nem igazolható.'},{status:500});
  }
