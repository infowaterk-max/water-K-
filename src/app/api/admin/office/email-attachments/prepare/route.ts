...
const schema=z.object({draftId:z.string().uuid(),files:z.array(fileSchema).min(1).max(OFFICE_PRIVATE_ATTACHMENT_MAX_FILES)});
...
export async function POST(request:Request){
  ...
  const parsed=schema.safeParse(raw);if(!parsed.success)return NextResponse.json({error:'Érvénytelen csatolmányadat.'},{status:400});
  const db=createAdminClient();
  const{data,error}=await db.rpc('admin_prepare_office_email_attachments_v1',{p_instance_id:scope.instanceId,p_actor:actor.id,p_draft_id:parsed.data.draftId,p_files:parsed.data.files});
  if(error){
    const reason=String(error.message??'');
    if(reason.includes('OFFICE_DRAFT_NOT_FOUND')||reason.includes('OFFICE_EMAIL_ATTACHMENT_DRAFT_REQUIRED'))return NextResponse.json({error:'A piszkozat nem található vagy nem használható ehhez a csatolmányhoz.'},{status:409});
    if(reason.includes('OFFICE_ATTACHMENT_'))return NextResponse.json({error:'A csatolmány adatai nem felelnek meg a biztonsági követelményeknek.'},{status:400});
    return NextResponse.json({error:'A csatolmány-feltöltés nem készíthető elő.'},{status:500});
  }
  const result=(data??{})as{draftId?:string;attachments?:PreparedRow[]};
  if(result.draftId!==parsed.data.draftId||!Array.isArray(result.attachments)||result.attachments.length!==parsed.data.files.length)return NextResponse.json({error:'A csatolmány-előjegyzés eredménye nem igazolható.'},{status:500});
  ...
  for(const item of result.attachments){
    ...
    if(signed.error||!signed.data?.token)return NextResponse.json({error:'A biztonságos feltöltési cím nem hozható létre.'},{status:500});
