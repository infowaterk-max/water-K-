...
export async function POST(request:Request){
 ...
 try{
  if(body.kind==='payment_followup'){
   ...
   if(typeof data!=='string'||!uuid.test(data))return NextResponse.json({error:'A kommunikáció sorba állításának eredménye nem igazolható.'},{status:500});
  ...
  const message=error instanceof Error?error.message:'A kommunikáció nem tehető sorba.';
  return NextResponse.json({error:message.includes('marketing consent required')?'Nincs aktív marketing-hozzájárulás.':'A kommunikáció nem tehető sorba.'},{status:409});
 }
}
