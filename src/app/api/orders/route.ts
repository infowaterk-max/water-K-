...
const checkoutSchema = z.object({
  ...
  companyName: z.string().trim().max(200).optional(), taxNumber: z.string().trim().max(40).optional(), billingAddress: z.string().trim().min(5).max(500), shippingAddress: z.string().trim().min(5).max(500),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({error:'A rendelési szolgáltatás átmenetileg nem elérhető.'},{status:503});
  }
}
