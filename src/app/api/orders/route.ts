...
export async function POST(request:Request){
  ...
  try{
    ...
    return NextResponse.json({ok:true,replayed,orderId:order.order_id,orderNumber:order.order_number,subtotal:order.subtotal_gross_huf,discount:order.discount_gross_huf,shippingFee:order.shipping_gross_huf,total:order.total_gross_huf,couponCode:order.coupon_code,status:checkout.paymentMethod==='kh_card'?'pending_payment':'pending_transfer',next:checkout.paymentMethod==='kh_card'?'payment':'confirmation'},{status:replayed?200:201});
