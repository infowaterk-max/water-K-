...
async function businessSnapshot(admin:ReturnType<typeof createAdminClient>){
  ...
  if(orderError||productError)throw orderError??productError??new Error('CONTACT_RETEST_BUSINESS_SNAPSHOT_FAILED');
...
async function assertAcceptanceAuthority(admin:ReturnType<typeof createAdminClient>){
  ...
  if(instanceError||!instance||instance.status!=='pilot'||instance.subscription_plan!=='alap'||!instance.organization_id)throw new Error('CONTACT_RETEST_TENANT_PREFLIGHT_FAILED');
  ...
  if(!allowed)throw new Error('CONTACT_RETEST_OWNER_BINDING_REQUIRED');
}
...
export async function GET(request:Request){
  ...
  const commitProof=new URL(request.url).searchParams.get('proof')??'';
  ...
  try{
    ...
    if(before.rawPages.length!==14||before.existing.length!==14)throw new Error('CONTACT_RETEST_PAGE_CARDINALITY_INVALID');
    if(before.rawPages.some(page=>page.published_revision_id!==null))throw new Error('CONTACT_RETEST_PUBLISHED_STATE_PRESENT');
    if(before.existing.some(page=>page.draftTemplateKey!==TEMPLATE_KEY||page.draftTemplateVersion!==TEMPLATE_VERSION||page.draftRevision===null))throw new Error('CONTACT_RETEST_CURRENT_TEMPLATE_STATE_INVALID');
    ...
    if(currentDocuments.length!==14)throw new Error('CONTACT_RETEST_CURRENT_DOCUMENTS_MISSING');
    const globalStyles=getStorefrontGlobalStyleState(currentDocuments[0]!);
    if(currentDocuments.slice(1).some(document=>!storefrontGlobalStyleStatesEqual(globalStyles,getStorefrontGlobalStyleState(document))))throw new Error('CONTACT_RETEST_GLOBAL_STYLES_DRIFT');
    ...
    if(!capability||capability.plan!=='alap')throw new Error('CONTACT_RETEST_CAPABILITY_UNAVAILABLE');
    ...
    if(!plan.gate.ok||plan.mode!=='refresh'||plan.pages.length!==14||plan.templateKey!==TEMPLATE_KEY||plan.templateVersion!==TEMPLATE_VERSION)throw new Error('CONTACT_RETEST_REFRESH_PLAN_INVALID');
    const plannedKeys=plan.pages.map(page=>page.pageKey).sort();
    const persistedKeys=[...beforeKeys].sort();
    if(JSON.stringify(plannedKeys)!==JSON.stringify(persistedKeys))throw new Error('CONTACT_RETEST_PAGE_KEYS_CHANGED');
    ...
    const{data,error}=await admin.rpc('save_storefront_template_drafts_v1',{
      ...
      p_operation_key:`phase4-contact-retest-${tag}-refresh`,
    });
    if(error)throw error;
    const result=(data??{}) as Record<string,unknown>;
    if(result.mutationScope!=='storefront_page_drafts_only')throw new Error('CONTACT_RETEST_MUTATION_SCOPE_INVALID');
    ...
    if(afterKeys.length!==14||JSON.stringify(afterKeys)!==JSON.stringify(beforeKeys)||!revisionStepOk||!exactDocuments||!allV20||publishedCount!==0||!boundaryUnchanged)throw new Error('CONTACT_RETEST_POSTFLIGHT_FAILED');
    ...
    return NextResponse.json({ok:false,errorCode:'PLAYROOM_CONTACT_RETEST_REFRESH_FAILED',detail:errorMessage(error).slice(0,500)},{status:500,headers:{'Cache-Control':'no-store'}});
  }
}
