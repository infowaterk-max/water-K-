...
async function businessSnapshot(admin:ReturnType<typeof createAdminClient>){
  ...
  if(orderError||productError)throw orderError??productError??new Error('HOME_RESPONSIVE_BUSINESS_SNAPSHOT_FAILED');
...
async function assertAuthority(admin:ReturnType<typeof createAdminClient>){
  ...
  if(error||!instance||instance.status!=='pilot'||instance.subscription_plan!=='alap'||!instance.organization_id)throw new Error('HOME_RESPONSIVE_TENANT_PREFLIGHT_FAILED');
  ...
  if(!allowed)throw new Error('HOME_RESPONSIVE_OWNER_BINDING_REQUIRED');
}
...
export async function GET(request:Request){
  ...
  try{
    ...
    if(before.pages.length!==14||before.existing.length!==14)throw new Error('HOME_RESPONSIVE_PAGE_CARDINALITY_INVALID');
    if(before.pages.some(page=>page.published_revision_id!==null))throw new Error('HOME_RESPONSIVE_PUBLISHED_STATE_PRESENT');
    if(before.existing.some(page=>page.draftTemplateKey!==TEMPLATE_KEY||page.draftTemplateVersion!==TEMPLATE_VERSION||page.draftRevision===null))throw new Error('HOME_RESPONSIVE_TEMPLATE_STATE_INVALID');
    const homeBefore=before.draftByPageKey.get('home');
    if(!homeBefore)throw new Error('HOME_RESPONSIVE_CURRENT_HOME_MISSING');
    ...
    if(!capability||capability.plan!=='alap')throw new Error('HOME_RESPONSIVE_CAPABILITY_UNAVAILABLE');
    ...
    if(!plan.gate.ok||plan.mode!=='refresh'||plan.pages.length!==14)throw new Error('HOME_RESPONSIVE_REFRESH_PLAN_INVALID');
    const plannedHome=plan.pages.find(page=>page.pageKey==='home'&&page.pageType==='home');
    if(!plannedHome)throw new Error('HOME_RESPONSIVE_PLANNED_HOME_MISSING');
    ...
    const{data,error}=await admin.rpc('save_storefront_page_draft_v1',{
      ...
      p_expected_draft_revision:homeBefore.revision_number,p_operation_key:`playroom-responsive-isolation-${proof.slice(0,12)}`,
    });
    if(error)throw error;
    ...
    if(!homeAfter)throw new Error('HOME_RESPONSIVE_POST_HOME_MISSING');
    ...
    if(!ok)throw new Error('HOME_RESPONSIVE_POSTFLIGHT_FAILED');
    ...
    return NextResponse.json({ok:false,errorCode:'PLAYROOM_RESPONSIVE_HOME_REFRESH_FAILED',detail:detail.slice(0,500)},{status:500,headers:{'Cache-Control':'no-store'}});
  }
}
