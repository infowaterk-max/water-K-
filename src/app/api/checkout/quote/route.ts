...
export async function POST(request:Request){
 ...
 try{const session=await createClient(),{data:{user}}=await session.auth.getUser();const quote=await quoteTenantCheckout({instanceId:instance.id,customerId:user?.id??null,couponCode:parsed.data.couponCode.toUpperCase(),shippingKind:shipping.kind,shippingFeeHuf:shipping.fee,freeShippingThresholdHuf:commerce.freeShippingThreshold,items:parsed.data.items});return NextResponse.json({ok:true,...quote})}
