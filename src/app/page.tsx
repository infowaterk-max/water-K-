...
export default async function HomePage(){
 ...
 const categories=[...new Set(products.flatMap(product=>product.useCases||[]))].slice(0,6);
 ...
 return <main data-storefront-legacy-fallback="home">
  ...
  <section className="editorialSection"><div className="shell editorialGrid"><div className="editorialPanel editorialDark"><span className="showcaseKicker">Egyszerű vásárlás</span><h2>Kevesebb keresgélés. Több biztos döntés.</h2><p>Átlátható termékadatok, készletinformáció és egyenes út a kosártól a rendelésig.</p><Link href="/webaruhaz">Irány a webshop →</Link></div><div className="editorialPanel"><span className="showcaseKicker">{brand}</span><h2>{c.introTitle||'Egy webshop, ami minden kijelzőn otthon van.'}</h2><p>{c.introLead||'Telefonon, tableten és asztali gépen is gyors, követhető és kényelmes.'}</p><div className="editorialStats"><div><strong>24/7</strong><span>elérhető</span></div><div><strong>100%</strong><span>reszponzív</span></div></div></div></div></section>
  <section className="section showcaseFinal"><div className="shell"><div className="showcaseFinalPanel"><span className="showcaseKicker">{c.finalEyebrow||brand}</span><h2>{c.finalTitle||'Találd meg, amit keresel.'}</h2><p>Fedezd fel a teljes kínálatot, hasonlítsd össze a lehetőségeket, és rendelj néhány egyszerű lépésben.</p><Link className="btn showcaseLightBtn" href="/webaruhaz">Webáruház megnyitása</Link></div></div></section>
 </main>;
}
