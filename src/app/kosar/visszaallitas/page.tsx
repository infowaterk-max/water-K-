...
import type { CartCommerceGroupMeta,CartItem } from '@/lib/cart/types';

export const dynamic='force-dynamic';
type SavedItem={productId:string;quantity:number;lineId?:string;commerceGroup?:CartCommerceGroupMeta};
type VariantRow={id:string;product_id:string;label:string;gross_price_huf:number;active:boolean;products:{slug:string;name:string;active:boolean}|null};
...
export default async function RecoveryPage({searchParams}:{searchParams:Promise<{token?:string}>}){
  ...
  const instance=await getCurrentWebshopInstance();if(!instance)return <main className="section"><div className="shell"><div className="card"><h1>Nincs aktív webáruház.</h1></div></div></main>;
  const admin=createAdminClient();
  const {data:intent}=await admin.from('checkout_recovery_intents').select('cart,status,expires_at').eq('instance_id',instance.id).eq('recovery_token',token).maybeSingle();
  ...
  const {data:variants}=await admin.from('product_variants').select('id,product_id,label,gross_price_huf,active,products!inner(slug,name,active,instance_id)').eq('instance_id',instance.id).eq('products.instance_id',instance.id).in('id',ids).eq('active',true).eq('products.active',true);
  const rows=(variants??[]) as unknown as VariantRow[],byId=new Map(rows.map(row=>[row.id,row]));
  const items:CartItem[]=saved.flatMap(savedItem=>{const row=byId.get(savedItem.productId);if(!row?.products)return[];return[{productId:row.product_id,variantId:row.id,slug:row.products.slug,name:`${row.products.name} – ${row.label}`,unitPrice:Number(row.gross_price_huf),quantity:Math.max(1,Math.min(99,Math.floor(Number(savedItem.quantity)||1))),lineId:savedItem.lineId,commerceGroup:savedItem.commerceGroup}];});
