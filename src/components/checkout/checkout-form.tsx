...
type OrderApiResponse={error?:string;orderNumber?:string;confirmationToken?:string;status?:OrderStatus;total?:number};
...
export function CheckoutForm({khEnabled}:{khEnabled:boolean}){
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      if(!response.ok||!payload.orderNumber||!payload.status||!payload.confirmationToken){setState('error');setError(payload.error??'Nem sikerült létrehozni a rendelést.');submitting.current=false;return;}
      ...
      clear(); const query=new URLSearchParams({token:payload.confirmationToken}); router.replace(`/rendeles-sikeres?${query.toString()}`);
