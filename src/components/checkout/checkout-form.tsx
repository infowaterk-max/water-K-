...
export function CheckoutForm({shippingOptions,paymentOptions,freeShippingThreshold,resellerApproved}:{shippingOptions:ShippingOption[];paymentOptions:PaymentOption[];freeShippingThreshold:number;resellerApproved:boolean}){
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){
    ...
    if(effectiveCustomerType!=='retail'){
      ...
      if(!isValidHuTaxNumber(normalizedTax)){setState('error');setError('Az adószám formátuma vagy ellenőrzőszáma hibás. Formátum: 12345676-1-12.');return}
  ...
  return <div className="checkoutLayout"><form className="checkout-form" onSubmit={submit} aria-busy={state==='sending'||quoteLoading}>
    ...
    <fieldset className="formSection" disabled={state==='sending'}><legend>Fizetés</legend><div className="choiceCards paymentChoices">{paymentOptions.map(o=>{const meta=paymentMeta(o.flow),active=o.code===paymentCode;return <label key={o.code} className={active?'choiceCard active':'choiceCard'}><input type="radio" name="paymentProvider" value={o.code} checked={active} onChange={()=>setPaymentCode(o.code)}/><span className="choiceIcon">{meta.icon}</span><span className="choiceBody"><strong>{o.label}</strong><em>{meta.description}</em></span><span className="choiceCheck">{active?'✓':''}</span></label>})}</div><label className="checkoutField"><span>Megjegyzés a rendeléshez</span><textarea name="note" placeholder="Megjegyzés a rendeléshez" rows={4}/></label></fieldset>
    <fieldset className="formSection" disabled={state==='sending'}><legend>Kupon</legend><div className="form-grid"><label className="checkoutField"><span>Kuponkód</span><input value={couponInput} onChange={e=>setCouponInput(e.target.value.toUpperCase())} placeholder="Kuponkód"/></label><button className="btn btnGhost" type="button" onClick={applyCoupon} disabled={quoteLoading||!quoteItems.length}>{quoteLoading?'Ellenőrzés…':'Kupon alkalmazása'}</button></div>{couponMessage&&<p className="helperText" role="status">{couponMessage}</p>}</fieldset>
