...
type OrderApiResponse={error?:string;orderNumber?:string;confirmationToken?:string;status?:OrderStatus;total?:number;paymentRedirectUrl?:string};
...
type Quote={items:QuoteLine[];subtotal_gross_huf:number;discount_gross_huf:number;shipping_gross_huf:number;total_gross_huf:number;coupon_code?:string|null};
...
export function CheckoutForm({shippingOptions,paymentOptions,freeShippingThreshold,resellerApproved}:{shippingOptions:ShippingOption[];paymentOptions:PaymentOption[];freeShippingThreshold:number;resellerApproved:boolean}){
  ...
  const payment=paymentOptions.find(o=>o.code===paymentCode)??paymentOptions[0];
  ...
  async function refreshQuote(code=couponCode){
    if(!shipping||!quoteItems.length||missingVariant){setQuote(null);setQuoteError(missingVariant?'A kosár egy régi, termékváltozat nélküli tételt tartalmaz. Töröld és tedd újra kosárba a terméket.':'');return null}
    setQuoteLoading(true);setQuoteError('');
    try{
      const r=await fetch('/api/checkout/quote',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({shippingProvider:shipping.code,couponCode:code||undefined,items:quoteItems})});
  ...
  function continueFromShipping(){
    ...
    if(shipping?.kind==='parcel_point'&&!parcelPointId){setPickupInvalid(true);setState('error');setError('Válassz átvételi pontot a folytatás előtt.');return}
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){
    ...
    if(submitting.current||!shipping||!paymentOptions.length)return;
    if(shipping.kind==='parcel_point'&&!parcelPointId){setActiveStep('shipping');setPickupInvalid(true);setState('error');setError('Válassz átvételi pontot a rendelés leadása előtt.');return}
  ...
  const shippingSummary=shipping?`${shipping.label} · kapcsolattartó és cím`:'Válassz szállítási módot';
    ...
    <form ref={formRef} className="checkout-form" onSubmit={submit} aria-busy={state==='sending'||quoteLoading}>
      ...
      <div className={styles.accordion}>
        ...
        <CheckoutAccordionStep step="payment" number={3} title="Fizetés" summary={paymentSummary} active={activeStep==='payment'} completed={furthestStep>1} locked={furthestStep<1} onOpen={()=>openStep('payment')}>
          <fieldset className="formSection" disabled={state==='sending'}><legend>Fizetési mód</legend><div className="choiceCards paymentChoices" data-addon-insertion-point="checkout.payment.methods">{paymentOptions.map(o=>{const meta=paymentMeta(o.flow),active=o.code===paymentCode;return <label key={o.code} className={active?'choiceCard active':'choiceCard'}><input type="radio" name="paymentProvider" value={o.code} checked={active} onChange={()=>setPaymentCode(o.code)}/><span className="choiceIcon">{meta.icon}</span><span className="choiceBody"><strong>{o.label}</strong><em>{meta.description}</em></span><span className="choiceCheck">{active?'✓':''}</span></label>})}</div></fieldset>
        ...
        <CheckoutAccordionStep step="summary" number={4} title="Összesítés" summary={quote?`Fizetendő: ${formatHuf(total)}`:'Végső ellenőrzés'} active={activeStep==='summary'} completed={false} locked={furthestStep<2} onOpen={()=>openStep('summary')}>
          <div className={styles.summaryPreview}><span>Szállítás <strong>{shipping?.label??'—'}</strong></span><span>Fizetés <strong>{payment?.label??'—'}</strong></span><span>Végösszeg <strong>{quote?formatHuf(total):'Ellenőrzés alatt'}</strong></span></div>
          ...
          <div className={styles.stepActions}><button className="btn btnGhost" type="button" onClick={()=>setActiveStep('payment')}>Vissza</button><button className="btn btnPrimary checkoutSubmit" type="submit" disabled={state==='sending'||quoteLoading||!quote||!legalAccepted}>{state==='sending'?'Rendelés előkészítése…':quoteLoading?'Kosár ellenőrzése…':`Rendelés leadása · ${formatHuf(total)}`}</button></div>
        </CheckoutAccordionStep>
      </div>
      {(state==='error'||quoteError)&&<p className="errorNotice" role="alert">{error||quoteError}</p>}
    </form>
    <aside className="checkoutSummary card"><span className="eyebrow">Rendelésed</span><h2>Ellenőrzött összesítő</h2>{resellerApproved&&<div className="partnerSummaryBadge">B2B partnerár és rendelési szabályok</div>}{quote?.items.map(i=><div className="summaryLine" key={i.variantId}><span>{i.name}{i.variantLabel?` · ${i.variantLabel}`:''} × {i.quantity}{i.channel==='b2b'&&i.orderMultiple&&i.orderMultiple>1?` · egység: ${i.orderMultiple} db`:''}</span><strong>{formatHuf(i.lineGrossHuf)}</strong></div>)}{!quote&&<p className="muted">{quoteLoading?'A kosár ellenőrzése folyamatban…':'A kosár ellenőrzésre vár.'}</p>}<div className="summaryLine"><span>Termékek</span><strong>{formatHuf(subtotal)}</strong></div>{discount>0&&<div className="summaryLine"><span>Kedvezmény · {couponCode}</span><strong>−{formatHuf(discount)}</strong></div>}<div className="summaryLine"><span>Szállítás</span><strong>{deliveryFee===0?'Díjmentes':formatHuf(deliveryFee)}</strong></div><div className="summaryTotal"><span>Fizetendő</span><strong>{formatHuf(total)}</strong></div>{freeShippingThreshold>0&&subtotal-discount<freeShippingThreshold&&shipping?.kind!=='pickup'&&<p className="shippingProgress">Még {formatHuf(freeShippingThreshold-(subtotal-discount))} a díjmentes szállításhoz.</p>}<div className="trustList"><span>✓ Ellenőrzött ár</span><span>✓ Ellenőrzött készlet</span><span>✓ Ehhez a webshophoz tartozó kupon és szállítás</span></div></aside>
  </div>;
}
