...
export function CheckoutForm({shippingOptions,paymentOptions,freeShippingThreshold,resellerApproved}:{shippingOptions:ShippingOption[];paymentOptions:PaymentOption[];freeShippingThreshold:number;resellerApproved:boolean}){
  ...
  return <div className={`${styles.root} checkoutLayout`} data-storefront-design-inheritance="current-theme" data-checkout-ux="guided-accordion" data-fulfillment-mode={quote?.fulfillment_mode??'unknown'}>
    <form ref={formRef} className="checkout-form" onSubmit={submit} aria-busy={state==='sending'||quoteLoading}>
      <div className="checkoutHeading"><span className="eyebrow">Biztonságos rendelés</span><h1>Pénztár</h1><p className="muted">A végösszeget, készletet és teljesítési módot a rendelés előtt szerveroldalon újra ellenőrizzük. Kézbesítés → Fizetés → Összesítés; egyszerre csak az aktuális lépés van nyitva.</p></div>
