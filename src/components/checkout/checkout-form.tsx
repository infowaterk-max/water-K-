...
export function CheckoutForm({shippingOptions,paymentOptions,freeShippingThreshold,resellerApproved,embedded=false,acceptancePreview=false,instanceId,signedIn=false,accountBenefitCapabilities}:{shippingOptions:ShippingOption[];paymentOptions:PaymentOption[];freeShippingThreshold:number;resellerApproved:boolean;embedded?:boolean;acceptancePreview?:boolean;instanceId:string|null;signedIn?:boolean;accountBenefitCapabilities:CheckoutAccountBenefitFlags}){
  ...
  const[authOpen,setAuthOpen]=useState(false),[authMode,setAuthMode]=useState<AuthMode>('login'),[accountConnected,setAccountConnected]=useState(signedIn),[accountPromptDismissed,setAccountPromptDismissed]=useState(false);
