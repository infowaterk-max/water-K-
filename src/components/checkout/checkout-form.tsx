...
export function CheckoutForm(){
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    try{
      const formData=new FormData(event.currentTarget); const checkout=Object.fromEntries(formData.entries());
      checkout.sameAddress=sameAddress?'true':'false';
      const response=await fetch('/api/orders',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({checkout,items:cart.items})});
  ...
  return <div className="checkoutLayout">
    <form className="checkout-form" onSubmit={submit}>
      ...
      <fieldset className="formSection"><legend>Kapcsolattartó és vásárlói típus</legend><div className="form-grid">
        <input name="name" required placeholder="Név / kapcsolattartó" autoComplete="name"/><input name="email" required type="email" placeholder="E-mail" autoComplete="email"/><input name="phone" required placeholder="Telefonszám" autoComplete="tel"/>
        <select name="customerType" value={customerType} onChange={e=>setCustomerType(e.target.value as CustomerType)}><option value="retail">Lakossági</option><option value="company">Céges</option><option value="reseller">Viszonteladó</option></select>
        {companyRequired&&<input name="companyName" required placeholder="Cégnév" autoComplete="organization"/>}{companyRequired&&<input name="taxNumber" required placeholder="Adószám"/>}
      </div></fieldset>
      ...
      <fieldset className="formSection"><legend>Szállítás</legend>
        <div className="form-grid"><select name="shippingMethod" value={shippingMethod} onChange={e=>setShippingMethod(e.target.value as ShippingMethod)}>{shippingOptions.map(option=><option key={option.id} value={option.id}>{option.label}</option>)}</select><select name="paymentMethod" defaultValue="kh_card"><option value="kh_card">Bankkártya – K&H</option><option value="bank_transfer">Banki átutalás</option></select>
          {shippingMethod==='foxpost'&&<input name="parcelPointId" required placeholder="Foxpost automata azonosító / választás"/>}
        </div>
