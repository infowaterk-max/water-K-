...
import { resolveCheckoutAccountBenefits,type CheckoutAccountBenefitFlags } from '@/lib/account/checkout-account-benefits';
...
type CheckoutCustomerDefaults={name:string;email:string;phone:string;billingPostcode:string;billingCity:string;billingAddress:string;companyName:string;taxNumber:string;customerType:CustomerType;businessIdentityLocked:boolean};
...
export function CheckoutForm({shippingOptions,paymentOptions,freeShippingThreshold,resellerApproved,embedded=false,acceptancePreview=false,instanceId,signedIn=false,customerDefaults,hasSavedBillingProfile=false,accountBenefitCapabilities}:{shippingOptions:ShippingOption[];paymentOptions:PaymentOption[];freeShippingThreshold:number;resellerApproved:boolean;embedded?:boolean;acceptancePreview?:boolean;instanceId:string|null;signedIn?:boolean;customerDefaults:CheckoutCustomerDefaults;hasSavedBillingProfile?:boolean;accountBenefitCapabilities:CheckoutAccountBenefitFlags}){
  ...
  const accountBenefits=resolveCheckoutAccountBenefits({...accountBenefitCapabilities,digitalDownloads:accountBenefitCapabilities.digitalDownloads&&containsDigital});
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){
    ...
    if(!termsAccepted||!privacyAcknowledged){setActiveStep('summary');setState('error');setError('A rendelés leadásához külön el kell fogadnod az ÁSZF-et és tudomásul kell venned az adatkezelési tájékoztatót.');return}
    ...
    try{
      checkout.sameAddress=sameAddress?'true':'false';checkout.legalAccepted='true';checkout.couponCode=couponCode;checkout.paymentProvider=payment.code;checkout.saveBillingProfile=saveBillingProfile?'true':'false';
      ...
      <div className={styles.accordion}>
        <CheckoutAccordionStep step="shipping" number={2} title={firstStepTitle} summary={shippingSummary} active={activeStep==='shipping'} completed={furthestStep>0} locked={false} onOpen={()=>openStep('shipping')}>
          <fieldset className="formSection" disabled={state==='sending'}><legend>Kapcsolattartó és vásárlói típus</legend><div className="form-grid">
            <label className="checkoutField"><span>Név / kapcsolattartó</span><input name="name" required autoComplete="name" placeholder="Név / kapcsolattartó" defaultValue={customerDefaults.name}/></label>
            <label className="checkoutField"><span>E-mail</span><input name="email" required type="email" autoComplete="email" placeholder="E-mail" defaultValue={customerDefaults.email}/></label>
            <label className="checkoutField"><span>Telefonszám</span><input name="phone" required autoComplete="tel" placeholder="Telefonszám" defaultValue={customerDefaults.phone}/></label>
            ...
            {effectiveCustomerType!=='retail'&&<label className="checkoutField"><span>Számlázási cégnév</span><input name="companyName" required autoComplete="organization" placeholder="Cégnév" defaultValue={customerDefaults.companyName} readOnly={customerDefaults.businessIdentityLocked}/>{customerDefaults.businessIdentityLocked&&<small>Hitelesített B2B cégadat; módosítása külön jóváhagyási folyamat.</small>}</label>}
            {effectiveCustomerType!=='retail'&&<label className="checkoutField"><span>Magyar adószám</span><input name="taxNumber" required inputMode="numeric" placeholder="12345676-1-12" defaultValue={customerDefaults.taxNumber} readOnly={customerDefaults.businessIdentityLocked} onBlur={e=>{if(!customerDefaults.businessIdentityLocked)e.currentTarget.value=normalizeHuTaxNumber(e.currentTarget.value)}}/><small>{customerDefaults.businessIdentityLocked?'Hitelesített B2B adószám; itt nem írható át.':'8 számjegy + áfakód + területi kód.'}</small></label>}
          </div></fieldset>
          <fieldset className="formSection" disabled={state==='sending'}><legend>Számlázási cím</legend><div className="form-grid">
            <label className="checkoutField"><span>Számlázási irányítószám</span><input name="billingPostcode" required autoComplete="postal-code" placeholder="Irányítószám" defaultValue={customerDefaults.billingPostcode}/></label>
            <label className="checkoutField"><span>Számlázási település</span><input name="billingCity" required autoComplete="address-level2" placeholder="Település" defaultValue={customerDefaults.billingCity}/></label>
            <label className="checkoutField"><span>Számlázási utca, házszám</span><input name="billingAddress" required autoComplete="street-address" placeholder="Utca, házszám" defaultValue={customerDefaults.billingAddress}/></label>
          </div>{accountConnected&&<label className="inlineCheck"><input type="checkbox" checked={saveBillingProfile} onChange={e=>setSaveBillingProfile(e.target.checked)}/><span>{hasSavedBillingProfile?'A módosított számlázási adatokat mentse a fiókomba':'Számlázási adatok mentése a fiókomba a következő vásárláshoz'}</span></label>}</fieldset>
        ...
        <CheckoutAccordionStep step="summary" number={4} title="Összesítés" summary={quote?`Fizetendő: ${formatHuf(total)}`:'Végső ellenőrzés'} active={activeStep==='summary'} completed={false} locked={furthestStep<2} onOpen={()=>openStep('summary')}>
          ...
          {!accountConnected&&!accountPromptDismissed?<section className={styles.accountOpportunity} data-checkout-account-opportunity="true"><span className={styles.accountOpportunityKicker}>Mielőtt befejezed</span><h3>Vásárolhatsz vendégként is — vagy kösd a rendelést a fiókodhoz.</h3><p>A fiók nem kötelező a rendeléshez. Bejelentkezés vagy regisztráció után ugyanitt folytatod a pénztárat.</p>{accountBenefits.length?<ul>{accountBenefits.map(benefit=><li key={benefit.key} data-account-benefit={benefit.key}>{benefit.label}</li>)}</ul>:null}<div className={styles.accountOpportunityActions}><button className="btn btnPrimary" type="button" onClick={()=>{setAuthMode('login');setAuthOpen(true)}}>Bejelentkezés</button><button className="btn btnGhost" type="button" onClick={()=>{setAuthMode('register');setAuthOpen(true)}}>Regisztráció</button><button className={styles.guestContinue} type="button" onClick={()=>setAccountPromptDismissed(true)}>Folytatás vendégként</button></div></section>:accountConnected?<p className={styles.accountConnected} role="status">✓ Bejelentkezve — ez a rendelés a fiókodhoz kapcsolódik.</p>:null}
