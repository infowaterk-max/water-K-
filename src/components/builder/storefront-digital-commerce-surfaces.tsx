...
function FulfillmentSummaryRenderer({config,node,viewport}:StorefrontComponentRenderProps){
  ...
  return <section data-storefront-digital-commerce="fulfillment-summary" data-fulfillment-mode={mode} data-presentation={text(config.presentation)||undefined} style={{...span(node),display:'grid',gap:'.85rem',padding:'clamp(.9rem,2vw,1.25rem)',...surface(slot),...slot('root')}}>
    ...
    {(mode==='digital'||mode==='mixed')&&bool(model?.showDocumentCenter,true)?<a href={documentsHref} style={{justifySelf:'start',color:'inherit',fontWeight:750,...slot('link')}}>{text(config.documentCenterLabel,'Dokumentumok és letöltések')}</a>:null}
  </section>;
}
...
function DocumentsCenterRenderer({config,node,viewport}:StorefrontComponentRenderProps){
  ...
  const digital=rows(model?.digital),orders=rows(model?.orderDocuments);
  const renderEntries=(items:Record<string,unknown>[],kind:'digital'|'order')=>items.map((item,index)=>{const status=text(item.status,'available'),href=status==='available'?safeInternalHref(item.href):'';return <article key={text(item.id,`${kind}-${index}`)} data-document-status={status} style={{display:'grid',gap:'.3rem',padding:'.85rem 0',borderTop:'1px solid var(--shoporation-color-border,#d8dce7)',...slot('entry')}}><strong>{text(item.title,kind==='digital'?'Digitális tartalom':'Rendelési dokumentum')}</strong>{text(item.description)?<small style={{color:'var(--shoporation-color-muted-text,#667085)',...slot('description')}}>{text(item.description)}</small>:null}{status==='revoked'?<small style={{fontWeight:750}}>Hozzáférés visszavonva</small>:status==='pending'?<small style={{fontWeight:750}}>Fizetés / feldolgozás után válik elérhetővé</small>:href?<a href={href} style={{justifySelf:'start',color:'inherit',fontWeight:750,...slot('download')}}>Megnyitás / letöltés</a>:null}</article>});
  return <section data-storefront-digital-commerce="documents-center" data-presentation={text(config.presentation)||undefined} style={{...span(node),display:'grid',gap:'1rem',...slot('root')}}><header style={{display:'grid',gap:'.35rem'}}>{text(config.eyebrow)?<small style={{fontWeight:800,letterSpacing:'.09em',textTransform:'uppercase',color:'var(--shoporation-color-accent,#2f7f6f)'}}>{text(config.eyebrow)}</small>:null}<h2 style={{margin:0,fontFamily:'var(--shoporation-heading-font,Georgia,serif)'}}>{text(config.title,'Dokumentumok és letöltések')}</h2>{text(config.copy)?<p style={{margin:0,color:'var(--shoporation-color-muted-text,#667085)'}}>{text(config.copy)}</p>:null}</header>{digital.length||orders.length?<div style={{display:'grid',gridTemplateColumns:viewport==='mobile'?'1fr':'repeat(2,minmax(0,1fr))',gap:'1rem'}}><section style={{padding:'1rem',...surface(slot)}}><h3 style={{margin:'0 0 .5rem'}}>{text(config.digitalTitle,'Digitális tartalmak')}</h3>{digital.length?renderEntries(digital,'digital'):<p style={{margin:0,color:'var(--shoporation-color-muted-text,#667085)'}}>Nincs digitális tartalom.</p>}</section><section style={{padding:'1rem',...surface(slot)}}><h3 style={{margin:'0 0 .5rem'}}>{text(config.orderTitle,'Rendelési dokumentumok')}</h3>{orders.length?renderEntries(orders,'order'):<p style={{margin:0,color:'var(--shoporation-color-muted-text,#667085)'}}>Nincs rendelési dokumentum.</p>}</section></div>:<p style={{margin:0,color:'var(--shoporation-color-muted-text,#667085)'}}>{text(config.emptyLabel,'Még nincs megjeleníthető dokumentum vagy letölthető tartalom.')}</p>}</section>;
}
