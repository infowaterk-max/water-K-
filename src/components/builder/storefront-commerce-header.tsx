...
function SearchRenderer({config,node,viewport}:StorefrontComponentRenderProps){
  ...
  const rootStyle:CSSProperties={gridColumn:`span ${node.resolved.gridSpan} / span ${node.resolved.gridSpan}`,display:'flex',width:'100%',minWidth:0,alignItems:'stretch',border:'1px solid var(--shoporation-color-border,#d8dce7)',borderRadius:'var(--shoporation-radius-m,.75rem)',background:'var(--shoporation-color-surface,#fff)',overflow:'hidden',...visualStyle(config.style,viewport),...slotStyle(config.styleSlots,'root',viewport),minHeight:viewport==='mobile'?'2.75rem':'2.7rem'};
...
function CommerceHeaderRenderer({config,children,node,viewport}:StorefrontComponentRenderProps){
  ...
  const innerStyle:CSSProperties={maxWidth:'var(--shoporation-content-max, 1200px)',marginInline:'auto',padding:mobile?'.7rem 1rem':tablet?'.8rem 1.4rem':'1rem clamp(1.25rem,3vw,2.6rem)',display:'grid',gap:mobile?'.7rem':'.8rem',...visualStyle(config.innerStyle,viewport),...slotStyle(config.styleSlots,'inner',viewport),paddingBlock:mobile?'.7rem':tablet?'.75rem .58rem':'.78rem .6rem'};
  const brand=<a href={safeHref(config.brandHref,'/')} style={{display:'flex',alignItems:'center',gap:'.7rem',minWidth:0,color:'inherit',textDecoration:'none',fontWeight:850,letterSpacing:'-.02em',...visualStyle(config.brandStyle,viewport),...slotStyle(config.styleSlots,'brand',viewport),fontSize:mobile?'.98rem':'1.08rem'}}>{logo?<img src={logo} alt={text(config.logoAlt,text(config.brandLabel,'Webshop'))} loading="eager" style={{display:'block',width:mobile?'2.25rem':'2.75rem',height:mobile?'2.25rem':'2.75rem',objectFit:'contain',...visualStyle(config.logoStyle,viewport),...slotStyle(config.styleSlots,'logo',viewport),minWidth:mobile?'2.25rem':'2.75rem',minHeight:mobile?'2.25rem':'2.75rem'}}/>:null}<span style={{display:'grid',gap:'.1rem',minWidth:0}}><strong style={{whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{text(config.brandLabel,'Webshop')}</strong>{text(config.tagline)?<small style={{color:'var(--shoporation-color-muted-text,#677084)',fontWeight:600,letterSpacing:'.05em',...visualStyle(config.taglineStyle,viewport),...slotStyle(config.styleSlots,'tagline',viewport),fontSize:mobile?'.7rem':'.74rem',lineHeight:1.2}}>{text(config.tagline)}</small>:null}</span></a>;
  const utilities=<nav aria-label="Webshop műveletek" style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:mobile?'.45rem':'.7rem',...visualStyle(config.utilityStyle,viewport),...slotStyle(config.styleSlots,'utility',viewport)}}>{utility.map(item=><a key={`${item.label}:${item.href}`} href={item.href} aria-label={item.label} title={item.label} style={{position:'relative',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:showUtilityLabels&&!mobile?'.38rem':undefined,minWidth:mobile?'2rem':showUtilityLabels?'auto':'2.25rem',minHeight:mobile?'2.35rem':'2.65rem',padding:showUtilityLabels&&!mobile?'.45rem .65rem':'.45rem',border:'1px solid var(--shoporation-color-border,#d8dce7)',borderRadius:'var(--shoporation-radius-m,.75rem)',color:'inherit',textDecoration:'none',fontWeight:800,...slotStyle(config.styleSlots,'utilityItem',viewport)}}><span aria-hidden="true">{item.symbol}</span>{showUtilityLabels&&!mobile?<span style={{whiteSpace:'nowrap',...slotStyle(config.styleSlots,'utilityLabel',viewport),fontSize:'.82rem',lineHeight:1.2}}>{item.label}</span>:null}{item.count?<small style={{position:'absolute',top:'-.35rem',right:'-.35rem',minWidth:'1.1rem',height:'1.1rem',padding:'0 .2rem',display:'grid',placeItems:'center',borderRadius:'999px',background:'var(--shoporation-color-accent,#ff6b5e)',color:'var(--shoporation-color-background,#fff)',fontSize:'.62rem',fontWeight:900}}>{item.count}</small>:null}</a>)}</nav>;
  const topStyle:CSSProperties=mobile?{display:'grid',gridTemplateColumns:'minmax(0,1fr) auto',alignItems:'center',gap:'.75rem',minHeight:'3rem'}:{display:'grid',gridTemplateColumns:tablet?'minmax(9rem,.8fr) minmax(15rem,1.4fr) auto':'minmax(11rem,.75fr) minmax(20rem,1.6fr) auto',alignItems:'center',gap:'clamp(1rem,2vw,2rem)',minHeight:tablet?'3.25rem':'3.5rem'};
  const navFrameStyle:CSSProperties={display:'flex',alignItems:'center',gap:denseDesktop?'.5rem':'.75rem',minWidth:0,overflowX:'auto',overflowY:'hidden',overscrollBehaviorX:'contain',paddingTop:mobile?'.25rem':'.5rem',borderTop:'1px solid color-mix(in srgb,var(--shoporation-color-border,#d8dce7) 70%,transparent)',...slotStyle(config.styleSlots,'navigationFrame',viewport),minHeight:mobile?'2.45rem':'2.75rem'};
