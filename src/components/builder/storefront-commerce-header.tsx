...
export const STOREFRONT_COMMERCE_HEADER_RENDERERS_VERSION='shoporation.storefront-commerce-header-renderers.v1' as const;
...
function SearchRenderer({config,node,viewport}:StorefrontComponentRenderProps){
  ...
  return <form data-storefront-component="system.search" data-presentation={presentation} role="search" method="get" action={action} style={rootStyle}>
    ...
    <button type="submit" style={{border:0,borderLeft:'1px solid var(--shoporation-color-border,#d8dce7)',padding:viewport==='mobile'?'.7rem .82rem':'.75rem 1rem',background:'var(--shoporation-color-primary,#111827)',color:'var(--shoporation-color-primary-contrast,#fff)',font:'inherit',fontWeight:750,cursor:'pointer',...visualStyle(config.buttonStyle,viewport),...slotStyle(config.styleSlots,'button',viewport)}}>{text(config.buttonLabel,'Keresés')}</button>
  </form>;
}
...
function CommerceHeaderRenderer({config,children,node,viewport}:StorefrontComponentRenderProps){
  ...
  const utilities=<nav aria-label="Webshop műveletek" style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:mobile?'.45rem':'.7rem',...visualStyle(config.utilityStyle,viewport),...slotStyle(config.styleSlots,'utility',viewport)}}>{utility.map(item=><a key={`${item.label}:${item.href}`} href={item.href} aria-label={item.label} title={item.label} style={{position:'relative',display:'inline-flex',alignItems:'center',justifyContent:'center',minWidth:mobile?'2rem':'2.25rem',minHeight:mobile?'2rem':'2.25rem',padding:'.35rem',border:'1px solid var(--shoporation-color-border,#d8dce7)',borderRadius:'var(--shoporation-radius-m,.75rem)',color:'inherit',textDecoration:'none',fontWeight:800,...slotStyle(config.styleSlots,'utilityItem',viewport)}}><span aria-hidden="true">{item.symbol}</span>{item.count?<small style={{position:'absolute',top:'-.35rem',right:'-.35rem',minWidth:'1.1rem',height:'1.1rem',padding:'0 .2rem',display:'grid',placeItems:'center',borderRadius:'999px',background:'var(--shoporation-color-accent,#ff6b5e)',color:'var(--shoporation-color-background,#fff)',fontSize:'.62rem',fontWeight:900}}>{item.count}</small>:null}</a>)}</nav>;
  ...
  const navFrameStyle:CSSProperties={display:'flex',alignItems:'center',minWidth:0,overflowX:'auto',overflowY:'hidden',overscrollBehaviorX:'contain',paddingTop:mobile?'.15rem':'.25rem',borderTop:'1px solid color-mix(in srgb,var(--shoporation-color-border,#d8dce7) 70%,transparent)',...slotStyle(config.styleSlots,'navigationFrame',viewport)};
  return <header data-storefront-component="system.commerce-header" data-storefront-protected-system="header" data-presentation={text(config.presentation,'commerce-two-tier')} style={rootStyle}>
    <div style={innerStyle}>
      ...
      <div style={navFrameStyle}>{navigation}</div>
    </div>
  </header>;
}
