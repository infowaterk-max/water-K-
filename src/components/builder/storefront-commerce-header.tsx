...
function CommerceHeaderRenderer({config,children,node,viewport}:StorefrontComponentRenderProps){
  ...
  const categoryTrigger=categoryLabel?<a href={categoryHref} aria-label={categoryLabel} style={{display:'inline-flex',alignItems:'center',gap:'.4rem',flex:'0 0 auto',color:'inherit',textDecoration:'none',fontWeight:850,...slotStyle(config.styleSlots,'categoryTrigger',viewport),fontSize:mobile?'.92rem':'.98rem',lineHeight:1.25}}><span aria-hidden="true">{categorySymbol}</span><span>{categoryLabel}</span></a>:null;
