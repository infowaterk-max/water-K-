...
function CommerceHeaderRenderer({config,children,node,viewport}:StorefrontComponentRenderProps){
  ...
  const utilities=<nav aria-label="Webshop műveletek" style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:mobile?'.45rem':'.7rem',...visualStyle(config.utilityStyle,viewport),...slotStyle(config.styleSlots,'utility',viewport)}}>{utility.map(item=>{const utilityItemStyle:CSSProperties={position:'relative',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:showUtilityLabels&&!mobile?'.38rem':undefined,minWidth:mobile?'2rem':showUtilityLabels?'auto':'2.25rem',minHeight:mobile?'2.4rem':'2.75rem',padding:showUtilityLabels&&!mobile?'.45rem .65rem':'.45rem',border:'1px solid var(--shoporation-color-border,#d8dce7)',borderRadius:'var(--shoporation-radius-m,.75rem)',background:'transparent',color:'inherit',textDecoration:'none',fontWeight:800,fontFamily:'inherit',cursor:'pointer',...slotStyle(config.styleSlots,'utilityItem',viewport)};return item.href==='/fiokom'?<StorefrontAccountAuthTrigger key={`${item.label}:${item.href}`} label={item.label} symbol={item.symbol} count={item.count} showLabel={showUtilityLabels&&!mobile} style={utilityItemStyle}/>:<a key={`${item.label}:${item.href}`} href={item.href} aria-label={item.label} title={item.label} style={utilityItemStyle}><span aria-hidden="true">{item.symbol}</span>{showUtilityLabels&&!mobile?<span style={{whiteSpace:'nowrap',...slotStyle(config.styleSlots,'utilityLabel',viewport),fontSize:'.88rem',lineHeight:1.2}}>{item.label}</span>:null}{item.count?<small style={{position:'absolute',top:'-.35rem',right:'-.35rem',minWidth:'1.1rem',height:'1.1rem',padding:'0 .2rem',display:'grid',placeItems:'center',borderRadius:'999px',background:'var(--shoporation-color-accent,#ff6b5e)',color:'var(--shoporation-color-background,#fff)',fontSize:'.62rem',fontWeight:900}}>{item.count}</small>:null}</a>})}</nav>;
