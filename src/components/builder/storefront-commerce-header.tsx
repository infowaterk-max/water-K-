...
function SearchRenderer({config,node,viewport}:StorefrontComponentRenderProps){
  ...
  return <form data-storefront-component="system.search" data-presentation={presentation} role="search" method="get" action={action} style={rootStyle}>
    ...
    <button type="submit" aria-label={iconOnly?text(config.ariaLabel,'Keresés a webshopban'):undefined} style={{border:0,borderLeft:'1px solid var(--shoporation-color-border,#d8dce7)',background:'var(--shoporation-color-primary,#111827)',color:'var(--shoporation-color-primary-contrast,#fff)',font:'inherit',fontWeight:750,cursor:'pointer',...visualStyle(config.buttonStyle,viewport),...slotStyle(config.styleSlots,'button',viewport),display:'grid',placeItems:'center',alignSelf:'stretch',flex:'0 0 auto',...(iconOnly?{boxSizing:'border-box',minWidth:'3rem',width:'3rem',minHeight:'2.75rem',padding:0,lineHeight:0}:{lineHeight:1,padding:viewport==='mobile'?'.7rem .82rem':'.75rem 1rem'})}}>{iconOnly?<SearchIcon/>:buttonLabel}</button>
  </form>;
}
