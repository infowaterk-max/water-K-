...
function CommerceHeaderRenderer({config,children,node,viewport}:StorefrontComponentRenderProps){
  ...
  const innerStyle:CSSProperties={maxWidth:'var(--shoporation-content-max, 1200px)',marginInline:'auto',padding:mobile?'.7rem 1rem':tablet?'.8rem 1.4rem':'1rem clamp(1.25rem,3vw,2.6rem)',display:'grid',gap:mobile?'.7rem':'.8rem',...visualStyle(config.innerStyle,viewport),...slotStyle(config.styleSlots,'inner',viewport),paddingBlock:mobile?'.72rem':tablet?'.8rem .62rem':'.84rem .68rem'};
  ...
  const utilities=<nav aria-label="Webshop műveletek" style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:mobile?'.45rem':'.7rem',...visualStyle(config.utilityStyle,viewport),...slotStyle(config.styleSlots,'utility',viewport)}}>{utility.map(item=><a key={`${item.label}:${item.href}`} href={item.href} aria-label={item.label} title={item.label} style={{position:'relative',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:showUtilityLabels&&!mobile?'.38rem':undefined,minWidth:mobile?'2rem':showUtilityLabels?'auto':'2.25rem',minHeight:mobile?'2.4rem':'2.75rem',padding:showUtilityLabels&&!mobile?'.45rem .65rem':'.45rem',border:'1px solid var(--shoporation-color-border,#d8dce7)',borderRadius:'var(--shoporation-radius-m,.75rem)',color:'inherit',textDecoration:'none',fontWeight:800,...slotStyle(config.styleSlots,'utilityItem',viewport)}}><span aria-hidden="true">{item.symbol}</span>{showUtilityLabels&&!mobile?<span style={{whiteSpace:'nowrap',...slotStyle(config.styleSlots,'utilityLabel',viewport),fontSize:'.88rem',lineHeight:1.2}}>{item.label}</span>:null}{item.count?<small style={{position:'absolute',top:'-.35rem',right:'-.35rem',minWidth:'1.1rem',height:'1.1rem',padding:'0 .2rem',display:'grid',placeItems:'center',borderRadius:'999px',background:'var(--shoporation-color-accent,#ff6b5e)',color:'var(--shoporation-color-background,#fff)',fontSize:'.62rem',fontWeight:900}}>{item.count}</small>:null}</a>)}</nav>;
