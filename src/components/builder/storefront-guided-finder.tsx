...
export const STOREFRONT_GUIDED_FINDER_RENDERERS_VERSION='shoporation.storefront-guided-finder-renderers.v3' as const;
      ...
      <div style={{display:'grid',gap:'1rem',padding:viewport==='mobile'?'1.5rem 1rem':'clamp(1.5rem,3vw,2.75rem)',...slot('content')}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:`repeat(${columns},minmax(0,1fr))`,gap:'.65rem',...slot('options')}}>{options.map((option,i)=>{
          ...
          return <a key={text(option.id,`${i}`)} href={safeHref(option.href,'#')} aria-current={selected?'true':undefined} style={{minHeight:viewport==='mobile'?'4.6rem':'5.25rem',padding:'.75rem',display:'grid',alignContent:'center',justifyItems:'center',gap:'.4rem',textAlign:'center',border:'1px solid var(--shoporation-color-border,#ddd)',background:selected?'var(--shoporation-color-surface,#f1f4f8)':'var(--shoporation-color-surface-muted,#f4f4f4)',color:'inherit',textDecoration:'none',borderRadius:'2px',...slot('option'),...(selected?slot('optionActive'):{})}}>
            ...
            {text(option.copy)?<small style={{fontSize:'.62rem',lineHeight:1.2,color:'var(--shoporation-color-muted-text,#667)',...slot('optionCopy')}}>{text(option.copy)}</small>:null}
          </a>;
        })}</div>
...
function AttributeNavigation({config,node,viewport}:StorefrontComponentRenderProps){
  const items=rows(config.items);const presentation=text(config.presentation);const media=presentation==='media-navigation';const cards=presentation==='cards';const requested=Math.max(2,Math.min(6,Math.round(num(config.columns,4))));const columns=media?(viewport==='mobile'?1:viewport==='tablet'?Math.min(2,requested):requested):viewport==='mobile'?2:viewport==='tablet'?Math.min(3,requested):requested;const slot=styles(config,viewport);
  return <section data-storefront-guided="attribute-navigation" data-presentation={presentation||undefined} style={{...span(node),display:'grid',gap:'1rem',...slot('root')}}>{eyebrow(config.eyebrow,slot('eyebrow'))}<div style={slot('header')}><h2 style={{...(media?titleStyle:{margin:0}),...slot('title')}}>{text(config.title,'Fedezd fel')}</h2>{text(config.copy)?<p style={{margin:'.35rem 0 0',lineHeight:1.55,...slot('copy')}}>{text(config.copy)}</p>:null}</div>{media?<div style={{display:'grid',gridTemplateColumns:`repeat(${columns},minmax(0,1fr))`,gap:'.55rem',...slot('grid')}}>{items.map((x,i)=>{const src=image(x.image);return <a key={text(x.id,`${i}`)} href={safeHref(x.href,'#')} style={{display:'grid',color:'inherit',textDecoration:'none',border:'1px solid var(--shoporation-color-border,#ddd)',...slot('card')}}><div style={{aspectRatio:'2.15 / 1',background:'var(--shoporation-color-surface-muted,#eee)',overflow:'hidden',...slot('media')}}>{src?<img src={src} alt={text(x.imageAlt,text(x.label))} loading="lazy" style={{width:'100%',height:'100%',objectFit:'cover',...slot('mediaImage')}}/>:null}</div><span style={{display:'grid',gap:'.2rem',padding:'.65rem',...slot('cardBody')}}><strong style={{fontSize:'.88rem',textTransform:'uppercase',...slot('label')}}>{text(x.label)}</strong>{text(x.copy)?<small style={slot('itemCopy')}>{text(x.copy)}</small>:null}{text(x.ctaLabel)?<small style={{fontWeight:650,...slot('cta')}}>{text(x.ctaLabel)} →</small>:null}</span></a>;})}</div>:<div style={{display:cards?'grid':'flex',gridTemplateColumns:cards?`repeat(${columns},minmax(0,1fr))`:undefined,gap:'.55rem',flexWrap:cards?undefined:'wrap',...slot('grid')}}>{items.map((x,i)=><a key={text(x.id,`${i}`)} href={safeHref(x.href,'#')} style={{padding:cards?'.7rem .5rem':'.7rem .85rem',display:cards?'grid':'block',placeItems:cards?'center':undefined,gap:cards?'.18rem':undefined,textAlign:cards?'center':undefined,border:'1px solid var(--shoporation-color-border,#ddd)',borderRadius:cards?'.65rem':'999px',color:'inherit',textDecoration:'none',...slot('card')}}>{cards&&text(x.symbol)?<span aria-hidden="true" style={{fontSize:'1.25rem',lineHeight:1,...slot('symbol')}}>{text(x.symbol)}</span>:null}<strong style={{fontSize:cards?'.72rem':undefined,...slot('label')}}>{text(x.label)}</strong>{cards&&text(x.copy)?<small style={{fontSize:'.6rem',lineHeight:1.15,...slot('itemCopy')}}>{text(x.copy)}</small>:null}</a>)}</div>}</section>;
}
