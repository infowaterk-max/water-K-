...
export const STOREFRONT_BUILDER_RENDERER_REGISTRY_VERSION='shoporation.storefront-builder-renderers.special-commerce-v6' as const;
export function createStorefrontVisualBuilderRendererRegistry(){const target=new StorefrontRendererRegistry(),seen=new Set<string>();const register=(componentKey:string,componentVersion:number,renderer:Parameters<StorefrontRendererRegistry['register']>[2])=>{const key=`${componentKey}@${componentVersion}`;if(seen.has(key))return;seen.add(key);target.register(componentKey,componentVersion,renderer);};for(const source of[createStorefrontCommerceHeaderRendererRegistry(),createStorefrontInteractiveSceneRendererRegistry(),createStorefrontRecipeCommerceRendererRegistry(),createStorefrontReleaseCommerceRendererRegistry(),createStorefrontProfessionalRendererRegistry(),createStorefrontStoryVisualRendererRegistry(),createStorefrontSharedContentRendererRegistry(),createStorefrontGrowthMarketingRendererRegistry()])for(const item of source.list())register(item.componentKey,item.componentVersion,item.renderer);for(const[componentKey,componentVersion,renderer]of STOREFRONT_PURCHASE_CONTROLS_RENDERERS)register(componentKey,componentVersion,renderer);return target;}
