...
export function CartView({freeShippingThreshold}:CartViewProps){const{items,total,setQuantity,remove}=useCart();if(!items.length)return <div className="emptyCart card"><span className="eyebrow">A kosár üres</span><h2>Még nincs termék a kosaradban.</h2><p className="muted">Válassz terméket, a kosár pedig ezen az eszközön megmarad, amíg be nem fejezed a vásárlást.</p><Link className="btn btnPrimary" href="/webaruhaz">Irány a webáruház</Link></div>;
 ...
 {items.map(item=>{const minimum=item.minimumQuantity??1,multiple=item.orderMultiple??1;return <div className="cartRow" key={cartLineKey(item)}><div className="cartProductIdentity"><div className="cartThumb">{item.name.trim().slice(0,2).toUpperCase()||'•'}</div><div><strong>{item.name}</strong><p>{formatHuf(item.unitPrice)} / db{minimum>1?` · minimum ${minimum} db`:''}</p></div></div><label className="quantityControl"><span className="srOnly">Mennyiség</span><input aria-label="Mennyiség" type="number" min={minimum} step={multiple} value={item.quantity} onChange={event=>setQuantity(item.productId,Number(event.target.value),item.variantId)}/></label><div className="cartLineTotal"><strong>{formatHuf(item.unitPrice*item.quantity)}</strong><button onClick={()=>remove(item.productId,item.variantId)}>Törlés</button></div></div>})}
