...
type CartContextValue={
 cart:Cart;items:CartItem[];total:number;
...
const CartContext=createContext<CartContextValue|null>(null),CART_STORAGE_KEY='shoperation-cart-v3',LEGACY_CART_STORAGE_KEYS=['shoperation-cart-v2','waterk-cart'] as const;
...
export function CartProvider({children}:{children:React.ReactNode}){const[cart,setCart]=useState<Cart>({items:[]}),[hydrated,setHydrated]=useState(false);
 ...
 const api=useMemo<CartContextValue>(()=>({cart,items:cart.items,total:cartTotal(cart),add(item){setCart(current=>({items:mergeCartItem(current.items,item)}))},addMany(items){setCart(current=>({items:items.filter(isCartItem).reduce((next,item)=>mergeCartItem(next,item),current.items)}))},remove(productId,variantId=null,lineId){const key=lineId?`line:${lineId}`:cartLineKey({productId,variantId});setCart(current=>({items:current.items.filter(item=>cartLineKey(item)!==key)}))},setQuantity(productId,quantity,variantId=null,lineId){const key=lineId?`line:${lineId}`:cartLineKey({productId,variantId});setCart(current=>({items:current.items.map(item=>cartLineKey(item)===key?{...item,quantity:normalizeQuantity(quantity,undefined,item.minimumQuantity??1,item.orderMultiple??1)}:item)}))},replace(items){setCart({items:items.filter(isCartItem).map(normalizeCartItem)})},clear(){setCart({items:[]})},removeCommerceGroup(type,id){setCart(current=>({items:current.items.filter(item=>!groupMatch(item,type,id))}))},replaceCommerceGroup(type,id,items){setCart(current=>({items:items.filter(isCartItem).reduce((next,item)=>mergeCartItem(next,item),current.items.filter(item=>!groupMatch(item,type,id))) }))}}),[cart]);
