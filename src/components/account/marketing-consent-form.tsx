...
export function MarketingConsentForm({initialConsent}:{initialConsent:boolean}){
 ...
 return <div className="marketingConsentPanel"><p className="marketingConsentLead">E-mailben küldött ajánlatok és webáruházi újdonságok. A rendelési és fiókbiztonsági üzenetek ettől függetlenek.</p><label className="consentRow marketingConsentRow"><input type="checkbox" checked={consent} disabled={saving} onChange={event=>change(event.target.checked)}/><span>{consent?'Marketing e-maileket kérek':'Marketing e-maileket nem kérek'}</span></label>{message&&<p className="muted marketingConsentStatus" role="status">{message}</p>}<p className="marketingConsentMeta">A hozzájárulás bármikor visszavonható. A változtatás időpontját és a szabályzat verzióját auditálhatóan rögzítjük.</p></div>;
}
