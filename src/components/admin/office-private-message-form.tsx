...
function attachmentHelp(checked:boolean,availability:AttachmentAvailability){if(!checked)return'A csatolmány biztonsági rendszerének állapotát ellenőrizzük…';if(availability.enabled)return'Legfeljebb 5 fájl, fájlonként 10 MB. Kép, PDF, TXT, CSV, DOCX vagy XLSX. A fájl először privát karanténba kerül, és csak fájlszignatúra- + vírusellenőrzés után csatolható az üzenethez.';if(availability.reason==='pro_required')return'A biztonságos fájlcsatolmányok a Pro csomag Team Chat 2.1 funkciói. A Team Chat szöveges használata az Alap csomagban is elérhető.';if(availability.reason==='scanner_unavailable')return'A Pro csatolmányküldés a biztonsági scanner jóváhagyásáig és konfigurálásáig le van tiltva. Szöveges belső üzenetet továbbra is küldhetsz.';return'A csatolmányküldés jelenleg nem érhető el. Szöveges belső üzenetet továbbra is küldhetsz.'}
...
export function OfficePrivateMessageForm({threadId,mentionOptions,objectOptions,compact=false}:Props){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      form.reset();setToolsOpen(false);setMessage(files.length?'Az üzenet és a tisztának minősített csatolmányok elküldve.':'Az üzenet elküldve.');router.refresh();
  ...
  return <form className={`stackForm${compact?' teamChatCompactComposer':''}`} onSubmit={submit}>
    <textarea name="body" required rows={compact?2:3} maxLength={10000} placeholder={compact?'Írj üzenetet…':'Privát belső üzenet'}/>
    <div className="teamChatComposerBar">
      {compact&&<button type="button" className="teamChatToolsButton" aria-expanded={toolsOpen} aria-label="További üzenetbeállítások" onClick={()=>setToolsOpen(value=>!value)}>＋</button>}
      <button className="btn btnPrimary" disabled={busy}>{busy?phaseLabel(phase):'Küldés'}</button>
    </div>
    <div className="teamChatComposerTools" data-open={!compact||toolsOpen?'true':'false'}>
