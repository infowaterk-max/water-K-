  ...
  return <div className={`${styles.builderShell} ${ui.builderShellV2} ${finalUi.finalShell}`}>
    ...
    <div className={`${ui.workspaceV2} ${finalUi.workspace}`} data-left-collapsed={leftCollapsed} data-inspector-open={inspectorOpen}>
      ...
      <main className={`${styles.canvasStage} ${ui.canvasStageV2} ${finalUi.canvasStage}`} onClick={()=>{setSelectedId(null);setInspectorOpen(false);}}>
        <div className={`${ui.breakpointContext} ${finalUi.breakpointContext}`} data-override={Boolean(selected&&responsiveOverrideActive)}><span><VisualBuilderIcon name={viewport==='desktop'?'desktop':viewport==='tablet'?'tablet':'mobile'}/> {viewport==='desktop'?'Desktop':viewport==='tablet'?'Tablet':'Mobil'} · {viewportWidth}px</span><b>{selected?(responsiveOverrideActive?'Egyedi override':'Alapérték'):'Élő nézet'}</b></div>
      ...
      <aside className={`${ui.inspector} ${finalUi.inspector}`} data-open={inspectorOpen}>
        {selected&&definition?<div className={styles.selectionEditor}>
          ...
          <div className={`${ui.inspectorBody} ${finalUi.inspectorBody}`}>
            ...
            {editorTab==='responsive'?<div className={styles.editorFields}><div className={`${ui.responsiveContext} ${finalUi.responsiveContext}`} data-override={responsiveOverrideActive}><div><span>{viewport==='desktop'?'Desktop':viewport==='tablet'?'Tablet':'Mobil'}</span><strong>{responsiveOverrideActive?'Egyedi override':'Alapérték'}</strong></div><small>{'Alap + '+(viewport==='desktop'?'Desktop':viewport==='tablet'?'Tablet':'Mobil')}</small></div><div className={styles.fieldGroup}><strong>{viewport==='desktop'?'Desktop':viewport==='tablet'?'Tablet':'Mobil'} beállítások</strong><label className={styles.switchField}><span>Elrejtés ezen a nézeten</span><input type="checkbox" checked={selected.responsive?.[viewport]?.hidden??false} onChange={event=>mutate({type:'responsive',nodeId:selected.id,viewport,hidden:event.target.checked})}/><i aria-hidden="true"/></label>{definition.manifest.responsiveMode!=='fixed'?<label className={styles.field}><span>Rácsszélesség</span><select value={selected.responsive?.[viewport]?.gridSpan??''} onChange={event=>mutate({type:'responsive',nodeId:selected.id,viewport,gridSpan:event.target.value?Number(event.target.value):null})}><option value="">Alapértelmezett / 12</option>{Array.from({length:12},(_,index)=>index+1).map(value=><option key={value} value={value}>{value} / 12</option>)}</select></label>:null}{responsiveOverrideActive?<button type="button" className={ui.resetInherited} onClick={resetResponsive}><VisualBuilderIcon name="reset"/> Viewport override törlése</button>:null}</div><p className={styles.emptyHint}>A Desktop, Tablet és Mobil külön viewport-authority ugyanazon Page Schema dokumentumban. Egy nézet módosítása nem írja át a másik kettőt. A mélyebb elrendezési kontroll a Globális elemek alatt, Haladó/Expert módban érhető el.</p></div>:null}
