...
import type {StorefrontComponentNode,StorefrontPageDocument,StorefrontRuntimeCapabilityContext,StorefrontResolvedComponentNode} from '@/lib/builder/storefront-runtime';
...
const VIEWPORTS:readonly {key:StorefrontViewport;label:string;width:number}[]=[{key:'desktop',label:'Desktop',width:1200},{key:'tablet',label:'Tablet',width:768},{key:'mobile',label:'Mobile',width:390}];
const TOKEN_OPTIONS:Record<string,readonly string[]>={
  gap:['none','xs','s','m','l','xl','2xl'],spacing:['none','xs','s','m','l','xl','2xl'],
  align:['start','center','end','stretch','left','right'],justify:['start','center','end','between'],
  width:['full','content','narrow'],tone:['background','surface','muted','primary','text'],
  direction:['vertical','horizontal'],fit:['cover','contain'],loading:['lazy','eager'],
  radius:['none','s','m','l','pill'],variant:['primary','secondary','ghost'],size:['s','m','l'],
};
...
function StructuredEditor({label,value,onChange,path=[]}:{label:string;value:unknown;onChange:(value:unknown)=>void;path?:(string|number)[]}){
  if(value===null||value===undefined||typeof value!=='object')return <ScalarEditor label={label} value={value??''} onChange={onChange}/>;
  if(Array.isArray(value))return <div className={styles.complexField}><strong>{label}</strong>{value.length===0?<span className={styles.muted}>Üres lista – adatbinding vagy komponens-logika töltheti.</span>:value.map((item,index)=><StructuredEditor key={index} label={`${index+1}. elem`} value={item} path={[...path,index]} onChange={next=>onChange(setNested(value,[index],next))}/>)}</div>;
  return <div className={styles.complexField}><strong>{label}</strong>{Object.entries(value as Record<string,unknown>).map(([key,child])=><StructuredEditor key={key} label={key} value={child} path={[...path,key]} onChange={next=>onChange(setNested(value,[key],next))}/>)}</div>;
}
...
function Outline({document,selectedId,onSelect,onMove}:{document:StorefrontPageDocument;selectedId:string|null;onSelect:(id:string)=>void;onMove:(nodeId:string,parentId:string|null,index:number)=>void}){
  const items=flatten(document);const[dragging,setDragging]=useState<string|null>(null);
  ...
  return <div className={styles.outline}>{items.map(entry=><div key={entry.node.id} className={`${styles.outlineRow} ${selectedId===entry.node.id?styles.selected:''}`} style={{paddingLeft:10+entry.depth*16}} draggable onDragStart={()=>setDragging(entry.node.id)} onDragEnd={()=>setDragging(null)} onDragOver={event=>event.preventDefault()} onDrop={event=>drop(event,entry)}>
...
export function StorefrontVisualBuilder({pages,document:initialDocument,pageId,draftRevision:initialDraftRevision,publishedRevision:initialPublishedRevision,revisions,capability,bindingContext,templates}:Props){
  ...
  const[draftRevision,setDraftRevision]=useState(initialDraftRevision);const[publishedRevision,setPublishedRevision]=useState(initialPublishedRevision);
  const[dirty,setDirty]=useState(false);const[notice,setNotice]=useState<string|null>(null);const[error,setError]=useState<string|null>(null);
  const document=history?.present??null;const selected=document?getNode(document,selectedId):null;const definition=selected?componentRegistry.get(selected.componentKey,selected.componentVersion):undefined;
  const viewportWidth=VIEWPORTS.find(item=>item.key===viewport)?.width??1200;
  const entitled=(template:TemplateEntry)=>template.minPlan==='alap'||capability.plan==='pro';
  const mutate=(mutation:StorefrontBuilderMutation)=>{if(!document)return;try{const next=applyStorefrontBuilderMutation({document,mutation,registry:componentRegistry,capability});setHistory(current=>current?pushStorefrontBuilderHistory(current,next):current);setDirty(true);setError(null)}catch(reason){setError(reason instanceof Error?reason.message:'A módosítás nem hajtható végre.')}};
  ...
  const save=()=>{if(!document)return;run(async()=>{const result=await saveVisualBuilderDraftAction({document,expectedDraftRevision:draftRevision,operationKey:operationKey('save')});setDraftRevision(result.revisionNumber);setDirty(false);setNotice(`Draft mentve · r${result.revisionNumber}${result.replayed?' · replay':''}`);router.refresh()})};
  const preview=()=>{if(!pageId||!draftRevision)return;run(async()=>{if(dirty)throw new Error('PREVIEW_REQUIRES_SAVED_DRAFT');const result=await createVisualBuilderPreviewAction({pageId,revisionNumber:draftRevision,operationKey:operationKey('preview')});window.open(`${result.href}?viewport=${viewport}`,'_blank','noopener,noreferrer');setNotice(`Preview létrehozva · r${result.revisionNumber}`)})};
  const publish=()=>{if(!pageId||!draftRevision)return;run(async()=>{if(dirty)throw new Error('PUBLISH_REQUIRES_SAVED_DRAFT');const result=await publishVisualBuilderPageAction({pageId,expectedDraftRevision:draftRevision,operationKey:operationKey('publish')});setPublishedRevision(result.revisionNumber);setNotice(`Publikálva · r${result.revisionNumber}`);router.refresh()})};
  const rollback=(target:number)=>{if(!pageId||!publishedRevision)return;run(async()=>{const result=await rollbackVisualBuilderPageAction({pageId,targetPublishedRevision:target,expectedCurrentPublishedRevision:publishedRevision,operationKey:operationKey('rollback')});setPublishedRevision(result.revisionNumber);setNotice(`Rollback elkészült új publikált revisionként · r${result.revisionNumber}`);router.refresh()})};
  ...
  if(!document)return <section className={styles.emptyState}><div><span className="eyebrow">Visual Builder</span><h1 className="sectionTitle">Indulj egy valódi sablonból</h1><p className="lead">A kiválasztott sablon kizárólag draft Page Schema oldalakat hoz létre. Nem publikál automatikusan és nem módosít üzleti adatot.</p></div><div className={styles.templateGrid}>{templates.map(template=><article className={styles.templateCard} key={`${template.templateKey}@${template.templateVersion}`}><span>{template.category}</span><h2>{template.templateKey}</h2><p>{template.pageTypes.length} oldal preset · {template.minPlan==='pro'?'Pro':'Alap'}</p><button type="button" disabled={busy||!entitled(template)} onClick={()=>install(template)}>{entitled(template)?'Sablon használata':'Pro szükséges'}</button></article>)}</div>{error&&<div className="errorNotice" role="alert">{error}</div>}{notice&&<div className={styles.notice}>{notice}</div>}</section>;
  ...
  return <div className={styles.builderShell}>
    <header className={styles.toolbar}><div><span className="eyebrow">Roadmap Block 22 · Visual Builder</span><h1>{document.pageKey}</h1><p>{document.templateKey} v{document.templateVersion} · draft r{draftRevision??'—'} · published r{publishedRevision??'—'}</p></div><div className={styles.toolbarActions}><button type="button" disabled={busy||!history?.past.length} onClick={()=>setHistory(current=>current?undoStorefrontBuilderHistory(current):current)}>↶ Undo</button><button type="button" disabled={busy||!history?.future.length} onClick={()=>setHistory(current=>current?redoStorefrontBuilderHistory(current):current)}>↷ Redo</button><button type="button" disabled={busy||!dirty} onClick={save}>Draft mentése</button><button type="button" disabled={busy||dirty||!draftRevision} onClick={preview}>Preview</button><button type="button" className={styles.publishButton} disabled={busy||dirty||!draftRevision} onClick={publish}>Publikálás</button></div></header>
    ...
    {error&&<div className="errorNotice" role="alert">{error}</div>}{notice&&<div className={styles.notice}>{notice}</div>}
    <div className={styles.workspace}>
      <aside className={styles.leftPanel}><div className={styles.panelTitle}><strong>Oldalstruktúra</strong><span>Page → section → component</span></div><Outline document={document} selectedId={selectedId} onSelect={setSelectedId} onMove={move}/><div className={styles.addBox}><strong>Hozzáadás</strong>{selected&&insertableSelected.length>0?<select defaultValue="" onChange={event=>{if(!event.target.value)return;const[key,version]=event.target.value.split('@');mutate({type:'add',parentId:selected.id,componentKey:key,componentVersion:Number(version)});event.currentTarget.value=''}}><option value="">A kijelölt elembe…</option>{insertableSelected.map(item=><option key={`${item.componentKey}@${item.componentVersion}`} value={`${item.componentKey}@${item.componentVersion}`}>{item.componentKey}</option>)}</select>:null}<select defaultValue="" onChange={event=>{if(!event.target.value)return;const[key,version]=event.target.value.split('@');mutate({type:'add',parentId:null,componentKey:key,componentVersion:Number(version)});event.currentTarget.value=''}}><option value="">Új section…</option>{insertableRoot.map(item=><option key={`${item.componentKey}@${item.componentVersion}`} value={`${item.componentKey}@${item.componentVersion}`}>{item.componentKey}</option>)}</select></div></aside>
      <main className={styles.canvasStage} onClick={()=>setSelectedId(null)}><div className={styles.canvasChrome}><span>{viewport}</span><span>{viewportWidth}px · élő runtime</span></div><div className={styles.canvasViewport} style={{width:`min(100%, ${viewportWidth}px)`}}><StorefrontRuntimeRenderer page={document} viewport={viewport} bindingContext={bindingContext} componentRegistry={componentRegistry} rendererRegistry={rendererRegistry} capability={capability} decorateNode={canvasDecorator}/></div></main>
      <aside className={styles.inspector}>{selected&&definition?<><div className={styles.panelTitle}><strong>Inspector</strong><span>{selected.componentKey}</span></div><div className={styles.nodeMeta}><code>{selected.id}</code><span>{definition.manifest.responsiveMode}</span><span>{definition.manifest.capability.minPlan}</span></div><section><h3>Tulajdonságok</h3>{definition.manifest.configurable.map(key=><StructuredEditor key={key} label={key} value={selected.config[key]} onChange={value=>setConfig(key,value)}/>)}</section><section><h3>{viewport} beállítások</h3><label className={styles.field}><span>Elrejtés ezen a breakpointon</span><input type="checkbox" checked={selected.responsive?.[viewport]?.hidden??false} onChange={event=>mutate({type:'responsive',nodeId:selected.id,viewport,hidden:event.target.checked})}/></label>{definition.manifest.responsiveMode!=='fixed'&&<label className={styles.field}><span>Grid span</span><select value={selected.responsive?.[viewport]?.gridSpan??''} onChange={event=>mutate({type:'responsive',nodeId:selected.id,viewport,gridSpan:event.target.value?Number(event.target.value):null})}><option value="">Örökölt / 12</option>{Array.from({length:12},(_,index)=>index+1).map(value=><option key={value} value={value}>{value} / 12</option>)}</select></label>}</section><section className={styles.nodeActions}><button type="button" disabled={definition.protectedSystem} onClick={()=>mutate({type:'duplicate',nodeId:selected.id})}>Duplikálás</button><button type="button" disabled={definition.protectedSystem} onClick={()=>{mutate({type:'remove',nodeId:selected.id});setSelectedId(null)}}>Eltávolítás</button></section></>:<div className={styles.inspectorEmpty}>Válassz ki egy sectiont vagy komponenst a vásznon vagy az oldalstruktúrában.</div>}
      <section className={styles.revisions}><div className={styles.panelTitle}><strong>Revision history</strong><span>immutábilis előzmények</span></div>{revisions.filter(item=>item.kind==='published').map(item=><div key={item.revisionId}><span>r{item.revisionNumber}</span><time>{new Date(item.createdAt).toLocaleString('hu-HU')}</time><button type="button" disabled={busy||item.revisionNumber===publishedRevision} onClick={()=>rollback(item.revisionNumber)}>{item.revisionNumber===publishedRevision?'Aktuális':'Rollback'}</button></div>)}</section>
    </div>
  </div>;
}
