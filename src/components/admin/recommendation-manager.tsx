...
export function RecommendationManager({products,rules}:{products:Product[];rules:RecommendationRule[]}){
  const router=useRouter(),[source,setSource]=useState('all'),[target,setTarget]=useState(products[0]?.id??''),[priority,setPriority]=useState(100),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
  ...
  async function create(){
    ...
    try{
      const r=await fetch('/api/admin/recommendations',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sourceVariantId:source==='all'?null:source,recommendedVariantId:target,placement:'cart',priority})});
