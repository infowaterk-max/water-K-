...
export function EmailBuilderEditor({template,initialDocument,previewContext}:{template:TemplateMeta;initialDocument:EmailDocument;previewContext:EmailRenderContext}){
  ...
  function setConditionRule(index:number,patch:Partial<EmailConditionRule>){if(!selected?.conditions)return;const rules=selected.conditions.rules.map((rule,i)=>i===index?{...rule,...patch}:rule);updateSelected({conditions:{...selected.conditions,rules}});}
