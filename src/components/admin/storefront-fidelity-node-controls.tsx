...
export function StorefrontFidelityNodeControls({document,node,viewport,configurable,onApply}:{
  ...
  return <>
    {childOrder.length>1?<div className={styles.fieldGroup}>
      ...
      <button type="button" className={styles.addSectionButton} onClick={()=>onApply(clearStorefrontResponsiveOrder(document,{parentId:node.id,viewport}),`${viewportLabel(viewport)} belső sorrend alapértékre állítva.`)}>Örökölt belső sorrend visszaállítása</button>
    </div>:null}
    ...
    {supportsLayerGeometry?<div className={styles.fieldGroup}>
      ...
      <label className={styles.field}><span>Rögzítési pont</span><select value={layerGeometry?.anchor??''} onChange={event=>{
        ...
        if(!anchor){onApply(setStorefrontLayerViewportGeometry(document,node.id,viewport,null),`${viewportLabel(viewport)} rétegpozíció alapértékre állítva.`);return;}
        applyLayerGeometry({anchor});
      }}><option value="">Alap</option>{STOREFRONT_LAYER_ANCHORS.map(anchor=><option key={anchor} value={anchor}>{LAYER_ANCHOR_LABELS[anchor]}</option>)}</select></label>
      ...
      {layerGeometry?<button type="button" className={styles.addSectionButton} onClick={()=>onApply(setStorefrontLayerViewportGeometry(document,node.id,viewport,null),`${viewportLabel(viewport)} rétegpozíció alapértékre állítva.`)}>Rétegpozíció visszaállítása</button>:null}
    </div>:null}

    {supportsTypography?<div className={styles.fieldGroup}>
      <strong>{viewportLabel(viewport)} tipográfia</strong>
      <p className={styles.emptyHint}>Csak ezt a nézetet módosítja; az üres mező a globális alapértéket használja, másik viewport értékét nem.</p>
      <label className={styles.field}><span>Betűcsalád</span><select value={typography.fontToken??''} onChange={event=>applyTypography({fontToken:event.target.value?event.target.value as StorefrontTypographyValue['fontToken']:undefined})}><option value="">Alap</option>{STOREFRONT_TYPOGRAPHY_FONT_TOKENS.map(token=><option key={token} value={token}>{token}</option>)}</select></label>
      ...
      <label className={styles.field}><span>Vastagság</span><select value={typography.fontWeight??''} onChange={event=>applyTypography({fontWeight:numberOrUndefined(event.target.value)})}><option value="">Alap</option>{[300,400,500,600,650,700,750,800,900].map(weight=><option key={weight} value={weight}>{weight}</option>)}</select></label>
    ...
    {supportsArtDirection?<div className={styles.fieldGroup}>
      ...
      <label className={styles.field}><span>Kitöltés</span><select value={source.objectFit??''} onChange={event=>applyArt({objectFit:event.target.value?event.target.value as 'cover'|'contain':undefined})}><option value="">Alap</option><option value="cover">Cover</option><option value="contain">Contain</option></select></label>
