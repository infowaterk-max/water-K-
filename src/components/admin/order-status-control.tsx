...
export function OrderStatusControl({id,status,trackingNumber='',shippingMethod,fulfillmentMode}:{id:string;status:string;trackingNumber?:string|null;shippingMethod?:string|null;fulfillmentMode?:FulfillmentMode|null}){
  const router=useRouter(),effectiveFulfillment=fulfillmentMode??(shippingMethod==='digital_delivery'?'digital':'physical');
  const allowed=adminOrderNextStatuses(status).filter(next=>status!=='processing'||(effectiveFulfillment==='digital'?next==='completed':next==='shipped'));
  ...
  async function persist(){
    ...
    if(value==='shipped'&&effectiveFulfillment!=='digital'&&shippingMethod!=='pickup'&&!tracking.trim()){setError('Feladáshoz add meg a csomagkövetési azonosítót.');return}
