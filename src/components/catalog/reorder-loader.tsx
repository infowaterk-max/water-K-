...
export function ReorderLoader({products}:{products:ReorderProduct[]}){
  ...
  useEffect(()=>{
    ...
    for(const entry of requested){
      ...
      add({productId:product.id,variantId:product.id,slug:product.slug,name:product.name,unitPrice:product.grossPrice,quantity:Math.min(quantity,product.stock)});
    }
