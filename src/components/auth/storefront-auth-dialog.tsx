...
const STOREFRONT_AUTH_THEME_VARS=[
...
function syncStorefrontAuthTheme(dialog:HTMLDialogElement){
 const source=dialog.parentElement??document.documentElement;
 const computed=getComputedStyle(source);
 for(const variable of STOREFRONT_AUTH_THEME_VARS){
  const value=computed.getPropertyValue(variable).trim();
  if(value)dialog.style.setProperty(variable,value);
 }
}
...
export function StorefrontAuthDialog({open,onClose,instanceId=null,initialMode='login',onAuthenticated,returnTo,title='Belépés vagy regisztráció'}:{open:boolean;onClose:()=>void;instanceId?:string|null;initialMode?:AuthMode;onAuthenticated?:()=>void;returnTo?:string|null;title?:string}){
 ...
 useEffect(()=>{const dialog=dialogRef.current;if(!dialog)return;if(open&&!dialog.open){restoreFocusRef.current=document.activeElement instanceof HTMLElement?document.activeElement:null;syncStorefrontAuthTheme(dialog);dialog.showModal();queueMicrotask(()=>closeRef.current?.focus())}if(!open&&dialog.open)dialog.close()},[open]);
