...
export async function sendExternalLogisticsOrderEmail(instanceId:string,orderId:string,recipient:string){
  ...
  const[{data:order,error:orderError},{data:items,error:itemError},identity]=await Promise.all([
    admin.from('orders').select('order_number,status,fulfillment_mode,customer_email,customer_phone,billing_name,billing_company,billing_tax_number,shipping_name,shipping_postcode,shipping_city,shipping_address,shipping_method,parcel_point_id,payment_method,subtotal_gross_huf,shipping_gross_huf,discount_gross_huf,total_gross_huf,note,created_at').eq('id',orderId).eq('instance_id',instanceId).maybeSingle(),
    admin.from('order_items').select('product_name,variant_label,sku,quantity,unit_gross_huf,line_total_gross_huf,fulfillment_type').eq('order_id',orderId).eq('instance_id',instanceId).eq('fulfillment_type','physical').order('id',{ascending:true}),
    getCommunicationIdentityForInstance(instanceId),
  ]);
  if(orderError||!order)throw new Error('External logistics order not found');
  if(order.fulfillment_mode==='digital'||order.shipping_method==='digital_delivery')throw new Error('Digital order must not be sent to external logistics');
  if(itemError||!items?.length)throw new Error('External logistics physical order items unavailable');
  ...
  await admin.from('order_events').insert({instance_id:instanceId,order_id:orderId,event_type:'logistics_order_email_sent',metadata:{provider:'external_logistics_email',recipient:normalized,message_id:payload.id,shipping_method:order.shipping_method,fulfillment_mode:order.fulfillment_mode,physical_line_count:items.length,physical_items_gross_huf:physicalItemsGross}});
  return{provider:'resend',messageId:payload.id,recipient:normalized,physicalLineCount:items.length};
}
