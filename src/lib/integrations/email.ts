...
export async function sendTransactionalEmail(input:EmailInput){
  ...
  const identity=input.identity,siteUrl=(input.siteUrl||identity.siteUrl).replace(/\/$/,''),c=content(input,identity.brandName),digitalAccess=await resolveDigitalAccess(input),html=`<!doctype html><html><body style="font-family:Arial,sans-serif;color:#17312b"><div style="max-width:620px;margin:auto;padding:24px"><h2>${escapeHtml(identity.brandName)}</h2><h1>${c.title}</h1><p>${c.body}</p>${bankTransferBlock(input)}${digitalAccessBlock(digitalAccess)}${input.invoiceUrl?`<p><a href="${escapeHtml(input.invoiceUrl)}">Számla megnyitása</a></p>`:''}<p><a href="${siteUrl}/fiokom">Rendeléseim megnyitása</a></p><hr style="border:0;border-top:1px solid #ddd;margin:28px 0"><p style="font-size:12px;color:#68726c">${escapeHtml(identity.brandName)} · tranzakciós értesítés</p></div></body></html>`;
  const sent=await deliverResend({to:input.to,subject:c.subject,html,identity,idempotencyKey:input.idempotencyKey});
  return{...sent,detailsUrl:digitalAccess?.url??`${siteUrl}/fiokom`,renderer:'legacy' as const};
}