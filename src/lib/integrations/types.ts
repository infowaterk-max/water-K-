...
export interface PaymentGateway{createPayment(input:{orderId:string;total:Money;returnUrl:string;cancelUrl?:string;idempotencyKey?:string}):Promise<{redirectUrl:string;providerReference:string}>;verifyCallback(payload:unknown):Promise<PaymentCallbackResult>}
