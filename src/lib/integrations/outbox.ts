...
export type IntegrationJobKind='payment_create'|'payment_callback'|'shipment_create'|'invoice_create';
...
export async function enqueueIntegrationJob(input:{orderId?:string|null;kind:IntegrationJobKind;provider:string;payload?:Record<string,unknown>;status?:IntegrationJobStatus;lastError?:string}){
  ...
  const {data,error}=await admin.from('integration_jobs').insert({
    order_id:input.orderId??null,kind:input.kind,provider:input.provider,status:input.status??'pending',payload:input.payload??{},last_error:input.lastError??null,
