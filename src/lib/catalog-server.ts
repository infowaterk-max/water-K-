...
export async function getProducts():Promise<Product[]>{
  try{
    ...
    if(!instance)return[];
    // Storefront catalogue is resolved server-side with the service client. This lets tenant RLS deny
    // direct anonymous table enumeration while the application still exposes only the safe Product DTO below.
