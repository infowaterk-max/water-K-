...
type VariantRow={id:string;sku:string;label:string;net_price_huf:number;gross_price_huf:number;reseller_net_price_huf:number|null;reseller_gross_price_huf:number|null;stock_quantity:number;weight_grams:number|null;minimum_order_quantity:number|null;order_multiple:number|null;primary_media_id:string|null;product_id:string;instance_id:string|null;fulfillment_type:string|null;products:{slug:string;name:string;short_description:string|null;seo_title:string|null;seo_description:string|null;active:boolean;audience:string|null;featured:boolean|null;created_at:string;use_cases:string[]|null;highlights:string[]|null;fulfillment_type:string|null;instance_id:string|null}|null};
...
const normalizeFulfillment=(value:string|null|undefined):NonNullable<Product['fulfillmentType']>=>value==='digital'?'digital':'physical';
...
export async function getProducts(options:{includeAllChannels?:boolean;throwOnError?:boolean}={}):Promise<Product[]>{
 try{
  const instance=await getCurrentWebshopInstance();if(!instance){if(options.throwOnError)throw new Error('webshop_instance_missing');return[]}
  const admin=createAdminClient(),includeAllChannels=options.includeAllChannels===true;let approvedReseller=false;
  if(!includeAllChannels){try{const supabase=await createClient(),{data:{user}}=await supabase.auth.getUser();if(user){const{data:relation}=await admin.from('customer_instance_roles').select('role,reseller_approved').eq('instance_id',instance.id).eq('user_id',user.id).maybeSingle();approvedReseller=relation?.role==='reseller'&&relation.reseller_approved===true;if(approvedReseller){const{data:channelState,error:channelError}=await admin.from('webshop_sales_channels').select('enabled').eq('instance_id',instance.id).eq('channel_code','b2b').maybeSingle();approvedReseller=!channelError&&channelState?.enabled===true}}}catch{approvedReseller=false}}
  const channel:SalesChannelCode=approvedReseller?'b2b':'b2c';
  const{data,error}=await admin.from('product_variants').select('id,sku,label,net_price_huf,gross_price_huf,reseller_net_price_huf,reseller_gross_price_huf,stock_quantity,weight_grams,minimum_order_quantity,order_multiple,primary_media_id,product_id,instance_id,fulfillment_type,products!inner(slug,name,short_description,seo_title,seo_description,active,audience,featured,created_at,use_cases,highlights,fulfillment_type,instance_id)').eq('instance_id',instance.id).eq('active',true).eq('products.instance_id',instance.id).eq('products.active',true).order('gross_price_huf');
  ...
  return rows.filter(row=>row.instance_id===instance.id&&row.products?.instance_id===instance.id).filter(row=>{if(includeAllChannels)return true;const setting=settingByProduct.get(row.product_id);if(channel==='b2b')return setting?.visible===true;return setting?setting.visible:normalizeAudience(row.products?.audience)!=='professional'}).map(row=>{
   ...
   let grossPrice=explicitChannelPrice?Math.max(0,Number(setting?.gross_price??baseGross)):baseGross,netPrice=explicitChannelPrice?deriveNet(grossPrice,baseGross,baseNet):baseNet;const discountPercent=!explicitChannelPrice&&!includeAllChannels?normalizeDiscount(setting?.discount_percent):null,originalGrossPrice=discountPercent!=null&&discountPercent>0?grossPrice:undefined;if(discountPercent!=null&&discountPercent>0){grossPrice=applyDiscount(grossPrice,discountPercent);netPrice=applyDiscount(netPrice,discountPercent)}
