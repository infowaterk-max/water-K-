...
export async function getCommerceProviders(type?:CommerceProviderType):Promise<CommerceProvider[]>{
 ...
 let catalogQuery=admin.from('commerce_provider_catalog').select('code,provider_type,name,connection_mode,adapter_key,fulfillment_kind,is_available,sort_order').eq('is_available',true).order('sort_order');
