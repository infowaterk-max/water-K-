...
export const emailBlockRegistry:Record<EmailBlockType,EmailBlockDefinition>={
  ...
  heading:{type:'heading',category:'content',contentSchema:z.object({text:z.string().min(1).max(500),level:z.enum(['h1','h2','h3']).default('h2'),align:z.enum(['left','center','right']).default('left')}).strict()},
  text:{type:'text',category:'content',contentSchema:z.object({text:z.string().max(8000),align:z.enum(['left','center','right']).default('left')}).strict()},
  button:{type:'button',category:'content',contentSchema:z.object({label:z.string().min(1).max(200),href:z.string().min(1).max(2000),align:z.enum(['left','center','right']).default('left')}).strict()},
