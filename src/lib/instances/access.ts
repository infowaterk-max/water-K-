...
export type StorefrontSocialLinks={facebook?:string;instagram?:string;youtube?:string;tiktok?:string;linkedin?:string};
...
export async function getCurrentWebshopInstance():Promise<WebshopInstance|null>{
  ...
  // role_bindings is authoritative once a user has any RBAC history. Revoked or expired
  // bindings must never be resurrected by the legacy compatibility membership table.
  const{data:bindingData,error:bindingError}=await admin.from('role_bindings')
    .select('organization_id,instance_id,valid_from,valid_until,revoked_at')
    .eq('user_id',auth.user.id);
  const bindings=(bindingData??[])as BindingRow[];
  const active=!bindingError?bindings.filter(binding=>!binding.revoked_at&&binding.valid_from<=now&&(!binding.valid_until||binding.valid_until>now)):[];
  if(!bindingError){
    const directIds=[...new Set(active.map(binding=>binding.instance_id).filter((id):id is string=>Boolean(id)))];
    const orgIds=[...new Set(active.filter(binding=>!binding.instance_id).map(binding=>binding.organization_id).filter(Boolean))];
  ...
  // Compatibility is permitted only for users that have never entered the new RBAC model.
  // A failed RBAC read also fails closed instead of falling back to weaker authorization.
  if(candidates.size===0&&!bindingError&&bindings.length===0){
