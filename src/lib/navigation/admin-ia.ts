...
export type ResolvedAdminNavItem=Pick<AdminNavItem,'id'|'href'|'label'|'description'|'group'|'reportFamily'>;
...
export const MERCHANT_NAVIGATION:readonly AdminNavSection[]=[
  {id:'home',label:'Kezdőlap',items:[
    {id:'overview',href:'/admin',label:'Áttekintés',description:'A webshop napi állapota, legfontosabb mutatói és rövid műveleti összképe.',permission:'store.read',reportFamily:'overview'},
    {id:'launch',href:'/admin/indulas',label:'Indítási központ',description:'A webshop indulásához és üzemeltetési készültségéhez tartozó ellenőrzések.',permission:'store.manage'},
  ]},
  {id:'sales',label:'Értékesítés',items:[
    ...
    {id:'customer-value',href:'/admin/ugyfelertek',label:'Ügyfélérték',description:'Ügyfélérték, aktivitás és megtartási mutatók döntéstámogatáshoz.',feature:'crm',permission:'analytics.read',group:'Ügyfélmunka',reportFamily:'customer'},
  ...
  {id:'catalog',label:'Katalógus',items:[
    ...
    {id:'inventory-analysis',href:'/admin/keszlet-elemzes',label:'Készletelemzés',description:'Készletkockázatok és készletmozgások elemzése.',feature:'advancedAnalytics',permission:'analytics.read',group:'Készletintelligencia',reportFamily:'inventory'},
  ...
  {id:'marketing',label:'Marketing és tartalom',items:[
    ...
    {id:'reviews',href:'/admin/velemenyek',label:'Vásárlói vélemények',description:'Vélemények és moderációs műveletek.',feature:'reviews',permission:'marketing.manage',group:'Tartalom'},
  ]},
  {id:'operations',label:'Működés',items:[
    {id:'automation',href:'/admin/automatizalas',label:'Automatizálási központ',description:'Automatizált üzleti és kommunikációs munkafolyamatok.',feature:'automation',permission:'store.manage',group:'Automatizálás és iroda'},
    {id:'office',href:'/admin/kommunikacio',label:'Digitális iroda',description:'Tenant-szintű kommunikációs munkafolyamatok és üzenetek.',feature:'officeCommunication',permission:'support.manage',group:'Automatizálás és iroda'},
    {id:'blocklist',href:'/admin/kommunikacio/tiltolista',label:'Kommunikációs tiltólista',description:'Kommunikációból kizárt címzettek és tiltási állapotok.',feature:'officeCommunication',permission:'support.manage',group:'Automatizálás és iroda'},
  ...
  {id:'analytics',label:'Elemzés és pénzügy',items:[
    {id:'analytics',href:'/admin/elemzes',label:'Értékesítési elemzés',description:'Forgalom, fedezet, rendelési érték és termékszintű jövedelmezőség.',feature:'advancedAnalytics',permission:'analytics.read',group:'Teljesítmény',reportFamily:'sales'},
    {id:'growth',href:'/admin/novekedes',label:'Növekedés',description:'Növekedési és megtartási mutatók üzleti döntésekhez.',feature:'advancedAnalytics',permission:'analytics.read',group:'Teljesítmény',reportFamily:'growth'},
    {id:'executive',href:'/admin/vezetoi',label:'Vezetői analitika',description:'Vezetői szintű összesített üzleti mutatók és kockázati jelzések.',feature:'executiveAnalytics',permission:'analytics.read',group:'Vezetői nézetek',reportFamily:'executive'},
    {id:'cashflow',href:'/admin/cashflow',label:'Cash-flow',description:'Pénzáramlási és likviditási döntéstámogató nézet.',feature:'cashflow',permission:'analytics.read',group:'Vezetői nézetek',reportFamily:'finance'},
  ]},
  {id:'system',label:'Rendszer',items:[
    {id:'pilot',href:'/admin/pilot-acceptance',label:'Pilot acceptance',description:'A pilot elfogadási bizonyítékai és ellenőrzési állapotai.',permission:'store.manage',group:'Bizonyíték és hozzáférés'},
...
const allowed=(item:AdminNavItem,plan:PlanCode,can:(permission?:StorePermission)=>boolean)=>
  (!item.feature||hasPlanFeature(plan,item.feature))&&can(item.permission);

const resolveItem=(item:AdminNavItem):ResolvedAdminNavItem=>({
  id:item.id,href:item.href,label:item.label,description:item.description,group:item.group,reportFamily:item.reportFamily,
});

export function resolveMerchantNavigation(plan:PlanCode,can:(permission?:StorePermission)=>boolean):ResolvedAdminNavSection[]{
  return MERCHANT_NAVIGATION.map(section=>({
    id:section.id,label:section.label,items:section.items.filter(item=>allowed(item,plan,can)).map(resolveItem),
...
export const ADMIN_REPORTING_DESTINATIONS=MERCHANT_NAVIGATION.flatMap(section=>section.items)
  ...
  .map(item=>({id:item.id,href:item.href,label:item.label,feature:item.feature,permission:item.permission,reportFamily:item.reportFamily as AdminReportFamily}));
