...
export async function getReleaseCommerceDefinitionsForInstance(instanceId:string,includeInactive=false):Promise<readonly ReleaseCommerceDefinition[]>{
  ...
  if(error)throw new Error(`RELEASE_COMMERCE_RELEASES_FAILED:${error.message}`);
  ...
  const byRelease=new Map<string,ItemRow[]>();
  for(const item of((itemResult.data??[]) as ItemRow[])){
    const list=byRelease.get(item.release_id)??[];
    list.push(item);
    byRelease.set(item.release_id,list);
  }
...
export async function getStorefrontReleaseCommerceBundleForInstance(instanceId:string):Promise<StorefrontReleaseCommerceBundle>{
  ...
  if(productResult.error)throw new Error(`RELEASE_COMMERCE_PRODUCTS_FAILED:${productResult.error.message}`);
  if(variantResult.error)throw new Error(`RELEASE_COMMERCE_VARIANTS_FAILED:${variantResult.error.message}`);
  if(channelResult.error)throw new Error(`RELEASE_COMMERCE_CHANNEL_FAILED:${channelResult.error.message}`);
  ...
  for(const mapping of mapped){
    const variant=variantsById.get(mapping.variantId),product=products.get(mapping.productId);
    if(!variant||!product||variant.product_id!==product.id)continue;
  ...
  const releaseStatus=primary?`${primary.title} · ${primary.statusLabel}${primary.state==='scheduled'?` · ${new Intl.DateTimeFormat('hu-HU',{dateStyle:'medium',timeStyle:'short',timeZone:'Europe/Budapest'}).format(new Date(primary.startsAt))}`:''}`:'Nincs aktív release státusz.';
