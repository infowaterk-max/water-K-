...
export const STOREFRONT_VISUAL_LAYER_COMPONENT_DEFINITIONS:readonly StorefrontRuntimeComponentDefinition[]=[
  {
    manifest:defineStorefrontBuilderComponent({foundationVersion:STOREFRONT_BUILDER_FOUNDATION_VERSION,componentKey:'visual.layered-canvas',componentVersion:1,schemaSlot:'children',pageTypes:PAGE_TYPES,configurable:['height','tone','radius'],responsiveMode:'container',capability:{minPlan:'alap',features:[]}}),
    allowsChildren:true,
  },
  {
    manifest:defineStorefrontBuilderComponent({foundationVersion:STOREFRONT_BUILDER_FOUNDATION_VERSION,componentKey:'visual.layer',componentVersion:1,schemaSlot:'children',pageTypes:PAGE_TYPES,configurable:['position','offsetX','offsetY','zIndex','width','tone','padding','opacity','pointerEvents'],responsiveMode:'container',capability:{minPlan:'alap',features:[]}}),
    allowsChildren:true,
  },
] as const;

export function createStorefrontVisualLayerComponentRegistry(){
