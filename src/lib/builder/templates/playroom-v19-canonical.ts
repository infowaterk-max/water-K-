...
function patchBlogArticleMedia(item:StorefrontComponentNode):StorefrontComponentNode{
  ...
  return{
    ...
    config:{
      ...
      style:{...style,height:'16rem',maxHeight:'16rem',minHeight:'0'},
    },
    bindings:{
      ...bindings,
...
function buildBlogArticle(){
  ...
  return complete(source,source.sections.map(patchBlogArticleMedia),{visualPreset:'playroom-v19-editorial-home-parity-media-polish',archetype:'editorial-reading',editorialFallbackMedia:'controller-setup-no-couch-lifestyle'});
}
