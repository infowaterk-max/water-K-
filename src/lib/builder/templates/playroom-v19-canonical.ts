...
const complete=(source:StorefrontPageDocument,sections:StorefrontComponentNode[],metadata:JsonRecord={}):StorefrontPageDocument=>({...source,sections,metadata:{...(source.metadata??{}),subpageParityRelease:'playroom-v19-complete-family-v3',homeParityGrammar:true,compactMedia:true,...metadata}});
...
function buildAccount(){
  ...
  return complete(source,[header(source),section('playroom-account-hero',[grid('playroom-account-hero-grid',[stack('playroom-account-copy',[eyebrow('playroom-account-kicker','PLAYER PROFILE','#55e7ff'),heading('playroom-account-title','A te Playroomod.',1),copy('playroom-account-copy-text','Rendelések, mentett elemek és fiókadatok egy sűrű, feladatorientált felületen.')],7,{...PANEL_ALT,padding:'1rem'}),stack('playroom-account-status',[micro('playroom-account-status-orders','ORDERS','Rendelések','Állapot és előzmények.',12),micro('playroom-account-status-saved','SAVED','Kedvencek','Gyors visszatérés.',12,'#ff63bf'),micro('playroom-account-status-profile','PROFILE','Fiókadatok','Kapcsolat és számlázás.',12,'#b8e34a')],5)],'m')]),section('playroom-account-navigation-presets',[grid('playroom-account-navigation-grid',[micro('playroom-account-orders','ORDERS','Rendeléseim','Korábbi és folyamatban lévő rendelések.',4),micro('playroom-account-favorites','SAVED','Kedvencek','Mentett játékok és kiegészítők.',4,'#ff63bf'),micro('playroom-account-profile','PROFILE','Fiókadatok','Személyes és kapcsolati adatok.',4,'#b8e34a')])],'m'),footer(source)],{visualPreset:'playroom-v19-account-home-parity',archetype:'account-task',addonSemanticContexts:['account.order-details']});
}
...
function buildFaq(){
  ...
  return complete(source,[header(source),section('playroom-faq-hero',[grid('playroom-faq-hero-grid',[stack('playroom-faq-hero-copy',[eyebrow('playroom-faq-kicker','HELP CENTER'),heading('playroom-faq-title','Gyors válaszok, játékos zaj nélkül.',1),copy('playroom-faq-copy','Olvasható, sűrű segítség — nem marketing hero.')],8,{...PANEL_ALT,padding:'1rem'}),stack('playroom-faq-topics',[micro('playroom-faq-topic-order','ORDER','Rendelés','Állapot és checkout.',12),micro('playroom-faq-topic-product','PRODUCT','Termék','Platform és készlet.',12,'#b8e34a')],4)],'m')]),section('playroom-faq-questions-preset',[grid('playroom-faq-list',[stack('playroom-faq-column-a',[faq('playroom-faq-platform','Honnan tudom, hogy kompatibilis-e?','A strukturált compatibility authority dönt.'),faq('playroom-faq-stock','A sablon mutat készletet és árat?','Igen, kizárólag valós commerce adatokból.')],6),stack('playroom-faq-column-b',[faq('playroom-faq-order','Mi történik a pénztárnál?','A közös E13 checkout végzi a végső validációt.'),faq('playroom-faq-payment','Hol jelennek meg a fizetési módok?','A Fizetés accordion lépésben, a jelenlegi design rendszerhez igazodva.')],6)],'m')]),section('playroom-faq-help-cta-preset',[grid('playroom-faq-help-grid',[stack('playroom-faq-help-copy',[eyebrow('playroom-faq-help-kicker','STILL STUCK?','#ff63bf'),heading('playroom-faq-help-title','Segítünk tovább.',2),copy('playroom-faq-help-copy','Ha a gyors válasz nem elég, innen természetesen jutsz az ügyfélszolgálathoz.'),button('playroom-faq-help-cta','Kapcsolat','/kapcsolat','secondary')],8,{...PANEL_ALT,padding:'1rem'}),compactImage('playroom-faq-help-image',PHOTO.audio,'Gaming headset részlet',4,'12rem')],'m')]),footer(source)],{visualPreset:'playroom-v19-faq-home-parity',pageRhythm:'calm-dense'});
}

function buildContact(){
  const source=pageOf('contact');
  return complete(source,[header(source),section('playroom-contact-hero',[grid('playroom-contact-hero-grid',[stack('playroom-contact-copy',[eyebrow('playroom-contact-kicker','PLAYER SUPPORT','#55e7ff'),heading('playroom-contact-title','Beszéljünk.',1),copy('playroom-contact-copy-text','Az ügyfélszolgálati oldal feladata a jó útvonal megmutatása, nem egy hatalmas lifestyle fotó.')],8,{...PANEL_ALT,padding:'1rem'}),stack('playroom-contact-expectations',[micro('playroom-contact-expect-order','ORDER','Rendelés','Rendelési azonosítóval gyorsabb.',12),micro('playroom-contact-expect-product','PRODUCT','Termék','Platform és kompatibilitás.',12,'#b8e34a')],4)],'m')]),section('playroom-contact-options-preset',[grid('playroom-contact-options-grid',[micro('playroom-contact-orders','ORDER HELP','Rendeléssel kapcsolatban','Rendelési kérdések és állapot.',4,'#b8e34a'),micro('playroom-contact-product','PRODUCT HELP','Termék és kompatibilitás','Termékinformációs segítség.',4),micro('playroom-contact-general','GENERAL','Általános kérdés','Egyéb ügyfélszolgálati kapcsolat.',4,'#ff63bf')])],'m'),footer(source)],{visualPreset:'playroom-v19-contact-home-parity',pageRhythm:'calm-dense'});
}
...
const overrides:Partial<Record<StorefrontPageDocument['pageType'],StorefrontPageDocument>>={
  home:buildHome(),search:buildSearch(),cart:buildCart(),checkout:buildCheckout(),account:buildAccount(),content:buildContent(),'blog-index':buildBlogIndex(),faq:buildFaq(),contact:buildContact(),legal:buildLegal(),'not-found':buildNotFound(),
};
