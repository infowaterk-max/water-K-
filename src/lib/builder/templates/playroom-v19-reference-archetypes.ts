...
function buildCatalog(){
  ...
  return withMetadata(source,[
    ...
    section('playroom-catalog-hero',[grid('playroom-catalog-hero-grid',[
      stack('playroom-catalog-hero-copy',[eyebrow('playroom-catalog-kicker','DISCOVER / PLAY / REPEAT','#ff63bf'),heading('playroom-catalog-title','Találd meg a következő játékod.',1),copy('playroom-catalog-copy','A katalógus itt nem egy óriásplakát: gyorsan elvisz a platformhoz, játékstílushoz és a valódi kínálathoz.'),button('playroom-catalog-cta','Mutasd a kínálatot','#playroom-catalog-products')],5,{...PANEL_ALT,padding:'1rem 1.05rem',justifyContent:'center'}),
      stack('playroom-catalog-hero-media',[compactImage('playroom-catalog-hero-image',PHOTO.setup,'Kompakt RGB gaming setup',12,'16.5rem'),grid('playroom-catalog-hero-signals',[micro('playroom-catalog-signal-new','NEW','Újdonságok','Friss belépési pont.',4,'#b8e34a'),micro('playroom-catalog-signal-coop','TOGETHER','Co-op','Közös játékra.',4,'#ff63bf'),micro('playroom-catalog-signal-setup','SETUP','Gear','Kiegészítők gyorsan.',4)],'s')],7)
    ],'m')]),
    section('playroom-catalog-platform-presets',[compactPlatformNav()],'s'),
    section('playroom-catalog-products',[eyebrow('playroom-catalog-products-kicker','BROWSE / FILTER','#b8e34a'),grid('playroom-catalog-products-layout',[stack('playroom-catalog-filters-shell',[facets],3,{...PANEL,padding:'.75rem',alignSelf:'start'}),stack('playroom-catalog-grid-shell',[products],9)],'m')],'m'),
    section('playroom-catalog-discovery-presets',[grid('playroom-catalog-discovery-grid',[micro('playroom-catalog-discovery-coop','PLAY TOGETHER','Co-op és multiplayer','Közös játékhoz.',4,'#ff63bf'),micro('playroom-catalog-discovery-setup','SETUP','Kiegészítők','Kontroller, audio, tér.',4),micro('playroom-catalog-discovery-gift','GIFT MODE','Ajándékötletek','Valódi termékadatokból.',4,'#b8e34a')],'m')],'s'),
    footer(source),
...
function buildProduct(){
  ...
  return withMetadata(source,[
    ...
    section('playroom-product-main',[grid('playroom-product-grid',[
      gallery,
      stack('playroom-product-buybox',[info,reviews,options,purchase,grid('playroom-product-buybox-signals',[micro('playroom-product-buybox-platform','PLATFORM','Ellenőrzött adat','Nem találunk ki kompatibilitást.',4),micro('playroom-product-buybox-stock','STOCK','Valós készlet','Commerce authorityból.',4,'#b8e34a'),micro('playroom-product-buybox-order','ORDER','Biztonságos','Szerveroldali validáció.',4,'#ff63bf')],'s')],5,{...PANEL,padding:'.75rem'})
    ],'m')],'m'),
    section('playroom-product-confidence',[grid('playroom-product-confidence-grid',[micro('playroom-product-confidence-platform','PLATFORM','Kompatibilitás','Strukturált bizonyíték.',4),micro('playroom-product-confidence-stock','AVAILABILITY','Elérhetőség','Valós készlet és ár.',4,'#b8e34a'),micro('playroom-product-confidence-checkout','CHECKOUT','Rendelés','Végső validáció.',4,'#ff63bf')],'m')],'s'),
    ...
    section('playroom-product-story-preset',[grid('playroom-product-story-grid',[compactImage('playroom-product-story-image',PHOTO.controller,'Gaming kontroller részlet',4,'16rem'),stack('playroom-product-story-copy',[eyebrow('playroom-product-story-kicker','GAME NIGHT READY','#ff63bf'),heading('playroom-product-story-title','A termékoldalnak döntést kell segítenie.',2),copy('playroom-product-story-copy-text','A nagy, ismétlődő fotó helyett itt kompakt média, használható termékinformáció és világos következő lépés dolgozik együtt.'),grid('playroom-product-story-notes',[micro('playroom-product-story-note-data','DATA','Specifikáció','Átlátható adatok.',4),micro('playroom-product-story-note-match','MATCH','Platform','Bizonyíték-alapú.',4,'#b8e34a'),micro('playroom-product-story-note-next','NEXT','Ajánlások','Valódi kínálatból.',4,'#ff63bf')],'s'),button('playroom-product-story-cta','Kapcsolódó ajánlatok','#playroom-product-recommendations','secondary')],8,{...PANEL,padding:'1rem'})],'m')],'m'),
...
function buildBlogArticle(){
  ...
  return withMetadata(source,[
    ...
    section('playroom-blog-article-body-preset',[grid('playroom-blog-article-body-grid',[
      ...
      grid('playroom-blog-article-related',[micro('playroom-blog-article-related-platform','PLATFORM','Platform guide','Melyik rendszer illik hozzád?',4),micro('playroom-blog-article-related-coop','TOGETHER','Co-op esték','Játékok közös élményhez.',4,'#ff63bf'),micro('playroom-blog-article-related-setup','SETUP','Setup tippek','Kontroller, audio és tér.',4,'#b8e34a')],'m')
    ],'m')],'m'),
    footer(source),
