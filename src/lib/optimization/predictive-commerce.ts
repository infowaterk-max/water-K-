...
export async function loadPredictiveCommerceSignals(instanceId:string):Promise<PredictiveCommerceSignal[]>{
  ...
  const input:PredictiveCommerceSnapshot={observedAt:new Date().toISOString(),variants:snapshot.variants.map(v=>({id:v.id,sku:v.sku,label:v.label,netPriceHuf:v.net_price_huf,unitCostNetHuf:v.unit_cost_net_huf,stockQuantity:v.stock_quantity,active:v.active})),opportunities:snapshot.opportunities.map(o=>({id:o.id,priorityScore:o.priority_score,expectedValueNetHuf:o.expected_value_net_huf,probabilityPercent:o.probability_percent,status:o.status,channel:o.channel,kind:o.kind})),promotions:snapshot.promotionPreviews,growth:{atRiskCustomers:n(snapshot.growth?.at_risk_customers),winbackCustomers:n(snapshot.growth?.winback_customers),openCheckoutRecoveries:n(snapshot.growth?.open_checkout_recoveries)}};
...
export async function saveCommerceAutonomyPolicy(instanceId:string,actorId:string,policy:CommerceAutonomyPolicy){
  ...
  const{data,error}=await admin.from('commerce_autonomy_policies').upsert(payload,{onConflict:'instance_id'}).select().single();if(error)throw error;return data;
}
...
export async function executeBoundedPrediction(instanceId:string,actorId:string,signal:PredictiveCommerceSignal){
  ...
  const{data:existing,error:existingError}=await admin.from('commerce_autonomy_runs').select('*').eq('instance_id',instanceId).eq('run_key',runKey).maybeSingle();if(existingError)throw existingError;if(existing)return existing;
  if(guard.decision==='approval_required'){
    const proposalId=await ensureHighRiskProposal(instanceId,actorId,signal);
    const{data,error}=await admin.from('commerce_autonomy_runs').insert({instance_id:instanceId,run_key:runKey,prediction_key:signal.key,action_kind:signal.actionKind,risk_class:signal.riskClass,confidence:signal.confidence,risk_score:signal.riskScore,expected_impact_net_huf:signal.expectedImpactNetHuf,status:'approval_required',proposal_id:proposalId,policy_snapshot:policy,evidence_snapshot:snapshot,result:{guard}}).select().single();if(error)throw error;return data;
  }
  if(guard.decision!=='autonomous_allowed'||!signal.autonomousEvent){
    const status=guard.decision==='supervised'?'supervised':'blocked';
    const{data,error}=await admin.from('commerce_autonomy_runs').insert({instance_id:instanceId,run_key:runKey,prediction_key:signal.key,action_kind:signal.actionKind,risk_class:signal.riskClass,confidence:signal.confidence,risk_score:signal.riskScore,expected_impact_net_huf:signal.expectedImpactNetHuf,status,policy_snapshot:policy,evidence_snapshot:snapshot,result:{guard}}).select().single();if(error)throw error;return data;
  }
...
export async function compensateAutonomyRun(instanceId:string,actorId:string,runId:string){
  ...
  const{data:transition,error:transitionError}=await admin.rpc('compensate_block19_runbook_v1',{p_instance_id:instanceId,p_runbook_instance_id:run.runbook_instance_id,p_actor_id:actorId,p_event_key:eventKey});
