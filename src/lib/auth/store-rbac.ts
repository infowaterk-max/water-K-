...
export async function getActiveStoreRoles(instanceId:string):Promise<StoreRole[]>{
  ...
  const {data}=await admin.from('role_bindings')
    ...
    .lte('valid_from',now);
  return(data??[])
    .filter(row=>(row.instance_id===instanceId||row.instance_id===null)&&(!row.valid_until||row.valid_until>now))
    .map(row=>row.role_code)
    .filter((role):role is StoreRole=>role in ROLE_PERMISSIONS);
}
