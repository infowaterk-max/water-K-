...
export async function getActiveStoreRoles(instanceId:string):Promise<StoreRole[]>{
  ...
  const {data}=await admin.from('role_bindings').select('role_code,valid_from,valid_until,revoked_at').eq('user_id',user.id).or(`instance_id.eq.${instanceId},instance_id.is.null`).is('revoked_at',null).lte('valid_from',now);
  return (data??[]).filter(row=>!row.valid_until||row.valid_until>now).map(row=>row.role_code).filter((role):role is StoreRole=>role in ROLE_PERMISSIONS);
}

export async function hasStorePermission(instanceId:string,permission:StorePermission){
  const roles=await getActiveStoreRoles(instanceId);
