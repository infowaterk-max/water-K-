...
export type WorkerSummary={recovered:number;queuedStock:number;claimed:number;sent:number;failed:number;blocked:number};
export async function runCommunicationWorker(limit=10):Promise<WorkerSummary>{
  const admin=createAdminClient();
  const summary:WorkerSummary={recovered:0,queuedStock:0,claimed:0,sent:0,failed:0,blocked:0};
