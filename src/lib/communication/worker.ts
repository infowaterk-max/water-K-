...
import { brandedSubject, getCommunicationIdentity } from './identity';
...
export async function runCommunicationWorker(limit=10):Promise<WorkerSummary>{
  ...
  const [{data,error},identity]=await Promise.all([admin.rpc('claim_communication_jobs',{p_limit:Math.max(1,Math.min(limit,50))}),getCommunicationIdentity()]);
  if(error)throw error;const jobs=(data??[]) as ClaimedJob[];summary.claimed=jobs.length;const provider=getCommunicationProvider();
  for(const job of jobs){try{const template=getCommunicationTemplate(job.template_key);if(!template||template.purpose!==job.purpose){await admin.rpc('fail_communication_job',{p_id:job.id,p_claim_token:job.claim_token,p_error:'INVALID_TEMPLATE_OR_PURPOSE',p_retry:false});summary.blocked++;continue;}
      const {data:suppressed,error:suppressionError}=await admin.rpc('is_communication_suppressed',{p_email:job.recipient_email});if(suppressionError||suppressed===true){await admin.rpc('fail_communication_job',{p_id:job.id,p_claim_token:job.claim_token,p_error:'RECIPIENT_SUPPRESSED_AT_SEND_TIME',p_retry:false});if(job.template_key==='stock_available')await admin.from('stock_notifications').update({status:'cancelled'}).eq('communication_job_id',job.id);summary.blocked++;continue;}
      ...
      const {data:completed,error:completeError}=await admin.rpc('complete_communication_job',{p_id:job.id,p_claim_token:job.claim_token,p_provider_message_id:result.providerMessageId});if(completeError||completed!==true)throw completeError??new Error('COMMUNICATION_CLAIM_LOST');if(job.template_key==='stock_available')await admin.from('stock_notifications').update({status:'sent',sent_at:new Date().toISOString()}).eq('communication_job_id',job.id);summary.sent++;
    }catch(error){const message=error instanceof Error?error.message:'UNKNOWN_COMMUNICATION_ERROR';const retry=job.attempts<5;await admin.rpc('fail_communication_job',{p_id:job.id,p_claim_token:job.claim_token,p_error:message,p_retry:retry});summary.failed++;}}
  return summary;
}
