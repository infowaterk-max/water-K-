...
export type CommunicationMessage = { to: string; subject: string; templateKey: string; purpose: 'transactional' | 'marketing'; payload: Record<string, unknown> };
...
function render(message: CommunicationMessage) {
  ...
  else if (message.templateKey === 'support_reply') {
    const order = p.orderNumber ? `<p>Kapcsolódó rendelés: <strong>${esc(p.orderNumber)}</strong></p>` : '';
  ...
  const unsub = message.purpose === 'marketing' ? unsubscribeUrl(message.to) : null;
  return `<!doctype html><html><body style="font-family:Arial,sans-serif;line-height:1.55;color:#18221d"><div style="max-width:620px;margin:auto;padding:24px"><h2>Water-K</h2>${body}<hr style="border:0;border-top:1px solid #ddd;margin:28px 0"><p style="font-size:12px;color:#68726c">Ez egy ${message.purpose === 'marketing' ? 'marketing' : 'tranzakciós'} üzenet a Water-K rendszeréből.${unsub ? ` <a href="${unsub}">Leiratkozás</a>` : ''}</p></div></body></html>`;
}
...
class ResendProvider implements CommunicationProvider {
  async send(message: CommunicationMessage): Promise<CommunicationSendResult> {
    ...
    const unsub = message.purpose === 'marketing' ? unsubscribeUrl(message.to) : null;
    if (message.purpose === 'marketing' && !unsub) throw new Error('MARKETING_UNSUBSCRIBE_NOT_CONFIGURED');
    const headers: Record<string, string> = {};
    if (unsub) { headers['List-Unsubscribe'] = `<${unsub}>`; headers['List-Unsubscribe-Post'] = 'List-Unsubscribe=One-Click'; }
    const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { authorization: `Bearer ${key}`, 'content-type': 'application/json' }, body: JSON.stringify({ from, to: [message.to], subject: message.subject, html: render(message), headers }) });
    const data = await response.json().catch(() => ({})) as { id?: string; message?: string };
    if (!response.ok || !data.id) throw new Error(data.message || `RESEND_HTTP_${response.status}`);
