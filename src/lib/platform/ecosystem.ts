...
type AppRow={app_key:string;display_name:string;version:string;release_state:string;allowed_scopes:string[]|null;metadata:Record<string,unknown>|null};
type InstallationRow={id:string;instance_id:string;app_key:string;status:string};
...
function cleanError(error:unknown){return(error instanceof Error?error.message:String(error)).slice(0,500);}
...
export async function registerExtensionApp(input:{appKey:string;displayName:string;version:string;releaseState:'draft'|'released'|'suspended';allowedScopes:unknown;actorId:string;metadata?:Record<string,unknown>}){
  ...
  const allowedScopes=normalizeExtensionScopes(input.allowedScopes);
  const admin=createAdminClient();
  const{data,error}=await admin.from('extension_app_catalog').upsert({app_key:appKey,display_name:displayName,version,release_state:input.releaseState,allowed_scopes:allowedScopes,metadata:boundedExtensionEvidence(input.metadata),created_by:input.actorId,updated_at:nowIso()},{onConflict:'app_key'}).select('app_key,display_name,version,release_state,allowed_scopes,metadata').single();
...
export async function installExtension(input:{instanceId:string;appKey:string;actorId:string;configuration?:Record<string,unknown>}){
  const admin=createAdminClient(),appKey=cleanAppKey(input.appKey);
  const{data:app,error:appError}=await admin.from('extension_app_catalog').select('app_key,release_state').eq('app_key',appKey).maybeSingle();
...
export async function setExtensionInstallationStatus(input:{instanceId:string;installationId:string;status:'enabled'|'disabled'|'revoked'}){
  const admin=createAdminClient();
  const{data,error}=await admin.from('extension_installations').update({status:input.status,updated_at:nowIso()}).eq('id',input.installationId).eq('instance_id',input.instanceId).select('id,instance_id,app_key,status,updated_at').maybeSingle();
...
export async function issueExtensionApiCredential(input:{instanceId:string;installationId:string;actorId:string;scopes:unknown;expiresAt?:string|null}){
  const admin=createAdminClient();
  const{data:installation,error:installationError}=await admin.from('extension_installations').select('id,instance_id,app_key,status').eq('id',input.installationId).eq('instance_id',input.instanceId).maybeSingle();
  ...
  if(appError)throw appError;if(!app||app.release_state!=='released')throw new Error('EXTENSION_APP_NOT_RELEASED');
  const scopes=normalizeExtensionScopes(input.scopes,Array.isArray(app.allowed_scopes)?app.allowed_scopes:[]);
  if(scopes.length===0)throw new Error('EXTENSION_SCOPE_REQUIRED');
  let expiresAt:string|null=null;
  if(input.expiresAt){const time=Date.parse(input.expiresAt);if(!Number.isFinite(time)||time<=Date.now()||time>Date.now()+366*24*60*60_000)throw new Error('EXTENSION_CREDENTIAL_EXPIRY_INVALID');expiresAt=new Date(time).toISOString();}
...
export async function revokeExtensionApiCredential(instanceId:string,credentialId:string){
  const admin=createAdminClient();
  const{data,error}=await admin.from('extension_api_credentials').update({revoked_at:nowIso()}).eq('id',credentialId).eq('instance_id',instanceId).is('revoked_at',null).select('id,key_prefix,revoked_at').maybeSingle();
...
export async function authenticateExtensionRequest(request:Request,requiredScope:ExtensionApiScope){
  ...
  const admin=createAdminClient();
  const{data:credential,error}=await admin.from('extension_api_credentials').select('id,instance_id,installation_id,key_prefix,secret_hash,scopes,expires_at,revoked_at').eq('key_prefix',parsed.prefix).maybeSingle();
  if(error||!credential)return null;
  const row=credential as CredentialRow;
  ...
  const[{data:installation},{data:apiEntitlement}]=await Promise.all([
    ...
    Promise.resolve(await hasFeatureEntitlement(row.instance_id,'apiAccess')),
  ]);
  if(!apiEntitlement||!installation||installation.status!=='enabled')return null;
  ...
  if(process.env.SECURITY_RATE_LIMIT_ENABLED==='true'){
    const{data:allowed,error:rateError}=await admin.rpc('consume_security_rate_limit',{p_rate_key:`extension:${row.id}`,p_window_seconds:60,p_max_count:120});
    if(rateError||allowed!==true)return null;
  }
...
export async function createExtensionWebhookSubscription(input:{instanceId:string;installationId:string;eventType:string;endpointUrl:unknown;actorId:string}){
  ...
  const admin=createAdminClient();
  const{data:installation,error:installationError}=await admin.from('extension_installations').select('id,status').eq('id',input.installationId).eq('instance_id',input.instanceId).maybeSingle();
  if(installationError)throw installationError;if(!installation||installation.status!=='enabled')throw new Error('EXTENSION_INSTALLATION_NOT_ENABLED');
  webhookMasterSecret();
...
export async function disableExtensionWebhookSubscription(instanceId:string,subscriptionId:string){
  const admin=createAdminClient();const{data,error}=await admin.from('extension_webhook_subscriptions').update({enabled:false,updated_at:nowIso()}).eq('id',subscriptionId).eq('instance_id',instanceId).select('id,enabled').maybeSingle();if(error)throw error;if(!data)throw new Error('EXTENSION_WEBHOOK_SUBSCRIPTION_NOT_FOUND');return data;
}

export async function queueExtensionWebhookEvent(input:{instanceId:string;eventType:string;eventKey:string;payload?:Record<string,unknown>}){
  const admin=createAdminClient();
  const{data:subscriptions,error}=await admin.from('extension_webhook_subscriptions').select('id,installation_id').eq('instance_id',input.instanceId).eq('event_type',input.eventType).eq('enabled',true);
  if(error)throw error;if(!subscriptions?.length)return 0;
  const rows=subscriptions.map(subscription=>({instance_id:input.instanceId,subscription_id:subscription.id,event_type:input.eventType,event_key:input.eventKey,payload:boundedExtensionEvidence(input.payload),status:'pending'}));
  const{error:insertError}=await admin.from('extension_webhook_deliveries').upsert(rows,{onConflict:'subscription_id,event_key',ignoreDuplicates:true});if(insertError)throw insertError;return rows.length;
}
...
async function automationAllowsDelivery(instanceId:string){
  const admin=createAdminClient();const{data}=await admin.from('automation_control').select('global_paused,circuit_open_until').eq('instance_id',instanceId).maybeSingle();
  if(!data)return true;if(data.global_paused)return false;return !(data.circuit_open_until&&Date.parse(data.circuit_open_until)>Date.now());
}

export async function processDueExtensionWebhookDeliveries(limit=20){
  const admin=createAdminClient(),now=Date.now();
  const{data,error}=await admin.from('extension_webhook_deliveries').select('id,instance_id,subscription_id,event_type,event_key,payload,status,attempt_count,next_attempt_at,updated_at').in('status',['pending','retry','processing']).order('created_at',{ascending:true}).limit(Math.max(1,Math.min(100,limit*4)));
  if(error)throw error;
  const due=((data??[]) as DeliveryRow[]).filter(row=>isDue(row,now)).slice(0,Math.max(1,Math.min(100,limit))),results:Array<{id:string;instanceId:string;status:string;error?:string}>=[],control=new Map<string,boolean>();
  for(const row of due){
    ...
    const attempt=Math.min(20,Number(row.attempt_count??0)+1),claimedAt=nowIso();
    const{data:claimed,error:claimError}=await admin.from('extension_webhook_deliveries').update({status:'processing',attempt_count:attempt,updated_at:claimedAt}).eq('id',row.id).eq('instance_id',row.instance_id).eq('status',row.status).select('id').maybeSingle();
    ...
    try{
      const{data:subscription,error:subscriptionError}=await admin.from('extension_webhook_subscriptions').select('id,installation_id,endpoint_url,enabled').eq('id',row.subscription_id).eq('instance_id',row.instance_id).maybeSingle();
      if(subscriptionError)throw subscriptionError;if(!subscription?.enabled)throw new Error('EXTENSION_WEBHOOK_SUBSCRIPTION_DISABLED');
      const{data:installation,error:installationError}=await admin.from('extension_installations').select('status').eq('id',subscription.installation_id).eq('instance_id',row.instance_id).maybeSingle();
      if(installationError)throw installationError;if(!installation||installation.status!=='enabled')throw new Error('EXTENSION_INSTALLATION_NOT_ENABLED');
      const timestamp=Math.floor(Date.now()/1000).toString(),body=JSON.stringify({version:PLATFORM_ECOSYSTEM_VERSION,deliveryId:row.id,eventType:row.event_type,eventKey:row.event_key,payload:row.payload??{}}),signingSecret=deriveExtensionWebhookSigningSecret(row.subscription_id),signature=createHmac('sha256',signingSecret).update(`${timestamp}.${body}`).digest('hex');
      const response=await fetch(subscription.endpoint_url,{method:'POST',headers:{'content-type':'application/json','user-agent':'Shoperation-Platform-Ecosystem/1.0','x-shoperation-delivery':row.id,'x-shoperation-event':row.event_type,'x-shoperation-timestamp':timestamp,'x-shoperation-signature':`v1=${signature}`},body,redirect:'error',signal:AbortSignal.timeout(10_000)});
      if(!response.ok)throw new Error(`EXTENSION_WEBHOOK_HTTP_${response.status}`);
      await admin.from('extension_webhook_deliveries').update({status:'delivered',response_status:response.status,last_error:null,next_attempt_at:null,delivered_at:nowIso(),updated_at:nowIso()}).eq('id',row.id).eq('instance_id',row.instance_id);
      results.push({id:row.id,instanceId:row.instance_id,status:'delivered'});
    }catch(deliveryError){
      ...
      await admin.from('extension_webhook_deliveries').update({status:terminal?'dead_letter':'retry',last_error:errorText,next_attempt_at:nextAttemptAt,updated_at:nowIso()}).eq('id',row.id).eq('instance_id',row.instance_id);
      results.push({id:row.id,instanceId:row.instance_id,status:terminal?'dead_letter':'retry',error:errorText});
    }
  }
  return results;
}
...
export async function loadExtensionDashboard(instanceId:string){
  const admin=createAdminClient();
  const[apps,installations,credentials,subscriptions,deliveries]=await Promise.all([
