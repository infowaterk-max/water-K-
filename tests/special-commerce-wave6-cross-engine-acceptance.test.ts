...
describe('Special Commerce Wave 6 cross-engine acceptance',()=>{
  ...
  it('uses one D/T/M responsive inheritance and binding-security contract across engines',()=>{
    for(const componentKey of crossEngineKeys){const node:StorefrontComponentNode={id:`responsive-${componentKey}`,componentKey,componentVersion:1,config:{},responsive:{desktop:{gridSpan:10},tablet:{hidden:true},mobile:{gridSpan:6}}};expect(resolveStorefrontResponsiveOverride(node,'desktop')).toEqual({hidden:false,gridSpan:10});expect(resolveStorefrontResponsiveOverride(node,'tablet')).toEqual({hidden:true,gridSpan:10});expect(resolveStorefrontResponsiveOverride(node,'mobile')).toEqual({hidden:true,gridSpan:6});}
