...
describe('shared cart quantity touch contract',()=>{
  it('keeps both stepper buttons at least 24px high and the remove control comfortably larger',()=>{
    ...
    expect(html).toContain('grid-template-rows:24px 24px');
    expect((html.match(/height:24px/g)??[]).length).toBeGreaterThanOrEqual(2);
    expect((html.match(/min-height:24px/g)??[]).length).toBeGreaterThanOrEqual(2);
