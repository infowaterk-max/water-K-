...
describe('Roadmap Block 4 IA, navigation and reporting contract',()=>{
  ...
  it('implements progressive disclosure, mobile/touch IA, breadcrumbs and reporting trust without a visual editor',()=>{
    ...
    expect(layout).toContain('resolveEntitledMerchantNavigation(hasFeature,can,instance?.status,canCapability)');expect(layout).toContain('getFeatureEntitlementDecisions(instance.id,featureCodes)');expect(layout).toContain('<AdminRouteContext sections={sections} operatorItems={operatorItems}/>');expect(layout).not.toContain('const MERCHANT_NAV');expect(navigation.toLowerCase()).not.toContain('drag');
  });
