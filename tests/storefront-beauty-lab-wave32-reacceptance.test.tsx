...
describe('Scale-out Wave 32 Beauty Lab current-baseline reacceptance',()=>{
  ...
  it('locks the three Beauty & Wellness families as structurally distinct journeys',()=>{
  ...
  it('hardens Formula Hero as eight independent shared visual layers with stable node ids and binding paths',()=>{
    ...
    const layerIds=['beauty-hero-image-layer','beauty-hero-overlay-layer','beauty-hero-decoration-layer','beauty-hero-badge-layer','beauty-hero-title-layer','beauty-hero-copy-layer','beauty-hero-primary-cta-layer','beauty-hero-secondary-cta-layer'];
    for(const id of layerIds)expect(home(id)?.componentKey).toBe('visual.layer');
  ...
  it('binds the major split marketing blocks through the shared Builder contract',()=>{
  ...
  it('keeps node identity unique inside every Page Schema and keeps merchant-adjustable design tokens',()=>{
    ...
    expect(BEAUTY_LAB_WAVE32_ACCEPTANCE.builderContract.stableIdentity).toBe('stable-node-ids-and-stable-binding-paths');
  ...
  it('validates all 14 Alap Page Schema presets through the shared Guided + Visual registry on D/T/M contract',()=>{
    ...
    for(const page of BEAUTY_LAB_TEMPLATE_PACKAGE.pages){const result=validateStorefrontPageDocument(page,registry,capability);expect(result.ok,`${page.pageType}: ${JSON.stringify(result.violations)}`).toBe(true);}
  });

  it('renders the layered Formula Hero and E3 surfaces through shared registries on desktop, tablet and mobile',()=>{
    const bindingContext={brand:{name:'Beauty Demo',homeHref:'/',copyright:'© Beauty Demo'},navigation:{primary:[],footer:[]},content:{formulaHero:{badge:'FORMULA DEMO',title:'Formula first',copy:'Strukturált és magyarázható.',primaryLabel:'Finder',primaryHref:'#formula-finder',secondaryLabel:'Összetevők',secondaryHref:'/osszetevok'},routineFeature:{},ingredientStory:{},newFormulas:{},beautyProductGrid:{}},finder:{currentStep:{title:'1. lépés',copy:'Preferencia'},currentQuestion:{label:'Textúra?',options:[]},progressLabel:'1 / 3',resultHref:'#finder-results'},collection:{concerns:[]},catalog:{ingredientIndex:[],newFormulas:[],textureNavigation:[],featured:[]},reviews:{rating:0,count:0,summary:'',href:'#reviews'}};
    for(const viewport of ['desktop','tablet','mobile'] as const){
      const html=renderToStaticMarkup(<StorefrontRuntimeRenderer page={BEAUTY_LAB_HOME_PAGE} viewport={viewport} bindingContext={bindingContext} componentRegistry={createStorefrontGuidedVisualComponentRegistry()} rendererRegistry={createStorefrontGuidedVisualRendererRegistry()} capability={capability}/>);
      expect(html).toContain('data-storefront-visual="layered-canvas"');
      expect(html).toContain('data-storefront-guided="finder"');
      expect(html).toContain('Formula first');
    }
  });

  it('uses the real E3 engine for deterministic exact/partial/zero semantics and excludes ineligible products',()=>{
  ...
  it('keeps Finder and commerce authority outside template switching',()=>{
  ...
  it('preserves the authoritative PDP 7/5 desktop-tablet grid and 12/12 mobile flow',()=>{
  ...
  it('keeps installation draft-only, demo content claim-neutral and checkout provider-neutral',()=>{
    const registry=createStorefrontGuidedVisualComponentRegistry();
