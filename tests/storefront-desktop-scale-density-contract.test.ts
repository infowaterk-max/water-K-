...
describe('Storefront Desktop scale / density contract',()=>{
  ...
  it('keeps operational account density semantic and capability navigation compact',()=>{
    ...
    const legacyCartCss=read('src/app/block3-pilot-batch.css');
    ...
    expect(cartStyleQuantity).toContain("color:'#fff',lineHeight:0");
    ...
    expect(legacyCartCss).toContain('.cartQuantityStepper:not([data-cart-quantity-layout]):not([data-rfq-quantity-layout]) button');
    expect(legacyCartCss).toContain("[data-rfq-step]{color:#fff!important");
