...
describe('Playroom v20 functional acceptance',()=>{
  ...
  it('renders the real E13 checkout inside the active Playroom template instead of the generic checkout shell',()=>{
    ...
    expect(normalizer).toContain("padding:'1.6rem 2.35rem 1.9rem'");
    expect(normalizer).toContain("minHeight:'13rem'");
    expect(normalizer).toContain("fontSize:footerRemFloor(style.fontSize,.82)");
    expect(normalizer).toContain("minHeight:'2rem'");
    ...
    expect(footer.config.style).toMatchObject({padding:'1.6rem 2.35rem 1.9rem',minHeight:'13rem'});
    ...
    expect(footerNav?.config.style).toMatchObject({lineHeight:1.5,gap:'.38rem',fontSize:'.82rem'});
    expect(footerNav?.config.styleSlots).toMatchObject({item:{base:{minHeight:'2rem',display:'flex',alignItems:'center'}}});
    const footerOnly={...normalizedRemapped,sections:[footer]};
    const renderedFooter=render(footerOnly,{});
    expect(renderedFooter).toContain('min-height:13rem');
    expect(renderedFooter).toContain('padding:1.6rem 2.35rem 1.9rem');
    expect(renderedFooter).toContain('line-height:1.5');
    expect(renderedFooter).toContain('font-size:.82rem');
    expect(renderedFooter).toContain('min-height:2rem');
