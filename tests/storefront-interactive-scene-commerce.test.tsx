...
describe('Special Commerce Wave 1 — Interactive Scene Commerce',()=>{
  ...
  it('is a first-class Pro Builder component and fails closed for Alap',()=>{
    ...
    expect(hasStorefrontRuntimeCapability(definition.manifest.capability,{plan:'pro',features:PLANS.pro.features})).toBe(true);
  ...
  it('renders accessible hotspot links from runtime binding data on the same Page Schema',()=>{
    const page:StorefrontPageDocument={
      ...
      sections:[{id:'scene',componentKey:'commerce.interactive-scene',componentVersion:1,config:{sceneKey:'look-1',sceneKind:'look',title:'Shop the Look',hotspots:config.hotspots,backgroundImage:'/look.jpg',backgroundAlt:'Őszi összeállítás',showSetSummary:true},bindings:{products:{path:'catalog.interactiveSceneProducts'}}}],
    };
    const html=renderToStaticMarkup(<StorefrontRuntimeRenderer page={page} viewport="mobile" bindingContext={{catalog:{interactiveSceneProducts:products}}} componentRegistry={createStorefrontVisualBuilderComponentRegistry()} rendererRegistry={createStorefrontVisualBuilderRendererRegistry()} capability={{plan:'pro',features:PLANS.pro.features}}/>);
