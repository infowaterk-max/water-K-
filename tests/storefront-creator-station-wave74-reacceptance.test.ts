...
const focusPages=['home','catalog','search','product','content','blog-index','blog-article','checkout'];
...
describe('Scale-out Wave 74 Creator Station current-baseline reacceptance',()=>{
  it('reconstructs Creator Station as the direct repository-proven successor of Wave 73 Table & Gift',()=>{
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE).toMatchObject({
      wave:74,historicalCounterpartWave:36,historicalPullRequest:198,currentBaselineCounterpartWave:55,currentBaselinePullRequest:257,
      originalTemplateWave:17,originalTemplatePullRequest:140,historicalAcceptance:CREATOR_STATION_WAVE36_ACCEPTANCE.mode,
      currentBaselineAcceptance:CREATOR_STATION_WAVE55_ACCEPTANCE.mode,predecessorAcceptance:TABLE_GIFT_WAVE73_ACCEPTANCE.mode,
      mode:'current-baseline-reacceptance-and-builder-hardening',templateKey:'tech.creator-station',templateVersion:1,inheritedImplementation:true,
    });
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.sequence).toMatchObject({
      previous:'wave73-food.table-gift',current:'tech.creator-station',
      historicalPrevious:'wave35-food.table-gift',historicalCurrent:'wave36-tech.creator-station',historicalNext:'wave37-tech.spec-lab',
      currentBaselinePrevious:'wave54-food.table-gift',currentBaselineCurrent:'wave55-tech.creator-station',
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.provenance).toMatchObject({
  ...
  it('keeps Creator Station visually and structurally distinct from Table & Gift and Spec Lab',()=>{
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.distinctness.separationIncludes).toEqual(expect.arrayContaining(['layout','section-order','workflow-journey','palette','typography','visual-language','creator-imagery']));
  });
  ...
  it('uses only current shared binding namespaces and retains workflow/story hardening without widening contracts',()=>{
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.builderContract).toMatchObject({
      runtimeAllowlistWidened:false,componentRegistryWidened:false,bindingNamespaceWidened:false,pageSchemaAllowlistWidened:false,
    });
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.bindingContract).toMatchObject({
      workflowState:'shared-configurator-namespace',editorialPresentation:'shared-content-namespace',runtimeAllowlistWidened:false,
    });
  });

  it('keeps complete shared E3 and E5 workflow/configurator surfaces without template-local authority',()=>{
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.commerceAuthority).toMatchObject({
      guidedFinder:'E3-guidance-and-ranking-only',configurator:'E5-setup-intent-and-read-model-only',
      noTemplateGuidanceAuthority:true,noTemplateConfiguratorAuthority:true,
    });
  });

  it('keeps E6/E7 explainable compatibility and structured product truth under shared engines',()=>{
  ...
  it('keeps Creator Magazine on shared E10 presentation and authoritative truth out of marketing imagery',()=>{
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.builderContract).toMatchObject({marketingCopyBakedIntoImages:false,authoritativeTruthBakedIntoImages:false});
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.commerceAuthority.editorial).toBe('E10-editorial-read-model-presentation-only');
  });

  it('validates all 14 Alap Desktop/Tablet/Mobile presets including the required focus pages',()=>{
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.builderContract.focusPageTypes).toEqual(focusPages);
  ...
  it('asserts actual Product node geometry rather than metadata-only responsive claims',()=>{
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.responsiveProduct).toEqual({
      galleryNode:'creator-product-gallery',gallery:{desktop:7,tablet:7,mobile:12},
      buyboxNode:'creator-product-buybox',buybox:{desktop:5,tablet:5,mobile:12},
    });
  });

  it('keeps installation draft-only with non-authoritative demo fixtures and immutable commerce/customer/order authority',()=>{
    const registry=createStorefrontConfiguratorComponentRegistry();
    ...
    expect(JSON.stringify(CREATOR_STATION_TEMPLATE_PACKAGE.demoFixtures??[])).not.toMatch(/compatible.?true|compatibilityStatus|guaranteed|fps|latency.?guarantee|fixedSetupPrice|fixed price|guaranteed stock|only [0-9]+ left/i);
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.installationContract).toMatchObject({draftOnly:true,demoNamespace:'tech-creator-station'});
  });

  it('preserves provider-neutral E13 and predecessor/current/historical acceptance while excluding release-side mutations and Wave 75',()=>{
    expect(TABLE_GIFT_WAVE73_ACCEPTANCE.mode).toBe('current-baseline-reacceptance-and-builder-hardening');
    expect(CREATOR_STATION_WAVE55_ACCEPTANCE.mode).toBe('current-baseline-reacceptance-and-builder-hardening');
    expect(CREATOR_STATION_WAVE36_ACCEPTANCE.mode).toBeTruthy();
    ...
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.commerceAuthority).toMatchObject({
      pricing:'shared-commerce-binding-only',inventory:'shared-commerce-binding-only',variants:'shared-commerce-binding-only',
      checkout:'shared-provider-neutral-E13',noTemplateVariantAuthority:true,noTemplatePaymentAuthority:true,
    });
    expect(CREATOR_STATION_WAVE74_ACCEPTANCE.nonScope).toEqual(expect.arrayContaining([
      ...
      'tenant-status-change','tenant-plan-change','main-merge','wave75-implementation',
    ]));
  });
});
