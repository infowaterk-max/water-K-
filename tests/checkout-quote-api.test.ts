...
describe('checkout quote API',()=>{
  it('resolves active tenant and configured shipping before quoting',()=>{
    const s=read('src/app/api/checkout/quote/route.ts');
    expect(s).toContain('getCurrentWebshopInstance');
    expect(s).toContain("['pilot','active']");
    expect(s).toContain('commerce.shippingOptions.find');
    expect(s).toContain('quoteTenantCheckout');
  });
  it('never accepts client supplied prices or totals',()=>{
    const s=read('src/app/api/checkout/quote/route.ts');
    expect(s).not.toContain('unitPrice:');
    expect(s).not.toContain('subtotal:');
    expect(s).not.toContain('total:');
  });
  it('returns the fulfillment-aware snake-case quote contract consumed by checkout UI',()=>{
    const s=read('src/app/api/checkout/quote/route.ts');
