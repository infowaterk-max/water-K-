...
import {materializeStorefrontFidelityPage,readStorefrontFidelityMetadata,resolveStorefrontStyleSlot} from '@/lib/builder/storefront-fidelity-engine';
...
import {BEAUTY_LAB_REFERENCE_V28_HOME_PAGE,BEAUTY_LAB_REFERENCE_V28_PRODUCT_PAGE,BEAUTY_LAB_TEMPLATE_PACKAGE as BEAUTY_LAB_REFERENCE_V28_PACKAGE} from '@/lib/builder/templates/beauty-lab-reference-v28';
...
describe('Beauty Lab reference v2.8 canonical shared fidelity integration',()=>{
  it('keeps the recovered Home composition in one flattened v2.8 source',()=>{
    expect(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.metadata).toMatchObject({referencePass:'beauty-lab-reference-v2.8',canonicalComposition:'flattened-v2.8-from-v2.4',responsiveReferenceComposition:true,referenceDensity:'compact-desktop-v1',mobileFeaturedPresentation:'single-column-bestseller-teaser'});
    ...
    expect(mobile.sections.slice(0,5).map(section=>section.id)).toEqual(['beauty-home-site-header','beauty-formula-hero','beauty-usp-row','beauty-new-formulas','beauty-formula-finder']);
    expect(desktop.sections.slice(0,5).map(section=>section.id)).toEqual(['beauty-home-site-header','beauty-formula-hero','beauty-usp-row','beauty-formula-finder','beauty-ingredient-index']);
    expect(readStorefrontFidelityMetadata(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE)?.designGuard).toMatchObject({presetId:'beauty-lab-reference-v2.5-home',baselineVersion:5});
  });
  ...
  it('preserves the recovered hero, texture and bestseller contracts without intermediate wrappers',()=>{
    ...
    expect(resolveStorefrontVisualStyle(find(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.sections,'beauty-formula-hero')?.config.style,'mobile')).toMatchObject({height:'24.5rem',minHeight:'24.5rem'});
    const texture=find(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.sections,'beauty-texture-navigation');
    expect((texture?.bindings?.items?.fallback as Array<{label:string}>).map(item=>item.label)).toEqual(['GÉL','KRÉM','MILK','OLAJ']);
    const featured=find(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.sections,'newFormulas');
    expect((featured?.bindings?.products?.fallback as Array<{badge?:string}>)[0]?.badge).toBe('BESTSELLER');
  });
  ...
  it('preserves compact shared Home style-slot fidelity from the recovery passes',()=>{
    ...
    expect(finder?.config).toMatchObject({actionLabel:'Tovább',copy:''});
    ...
    expect(resolveStorefrontStyleSlot(find(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.sections,'beauty-ingredient-index-block')?.config.styleSlots,'media','desktop').aspectRatio).toBe('1.48 / 1');
    expect(resolveStorefrontStyleSlot(find(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.sections,'beauty-texture-navigation')?.config.styleSlots,'media','desktop').aspectRatio).toBe('2.5 / 1');
    const featured=find(BEAUTY_LAB_REFERENCE_V28_HOME_PAGE.sections,'newFormulas');
  ...
  it('replaces Home and PDP generic trust workarounds with the shared trust-strip primitive',()=>{
    ...
    expect(homeTrust).toMatchObject({componentKey:'content.trust-strip',componentVersion:1});
    ...
    expect(productTrust).toMatchObject({componentKey:'content.trust-strip',componentVersion:1});
  ...
  it('keeps the recovered PDP flow and buybox ordering while using shared commerce primitives',()=>{
    expect(BEAUTY_LAB_REFERENCE_V28_PRODUCT_PAGE.sections.slice(0,5).map(section=>section.id)).toEqual(['beauty-product-site-header','beauty-product-main','beauty-product-tabs','beauty-product-specifications','beauty-product-related']);
    ...
    expect(purchase.bindings).toMatchObject({productId:{path:'product.id',fallback:''},availableQuantity:{path:'inventory.availableQuantity',fallback:0},minimumQuantity:{path:'inventory.minimumQuantity',fallback:1},orderMultiple:{path:'inventory.orderMultiple',fallback:1}});
    ...
    expect(BEAUTY_LAB_REFERENCE_V28_PRODUCT_PAGE.metadata).toMatchObject({pdpFlow:'main-tabs-ingredients-related-results',purchaseControls:'shared-functional-quantity-cart-wishlist-v1',canonicalComposition:'flattened-v2.8-from-v2.4'});
  });
  ...
  it('wires before/after to merchant evidence while failing closed by default',()=>{
    ...
    expect(evidence).toMatchObject({componentKey:'editorial.before-after',componentVersion:1});
    expect(evidence.config).toMatchObject({beforeImage:'',afterImage:'',evidenceStatus:'unverified',presentation:'merchant-evidence'});
    expect(evidence.bindings).toMatchObject({beforeImage:{path:'content.beforeAfter.beforeImage',fallback:''},afterImage:{path:'content.beforeAfter.afterImage',fallback:''},evidenceStatus:{path:'content.beforeAfter.evidenceStatus',fallback:'unverified'}});
  ...
  it('keeps the Visual Builder renderer registry symmetric with every Beauty Lab page node',()=>{
    ...
    for(const page of BEAUTY_LAB_CANONICAL_V2_PACKAGE.pages){
      for(const node of flatten(page.sections))expect(renderers.get(node.componentKey,node.componentVersion),`${page.pageType}:${node.id}:${node.componentKey}@${node.componentVersion}`).toBeDefined();
    }
  ...
  it('promotes flattened v2.8 as the canonical v2 source without creating a new template identity',()=>{
    ...
    const recomposed=BEAUTY_LAB_CANONICAL_V2_PACKAGE.pages.filter(page=>page.pageType==='home'||page.pageType==='product');
    expect(recomposed).toHaveLength(2);
    expect(recomposed.every(page=>page.metadata?.canonicalComposition==='flattened-v2.8-from-v2.4')).toBe(true);
  });
});