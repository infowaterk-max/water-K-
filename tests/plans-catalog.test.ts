...
const ALAP_REQUIRED: FeatureCode[] = [
  'catalog','inventory','orders','returns','customers','coupons','basicAnalytics','marketingBasics','contentMarketing','importExport','bulkOperations','wishlists','stockNotifications','productRecommendations','reviews','searchFiltering','commerceIntegrations','support','teamChat','officeCommunication',
];

const PRO_ONLY: FeatureCode[] = [
  'advancedAnalytics','crm','advancedCampaigns','officeCommunicationAdvanced','automation','procurement','cashflow','executiveAnalytics','advancedIntegrations','apiAccess','interactiveSceneCommerce','recipeCommerce',
];
...
describe('business plan entitlement matrix', () => {
  ...
  it('releases API Access only in Pro while planned capabilities remain disabled', () => {
  ...
  it('prevents accidental package drift', () => {
    ...
    for (const feature of PLANS.alap.features) expect(PLANS.pro.features).toContain(feature);
  });

  it('accepts only supported persisted package codes', () => {
    expect(isPlanCode('alap')).toBe(true);expect(isPlanCode('pro')).toBe(true);expect(isPlanCode('bronze')).toBe(false);expect(isPlanCode('gold')).toBe(false);expect(isPlanCode('')).toBe(false);expect(isPlanCode(null)).toBe(false);
  });
});
