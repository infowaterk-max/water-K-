...
try{
  for(const pageType of pageTypes){
    ...
    const roots=page.locator('[data-visual-fidelity-root="runtime"]:visible');if(await roots.count()!==1)throw new Error(`PLAYROOM_V20_RUNTIME_ROOT_COUNT:${pageType}:${await roots.count()}`);
    const root=roots.first();await root.waitFor({state:'visible',timeout:15000});await page.waitForTimeout(180);const box=await root.boundingBox();if(!box)throw new Error(`PLAYROOM_V20_ROOT_BOX_MISSING:${pageType}`);
    const path=`${outputDir}/playroom-${pageType}-desktop-full.png`;await root.screenshot({path,animations:'disabled',timeout:20000});captures.push({pageType,url,path,width:box.width,height:box.height,title:await page.title()});await page.close();
  }
}finally{await browser.close();}
const evidence={version:'shoporation.playroom-page-family.v20',template:'gaming.playroom',templateVersion,sourceCommit:process.env.GITHUB_SHA??null,viewport:{width:1440,height:1000},captures,capturedAt:new Date().toISOString()};
await writeFile(`${outputDir}/manifest.json`,JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence,null,2));
