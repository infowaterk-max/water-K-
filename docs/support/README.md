...
35. **Direct-root reusable components must own their storefront text-token inheritance.** Do not rely on an application/body color or add a wrapper solely to repair contrast; preserve stable node/component identity and fix the shared renderer when the component is the authority.
36. **Representative product Preview media belongs to demo fixture authority, not incidental page-local artwork.** Listing pages must not degrade to blank product media merely because their Page Schema contains no decorative image; never add hidden components just to seed Preview imagery.
37. **Platform template acceptance and merchant template installation are separate entry authorities.** In Preview, an authenticated platform operator may inspect representative template output without a selected tenant; merchant preview remains tenant- and entitlement-aware, and missing context must produce an explicit access state rather than a generic runtime error.
38. **Technical candidate is not Product Owner preview.** CI, 14×3 responsive/browser proof and Runtime health may validate a skeleton, but they do not make it a finished template. Product Owner preview stays closed until the accepted visual reference, representative media, shopper-facing polish and internal screenshot review are complete and the quality manifest explicitly marks the template review-ready.
