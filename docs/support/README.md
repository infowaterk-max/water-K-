...
37. **Planned media is not ready media.** Template recipes may declare the complete production media plan before assets exist, but only assets explicitly marked `ready` count toward representative-media coverage or Product Owner readiness. Fixture-owned product media counts as wired only when the fixture references the declared asset; filenames alone never satisfy readiness.
38. **Ready local media requires physical-file proof.** A local Factory asset may be marked `ready` only when the exact file exists under `public/`; the Template Factory quality workflow must run this proof so a missing upload or typo cannot satisfy media readiness.
