-- Customer baseline mirror: harden storefront social profile settings to HTTPS and provider-owned domains.
...
declare
  ...
  v_value text;
begin
  if p_instance_id is null or p_actor is null then raise exception 'STOREFRONT_SOCIAL_IDENTITY_REQUIRED'; end if;
  if not public.can_manage_storefront(p_instance_id,p_actor) then raise exception 'STOREFRONT_MANAGE_PERMISSION_REQUIRED'; end if;
  if p_social_links is null or pg_catalog.jsonb_typeof(p_social_links)<>'object' then raise exception 'STOREFRONT_SOCIAL_PAYLOAD_INVALID'; end if;
  if pg_catalog.octet_length(p_social_links::text)>12000 then raise exception 'STOREFRONT_SOCIAL_PAYLOAD_TOO_LARGE'; end if;

  for v_key in select pg_catalog.jsonb_object_keys(p_social_links)
  loop
    if v_key not in ('facebook','instagram','youtube','tiktok','x','twitch','linkedin','pinterest') then raise exception 'STOREFRONT_SOCIAL_PROVIDER_INVALID'; end if;
    if pg_catalog.jsonb_typeof(p_social_links->v_key)<>'string' then raise exception 'STOREFRONT_SOCIAL_URL_INVALID'; end if;
    v_value:=pg_catalog.btrim(p_social_links->>v_key);
    if pg_catalog.char_length(v_value)<8 or pg_catalog.char_length(v_value)>500 then raise exception 'STOREFRONT_SOCIAL_URL_INVALID'; end if;
    ...
    then raise exception 'STOREFRONT_SOCIAL_URL_INVALID'; end if;
  end loop;
