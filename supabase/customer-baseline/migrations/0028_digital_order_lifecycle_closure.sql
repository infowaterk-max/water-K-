...
begin
  ...
  if v_order.status=v_target_status then
    return jsonb_build_object('order_id',v_order.id,'status',v_order.status,'replayed',true,'inventory_restored',false);
  end if;
  ...
  v_allowed:=case v_order.status
    ...
    when 'processing' then case when v_order.fulfillment_mode='digital' then v_target_status in ('completed','refunded') else v_target_status in ('shipped','refunded') end
    when 'shipped' then v_target_status in ('completed','refunded')
    when 'completed' then v_target_status='refunded'
    else false
  end;

  if not v_allowed then raise exception 'Nem engedélyezett rendelési állapotváltás: % -> %',v_order.status,v_target_status; end if;

  if v_order.fulfillment_mode in('digital','mixed')
     and v_target_status in('processing','shipped','completed')
     and v_order.paid_at is null then
    raise exception 'DIGITAL_COMMERCE_PAYMENT_EVIDENCE_REQUIRED';
  end if;
  ...
  if v_target_status='cancelled' then
    for v_line in
      ...
      where oi.order_id=p_order_id and oi.instance_id=p_instance_id and oi.variant_id is not null
    ...
    loop
      ...
      where r.instance_id=p_instance_id and r.order_item_id=v_line.id;
      v_remaining:=greatest(0,v_line.quantity-v_already);
      if v_remaining>0 then
        select public.restore_order_item_inventory_v1(p_instance_id,p_order_id,v_line.id,'order_cancelled',p_order_id,v_remaining,p_actor) into v_result;
  ...
  set status=v_target_status,
      ...
      paid_at=case when v_target_status='paid' and paid_at is null then now() else paid_at end,
      updated_at=now()
  where id=p_order_id and instance_id=p_instance_id;
  ...
  return jsonb_build_object('order_id',p_order_id,'status',v_target_status,'inventory_restored',v_restore,'replayed',false);
end;
$$;
...
is 'Tenant-safe fulfillment-aware order lifecycle transition. Digital/mixed fulfillment requires canonical paid_at evidence; pure digital skips physical shipment.';
