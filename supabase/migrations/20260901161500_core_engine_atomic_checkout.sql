-- Core Engine 2.0: atomic tenant checkout authority.
-- Replaces the broken v4->v3->v2->place_order legacy dependency with one transaction-safe tenant function.
...
begin
 ...
 -- Lock coupon before pricing/consumption so concurrent checkouts cannot exceed its usage limit.
 if v_code<>'' then
  select * into v_coupon from public.coupons where instance_id=p_instance_id and code=v_code for update;
 ...
 for v_item in select * from jsonb_array_elements(p_items) loop
  ...
  select pv.id,pv.product_id,pv.sku,pv.label,pv.gross_price_huf,pv.reseller_gross_price_huf,pv.stock_quantity,pv.active,pv.unit_cost_net_huf,p.net_price_huf as product_net,p.name product_name,p.active product_active into v_variant
  from public.product_variants pv join public.products p on p.id=pv.product_id where pv.id=(v_item->>'variant_id')::uuid and pv.instance_id=p_instance_id and p.instance_id=p_instance_id for update of pv;
  ...
  insert into public.order_items(instance_id,order_id,variant_id,product_name,variant_label,sku,quantity,unit_gross_huf,line_total_gross_huf,unit_cost_net_huf_snapshot,cost_snapshot_source)
  values(p_instance_id,v_order_id,v_variant.id,v_variant.product_name,v_variant.label,v_variant.sku,v_qty,v_price,v_price*v_qty,v_variant.unit_cost_net_huf,case when v_variant.unit_cost_net_huf is null then null else 'variant' end);
 ...
 if v_code<>'' then
  ...
  v_discount:=greatest(0,least(v_discount,v_subtotal));update public.coupons set usage_count=usage_count+1,updated_at=now() where id=v_coupon.id and instance_id=p_instance_id;
 end if;
 v_shipping:=case when p_shipping_kind='pickup' then 0 when p_free_shipping_threshold_huf>0 and (v_subtotal-v_discount)>=p_free_shipping_threshold_huf then 0 else p_shipping_fee_huf end;
 ...
 insert into public.order_events(instance_id,order_id,event_type,to_status,actor_user_id,metadata) values(p_instance_id,v_order_id,'order_created','pending',p_customer_id,jsonb_build_object('payment_method',p_payment_provider,'shipping_method',p_shipping_provider,'shipping_kind',p_shipping_kind));
 if v_code<>'' then insert into public.order_events(instance_id,order_id,event_type,to_status,actor_user_id,metadata) values(p_instance_id,v_order_id,'coupon_applied','pending',p_customer_id,jsonb_build_object('code',v_code,'discount_gross_huf',v_discount));end if;
 ...
 update public.order_request_keys set response=v_response,request_fingerprint=v_fp where idempotency_key=v_key;
 return v_response;
end $$;
