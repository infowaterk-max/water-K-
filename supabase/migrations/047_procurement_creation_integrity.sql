...
begin
  if p_items is null or jsonb_typeof(p_items) <> 'array' or jsonb_array_length(p_items)=0 then
    raise exception 'A beszerzéshez legalább egy tétel szükséges.';
  end if;
  ...
  values(p_order_number,p_supplier_id,'draft',p_expected_at,p_payment_due_at,v_total,p_notes,p_created_by)
...
revoke all on function public.create_purchase_order(text,uuid,date,date,text,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.create_purchase_order(text,uuid,date,date,text,uuid,jsonb) to service_role;
