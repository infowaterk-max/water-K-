-- Tenant-safe replacement for the legacy bulk catalog RPC.
-- The legacy function is intentionally left restricted to service_role for compatibility,
-- while all application callers must use the v2 function with an explicit instance_id.

create or replace function public.bulk_update_product_variants_v2(
  p_instance_id uuid,
  p_changes jsonb,
  p_actor uuid
)
...
begin
  if p_instance_id is null then
    raise exception 'p_instance_id is required';
  end if;
  ...
  loop
    ...
    if not found then
      raise exception 'variant not found in instance: %', variant_id;
    end if;
    ...
    if new_stock <> current_row.stock_quantity then
      ...
      values(
        p_instance_id,
        variant_id,
        new_stock-current_row.stock_quantity,
        current_row.stock_quantity,
        new_stock,
        'bulk_admin_adjustment',
        p_actor,
        jsonb_build_object('sku',current_row.sku)
      );
    end if;
    ...
    results := results || jsonb_build_array(
      jsonb_build_object(
        ...
        'before',jsonb_build_object(
          'stock',current_row.stock_quantity,
          'grossPrice',current_row.gross_price_huf,
          'netPrice',current_row.net_price_huf,
          'active',current_row.active
        ),
        'after',jsonb_build_object(
          'stock',new_stock,
          'grossPrice',new_gross,
          'netPrice',new_net,
          'active',new_active
        )
      )
...
revoke all on function public.bulk_update_product_variants_v2(uuid,jsonb,uuid) from public, anon, authenticated;
grant execute on function public.bulk_update_product_variants_v2(uuid,jsonb,uuid) to service_role;

comment on function public.bulk_update_product_variants_v2(uuid,jsonb,uuid)
is 'Tenant-scoped atomic bulk catalog update. Application callers must pass the authorized webshop instance_id.';
