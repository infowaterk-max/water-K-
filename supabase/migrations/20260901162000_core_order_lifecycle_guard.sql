-- Core Engine 2.0: tenant-safe lifecycle. Cancellation restores unfulfilled stock; refund is financial only.
create table if not exists public.order_inventory_restorations(
 id uuid primary key default gen_random_uuid(),instance_id uuid not null references public.webshop_instances(id) on delete cascade,order_id uuid not null references public.orders(id) on delete cascade,order_item_id uuid not null references public.order_items(id) on delete cascade,source_type text not null check(source_type in('order_cancelled','return_case')),source_id uuid not null,quantity integer not null check(quantity>0),actor_user_id uuid references auth.users(id),created_at timestamptz not null default now(),unique(instance_id,order_item_id,source_type,source_id));
...
revoke all on function public.restore_order_item_inventory_v1(uuid,uuid,uuid,text,uuid,integer,uuid) from public,anon,authenticated;grant execute on function public.restore_order_item_inventory_v1(uuid,uuid,uuid,text,uuid,integer,uuid) to service_role;
...
revoke all on function public.restock_return_case(uuid,uuid) from public,anon,authenticated;grant execute on function public.restock_return_case(uuid,uuid) to service_role;
...
revoke all on function public.transition_tenant_order_v1(uuid,uuid,uuid,text,text) from public,anon,authenticated;grant execute on function public.transition_tenant_order_v1(uuid,uuid,uuid,text,text) to service_role;
