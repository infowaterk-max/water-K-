alter table public.product_variants add column if not exists reseller_net_price_huf integer check (reseller_net_price_huf is null or reseller_net_price_huf >= 0);
alter table public.product_variants add column if not exists reseller_gross_price_huf integer check (reseller_gross_price_huf is null or reseller_gross_price_huf >= 0);

-- Approved reseller pricing is resolved only inside place_order(). Public catalog
-- queries keep returning retail prices; reseller prices remain server-side/admin data.
...
declare v_order_id uuid; v_order_number text; v_subtotal integer:=0; v_shipping integer:=0; v_total integer:=0; v_item jsonb; v_variant record; v_qty integer; v_role public.customer_role; v_reseller_approved boolean:=false; v_previous_stock integer; v_unit_price integer;
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    select pv.id,pv.product_id,pv.sku,pv.label,pv.gross_price_huf,pv.net_price_huf,pv.reseller_gross_price_huf,pv.reseller_net_price_huf,pv.stock_quantity,pv.active,p.name as product_name,p.active as product_active into v_variant from public.product_variants pv join public.products p on p.id=pv.product_id where pv.id=(v_item->>'variant_id')::uuid for update of pv;
