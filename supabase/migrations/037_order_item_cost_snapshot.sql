-- V8: freeze unit cost at order creation so historical COGS/margin does not change
-- when the current product cost is edited later.
alter table public.order_items
  add column if not exists unit_cost_net_huf_snapshot numeric(12,2),
  add column if not exists cost_snapshot_source text;

alter table public.order_items
  drop constraint if exists order_items_unit_cost_snapshot_check;
alter table public.order_items
  add constraint order_items_unit_cost_snapshot_check
  check (unit_cost_net_huf_snapshot is null or unit_cost_net_huf_snapshot >= 0);

-- Historical rows cannot be reconstructed perfectly. We seed them from the current
-- variant cost and explicitly mark the provenance so analytics can disclose it.
update public.order_items oi
set unit_cost_net_huf_snapshot = pv.unit_cost_net_huf,
    cost_snapshot_source = 'current_cost_backfill'
from public.product_variants pv
where oi.variant_id = pv.id
  and oi.unit_cost_net_huf_snapshot is null
  and pv.unit_cost_net_huf is not null;
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    select pv.id,pv.product_id,pv.sku,pv.label,pv.gross_price_huf,pv.reseller_gross_price_huf,pv.stock_quantity,pv.unit_cost_net_huf,pv.active,p.name as product_name,p.active as product_active into v_variant from public.product_variants pv join public.products p on p.id=pv.product_id where pv.id=(v_item->>'variant_id')::uuid for update of pv;
    ...
    insert into public.order_items(order_id,variant_id,product_name,variant_label,sku,quantity,unit_gross_huf,line_total_gross_huf,unit_cost_net_huf_snapshot,cost_snapshot_source)
    values(v_order_id,v_variant.id,v_variant.product_name,v_variant.label,v_variant.sku,v_qty,v_unit_price,v_unit_price*v_qty,v_variant.unit_cost_net_huf,'order_created');
