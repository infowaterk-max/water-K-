...
-- Block 20 is the explicit release point for the previously reserved Pro API capability.
...
-- Refresh only derived plan entitlements through the existing Block 11 authority.
do $$
declare r record;
begin
...
create index if not exists extension_api_credentials_instance_installation_idx
  on public.extension_api_credentials(instance_id,installation_id);
create index if not exists extension_webhook_subscriptions_instance_event_idx
  on public.extension_webhook_subscriptions(instance_id,event_type) where enabled;
create index if not exists extension_webhook_deliveries_due_idx
  on public.extension_webhook_deliveries(status,next_attempt_at,created_at)
