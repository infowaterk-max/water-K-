...
-- Reuse Block 17's canonical event evidence. This trigger only fans out sanitized evidence;
-- it cannot create events or mutate commerce state.
...
begin
  ...
  select
    ...
    new.run_key,
    jsonb_build_object(
      'sourceId',left(coalesce(v_event->>'sourceId',''),180),
      'occurredAt',v_event->>'occurredAt',
      'evidence',coalesce(v_event->'evidence','{}'::jsonb)
    ),
    'pending'
  from public.extension_webhook_subscriptions s
  join public.extension_installations i
    on i.id=s.installation_id and i.instance_id=s.instance_id and i.status='enabled'
  join public.extension_app_catalog a
    on a.app_key=i.app_key and a.release_state='released'
  where s.instance_id=new.instance_id
    and s.enabled
    and s.event_type=v_event_type
