-- Visual Builder Global Styles additive accent palette hardening.
-- Forward-only replacement of the existing draft-only RPC validation. No publish authority is added.
...
begin
  ...
  if jsonb_object_length(p_global_styles->'tokens')>18 then raise exception 'STOREFRONT_GLOBAL_STYLES_TOKENS_TOO_LARGE'; end if;

  for v_key in select jsonb_object_keys(p_global_styles->'tokens') loop
    if v_key<>all(array['background','surface','surfaceMuted','text','mutedText','border','primary','primaryContrast','accent','accentSecondary','accentTertiary','headingFont','bodyFont','spacingScale','radiusScale']) then
      raise exception 'STOREFRONT_GLOBAL_STYLES_TOKEN_UNKNOWN';
    end if;
  end loop;
  for v_key in select unnest(array['background','surface','surfaceMuted','text','mutedText','border','primary','primaryContrast','accent','accentSecondary','accentTertiary']) loop
...
comment on function public.save_storefront_global_style_drafts_v1(uuid,uuid,jsonb,jsonb,text) is 'Atomically snapshots one validated tenant-wide global style state into Page Schema drafts only, including the controlled secondary/tertiary accent palette. It has no publish authority.';
